...
- [ ] 形成一份明确的文档更新范围，区分“当前事实文档”“历史但需补状态说明的根文档”“保留原状的历史/ADR 文档”。
- [ ] 选定范围内的当前事实文档不再把已删除的 `code_assistant` CLI 方案写成当前主线。
- [ ] 选定范围内的当前事实文档不再把旧的 Rust + axum / SQLite / `LlmGatewayBinding` / `PluginDraft` 等历史方案写成当前主线，除非明确标注为历史背景。
- [ ] README 与核心架构文档中的技术栈、目录结构、产品主链路可直接用于对外介绍项目，不再与现有代码冲突。
- [ ] 对保留历史定位但仍在 `docs/` 根目录的文档，补充足够明显的状态说明或跳转，避免读者误判其为当前实现。
