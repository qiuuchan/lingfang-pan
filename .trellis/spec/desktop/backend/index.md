...
适用于 `apps/desktop` 视角下跨前端与 Tauri 后端的插件/SDK Runtime 架构说明。真正的 Rust 命令、进程管理、插件持久化和能力网关实现规范归属 `.trellis/spec/lingfang-desktop/backend/`。

## Pre-Development Checklist

- 修改插件创建、插件运行、SDK Runtime 模型服务连接或桌面/后端交互流程时，先读 [plugin-system-architecture.md](./plugin-system-architecture.md)。
