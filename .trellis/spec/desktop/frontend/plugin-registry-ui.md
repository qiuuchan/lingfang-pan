...
- 远端未下载项没有运行按钮；Installed 行只提供运行、详情和卸载，builtin 不可卸载。回滚、复制编辑等修改入口不得放回 Installed。
- 插件中心负责团队/市场下载与本机卸载；本地制品上传、导入为可编辑 workspace 和进入 Agent 编辑统一放在 Draft 页。底层本地 installation/import API 可以保留，但 Installed 页不暴露上传入口。
- Draft 页提供搜索/状态筛选/创建/继续编辑/预览/发布/导入/导出/删除；恢复 `conversationId`，删除时清关联 localStorage conversation。
- Installed/Team/Marketplace 三个目录均使用客户端来源筛选和分页；来源筛选变化或切回 Tab 时页码归 1，数据缩减时页码必须夹紧。Marketplace 使用统一列表，精选/优质通过 badge 表达，不按会重复插件的分组切片分页。
...
- 需要修改已安装插件 -> 从 Draft/已发布工作台创建或选择 workspace，禁止直接写发行目录；Installed 不提供编辑或回滚按钮。

### 5. Good/Base/Bad Cases

- Good：离线打开 Installed 可运行或卸载已购市场插件；上传/编辑在 Draft，发布后治理在“已发布”。
- Base：release 已是 active 或 pending 时显示“已下载”，catalog row 没有运行入口；来源筛选后分页从第 1 页开始。
- Bad：把上传、复制编辑或回滚重新塞进 Installed，或让 Marketplace 分组导致同一插件跨分页重复出现。

### 6. Tests Required

- 三 Tab 边界、未下载不可运行、本地/团队/市场来源 badge 与筛选、分页夹紧、购买/更新/进度错误。
- Installed 操作边界：只有运行、详情、卸载；builtin 卸载禁用；无上传、复制编辑和回滚入口。
