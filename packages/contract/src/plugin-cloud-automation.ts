...
export const CloudActionDeployment = z.object({
  id: Identifier,
  ...
  response_limit_bytes: z.number().int().min(1_024).max(16 * 1024 * 1024),
...
export const CloudActionRouting = z.object({
...
export const WorkflowCloudBinding = z.object({
...
export const AutomationScheduleStatus = z.enum(['ACTIVE', 'PAUSED', 'COMPLETED', 'MISSED', 'DELETED']);
...
export const AutomationScheduleTrigger = z.discriminatedUnion('kind', [
  ...
  z.object({ kind: z.literal('WEEKLY'), time_zone: TimeZone, day_of_week: z.number().int().min(1).max(7), local_time: LocalTime }).strict(),
]);
...
export const AutomationOutboxKind = z.enum(['UPSERT_SCHEDULE', 'REMOVE_SCHEDULE', 'ENQUEUE_RUN', 'ENQUEUE_ACTION', 'CANCEL_RUN']);
export const AutomationOutboxStatus = z.enum(['PENDING', 'PROCESSING', 'DONE', 'FAILED']);
export const AutomationOutboxRecord = z.object({
  id: Identifier,
...
export const CloudUsageOutcome = z.enum(['SUCCEEDED', 'FAILED', 'CANCELED', 'TIMED_OUT', 'RESULT_UNKNOWN']);
export const CloudUsageEvent = z.object({
  id: Identifier,
  team_id: Identifier,
...
export const CloudWorkflowAttemptTransport = z.object({
...
export const CloudWorkflowRunScheduleContext = z.object({
  schedule_id: Identifier,
