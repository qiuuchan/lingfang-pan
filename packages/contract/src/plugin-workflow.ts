...
export const WorkflowNodeId = z.string().trim().regex(/^[a-z][a-z0-9_-]{0,63}$/);
const JsonPointer = z.string().max(1024).refine((value) => value === '' || value.startsWith('/') && value.split('/').slice(1).every((part) => !/~(?:[^01]|$)/.test(part)), 'must be a valid RFC 6901 JSON Pointer');
export type WorkflowJsonValue = null | string | number | boolean | WorkflowJsonValue[] | { [key: string]: WorkflowJsonValue };
export const WorkflowJsonValue: z.ZodType<WorkflowJsonValue> = z.lazy(() => z.union([
  z.null(),
  z.string(),
  z.number().finite(),
  z.boolean(),
...
const BindingSource = z.discriminatedUnion('kind', [
  ...
  z.object({ kind: z.literal('node_output'), node_id: WorkflowNodeId, source_pointer: JsonPointer }).strict(),
  z.object({ kind: z.literal('literal'), value: WorkflowJsonValue }).strict(),
]);
export const WorkflowBinding = z.object({ target_pointer: JsonPointer, source: BindingSource }).strict();
export type WorkflowBinding = z.infer<typeof WorkflowBinding>;

export const WorkflowDefinitionNodeV1 = z.object({
  node_id: WorkflowNodeId,
  declared_version_range: z.string().trim().min(1).max(128),
  target: ActionTarget,
  depends_on: z.array(WorkflowNodeId).max(WORKFLOW_MAX_NODES).transform((items) => [...new Set(items)].sort()),
  input_bindings: z.array(WorkflowBinding).max(256),
  retry_limit: z.union([z.literal(0), z.literal(1), z.literal(2)]),
}).strict();

export const WorkflowDefinitionV1 = z.object({
  ...
  definition.nodes.forEach((node, index) => {
    if (ids.has(node.node_id)) ctx.addIssue({ code: z.ZodIssueCode.custom, path: ['nodes', index, 'node_id'], message: 'node_id must be unique' });
    ids.add(node.node_id); edges += node.depends_on.length;
    const targets = new Set<string>(); node.input_bindings.forEach((binding, bindingIndex) => { if (targets.has(binding.target_pointer)) ctx.addIssue({ code: z.ZodIssueCode.custom, path: ['nodes', index, 'input_bindings', bindingIndex, 'target_pointer'], message: 'target pointer must be unique' }); targets.add(binding.target_pointer); });
  });
  if (edges > WORKFLOW_MAX_EDGES) ctx.addIssue({ code: z.ZodIssueCode.custom, path: ['nodes'], message: `workflow may contain at most ${WORKFLOW_MAX_EDGES} dependency edges` });
  definition.nodes.forEach((node, index) => node.depends_on.forEach((dependency) => { if (!ids.has(dependency)) ctx.addIssue({ code: z.ZodIssueCode.custom, path: ['nodes', index, 'depends_on'], message: `unknown dependency: ${dependency}` }); if (dependency === node.node_id) ctx.addIssue({ code: z.ZodIssueCode.custom, path: ['nodes', index, 'depends_on'], message: 'node cannot depend on itself' }); }));
  const outputTargets = new Set<string>(); definition.output_bindings.forEach((binding, index) => { if (outputTargets.has(binding.target_pointer)) ctx.addIssue({ code: z.ZodIssueCode.custom, path: ['output_bindings', index, 'target_pointer'], message: 'target pointer must be unique' }); outputTargets.add(binding.target_pointer); });
});
...
export const WorkflowRunStatus = z.enum(['PENDING', 'RUNNING', 'FAILING', 'SUCCEEDED', 'FAILED', 'CANCELING', 'CANCELED']);
export type WorkflowRunStatus = z.infer<typeof WorkflowRunStatus>;
export const WorkflowStepStatus = z.enum(['PENDING', 'READY', 'RUNNING', 'SUCCEEDED', 'FAILED', 'SKIPPED', 'CANCELED']);
export type WorkflowStepStatus = z.infer<typeof WorkflowStepStatus>;
export const WorkflowErrorCode = z.enum(['workflow_invalid', 'workflow_cycle_detected', 'workflow_limit_exceeded', 'workflow_mapping_invalid', 'workflow_policy_denied', 'workflow_run_conflict', 'workflow_executor_unavailable', 'workflow_executor_session_invalid', 'workflow_inventory_changed', 'workflow_installation_mismatch', 'workflow_lease_expired', 'workflow_cancelled', 'workflow_side_effect_unknown', 'workflow_step_failed']);

const Digest = z.string().regex(/^[a-f0-9]{64}$/);
const IsoDate = z.string().datetime();
const NullableIsoDate = IsoDate.nullable();

export const WorkflowExecutionPlanNode = z.object({
  node_id: WorkflowNodeId,
  declared_version_range: z.string().trim().min(1).max(128),
  target: ActionTarget,
 ...
 */
export const WorkflowFrozenSubplan = z.object({
...
export const WorkflowExecutionPlan = z.object({
  ...
  workflow_subplans: z.array(WorkflowFrozenSubplan).max(WORKFLOW_MAX_NESTING_DEPTH * WORKFLOW_MAX_NODES).default([]),
  output_bindings: z.array(WorkflowBinding).max(256),
  desktop_executor: z.object({
...
export const WorkflowPreflightDiagnostic = z.object({
...
export const WorkflowRunCreateRequest = z.object({
...
export const WorkflowPreflightResponse = z.object({
  eligible: z.boolean(),
  workflow_release_id: z.string().min(1),
  execution_target: WorkflowExecutionTarget,
  execution_scope: WorkflowExecutionScope,
...
export const WorkflowRunError = z.object({
  code: z.string().max(128),
  message: z.string().max(1000),
}).strict().nullable();

export const WorkflowAttemptCounts = z.object({
...
export const WorkflowStepAttemptDTO = z.object({
  id: z.string().min(1),
...
export const WorkflowRunListRequest = z.object({
...
export const WorkflowRunListResponse = z.object({
...
export const WorkflowUpgradeSuggestion = z.object({
  node_id: WorkflowNodeId,
  declared_version_range: z.string().trim().min(1).max(128),
...
export const WorkflowUpgradeSuggestionResponse = z.object({
  workflow_release_id: z.string().min(1),
  workflow_release_sha256: Digest,
...
export const DesktopExecutorInventoryEntry = z.object({ installation_id: z.string().min(1), package_id: z.string().min(1), release_id: z.string().min(1), sha256: Digest, dependency_status: z.enum(['pending', 'preparing', 'ready', 'failed']) }).strict();
export const DesktopExecutorSession = z.object({ id: z.string().min(1), device_id: z.string().min(1), inventory_sha256: Digest, status: z.enum(['ACTIVE', 'EXPIRED', 'REVOKED']), expires_at: IsoDate, last_heartbeat_at: IsoDate }).strict();
export const CreateDesktopExecutorSessionRequest = z.object({ device_id: z.string().min(1).max(256), inventory: z.array(DesktopExecutorInventoryEntry).max(512) }).strict();
export const CreateDesktopExecutorSessionResponse = z.object({ session: DesktopExecutorSession, token: z.string().min(32) }).strict();
export const HeartbeatDesktopExecutorSessionRequest = z.object({ inventory: z.array(DesktopExecutorInventoryEntry).max(512) }).strict();
export const HeartbeatDesktopExecutorSessionResponse = z.object({ session: DesktopExecutorSession }).strict();
...
export const WorkflowExecutorClaimResponse = z.object({
  attempt: z.object({ id: z.string().min(1), run_id: z.string().min(1), node_id: WorkflowNodeId, input: WorkflowJsonValue, target: ActionTarget, deadline_at: IsoDate, lease_expires_at: IsoDate, lease_token: z.string().min(32) }).strict().nullable(),
}).strict();
...
export const WorkflowExecutorHeartbeatResponse = z.object({ ok: z.literal(true), lease_expires_at: z.string().datetime() }).strict();
