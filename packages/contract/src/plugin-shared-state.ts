...
export const SharedNamespaceDeclaration = z.object({
  name: z.string().trim().regex(/^[a-z][a-z0-9._-]{0,63}$/),
  ...
  value.schemas.forEach((entry, index) => {
    if (versions.has(entry.schema_version)) ctx.addIssue({ code: z.ZodIssueCode.custom, path: ['schemas', index, 'schema_version'], message: 'schema_version must be unique' });
    versions.add(entry.schema_version);
  });
  if (!versions.has(value.active_schema_version)) ctx.addIssue({ code: z.ZodIssueCode.custom, path: ['active_schema_version'], message: 'active_schema_version must have a matching schema' });
});
export type SharedNamespaceDeclaration = z.infer<typeof SharedNamespaceDeclaration>;

export const SharedNamespace = z.object({
  id: Identifier,
  team_id: Identifier,
...
export const SharedArtifactEdge = z.object({
...
export const SharedValue = z.object({
  namespace_id: Identifier,
  namespace_generation: z.number().int().positive(),
  key: z.string().trim().min(1).max(SHARED_KEY_MAX_LENGTH),
...
export const SharedWrite = z.object({
  value: WorkflowJsonValue,
  schema_version: z.number().int().positive(),
...
export const SharedSchemaMigration = z.object({
  value: WorkflowJsonValue,
  source_schema_version: z.number().int().positive(),
  target_schema_version: z.number().int().positive(),
  expected_revision: DecimalCursor,
}).strict().refine((value) => value.source_schema_version !== value.target_schema_version, {
...
export const SharedNamespaceReactivate = z.object({
  active_schema_version: z.number().int().positive(),
}).strict();
export type SharedNamespaceReactivate = z.infer<typeof SharedNamespaceReactivate>;

export const SharedNamespaceLifecycleResult = z.object({
  namespace_id: Identifier,
  namespace_generation: z.number().int().positive(),
  active_schema_version: z.number().int().positive(),
  next_value_revision: DecimalCursor,
...
export const SharedNamespaceExportLine = z.object({
  namespace_id: Identifier,
  namespace_generation: z.number().int().positive(),
  key: z.string().trim().min(1).max(SHARED_KEY_MAX_LENGTH),
  value: WorkflowJsonValue,
...
export const SharedPage = z.object({
...
export const SharedConflict = z.object({
...
export const SharedChangeEvent = z.object({
  namespace_id: Identifier,
  namespace_generation: z.number().int().positive(),
...
export const SharedChangesPage = z.object({
...
export const SharedRealtimeInvalidation = z.object({
  cursor: DecimalCursor,
  key: z.string().trim().min(1).max(SHARED_KEY_MAX_LENGTH),
  revision: DecimalCursor,
}).strict();
...
export const SharedPresenceContext = z.object({
...
export const SharedPresenceMember = z.object({
  user_id: Identifier,
...
export const SharedPresenceSnapshot = z.object({
  namespace_id: Identifier,
  namespace_generation: z.number().int().positive(),
