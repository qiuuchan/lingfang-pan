...
export const ActionId = z.string().trim().regex(ACTION_ID_PATTERN, 'action_id must match ^[a-z][a-z0-9._-]{0,63}$');
...
export const ActionInvocationStatus = z.enum(['AUTHORIZED', 'RUNNING', 'SUCCEEDED', 'FAILED', 'CANCELED', 'TIMED_OUT']);
...
type NullableSchemaType = SchemaTypeName | [Exclude<SchemaTypeName, 'null'>, 'null'] | ['null', Exclude<SchemaTypeName, 'null'>];
...
const ALLOWED_SCHEMA_KEYS = new Set([
  ...
  'multipleOf', 'minItems', 'maxItems', 'format', '$ref',
]);
...
function validatePortableSchemaNode(
  ...
  for (const key of keys) {
    if (!ALLOWED_SCHEMA_KEYS.has(key)) addIssue(ctx, [...path, key], `unsupported schema keyword: ${key}`);
  }

  if ('$ref' in value) {
    if (value.$ref !== 'lingfang://schemas/artifact-ref/v1' || keys.length !== 1) {
      addIssue(ctx, [...path, '$ref'], 'only lingfang://schemas/artifact-ref/v1 is supported as $ref');
    }
    return;
  }
  ...
  if (typeof type === 'string') {
    ...
    if (type.length !== 2 || !type.includes('null') || type[0] === type[1] || type.some((item) => typeof item !== 'string')) {
      addIssue(ctx, [...path, 'type'], 'nullable type must contain exactly one non-null type and null');
    } else {
  ...
  const rejectUnless = (name: string, valid: boolean): void => {
    if (has(name) && !valid) addIssue(ctx, [...path, name], `${name} is not valid for this schema type`);
  };

  if (hasType('object')) {
    if (value.additionalProperties !== false) addIssue(ctx, [...path, 'additionalProperties'], 'objects must set additionalProperties to false');
    if (has('properties')) {
      if (!isRecord(value.properties)) addIssue(ctx, [...path, 'properties'], 'properties must be an object');
      else {
        const properties = value.properties;
        const propertyNames = Object.keys(properties);
        if (propertyNames.length > ACTION_SCHEMA_MAX_PROPERTIES) addIssue(ctx, [...path, 'properties'], `objects may contain at most ${ACTION_SCHEMA_MAX_PROPERTIES} properties`);
        propertyNames.forEach((name) => validatePortableSchemaNode(properties[name], ctx, [...path, 'properties', name], state, depth + 1));
      }
    }
    if (has('required')) {
      if (!Array.isArray(value.required) || value.required.some((item) => typeof item !== 'string' || item.length === 0)) {
        ...
        value.required.forEach((name, index) => {
          if (seen.has(name)) addIssue(ctx, [...path, 'required', index], 'required must not contain duplicates');
          if (!properties.includes(name)) addIssue(ctx, [...path, 'required', index], 'required property must be declared');
          seen.add(name);
        });
      }
    }
  ...
  if (has('minLength')) { stringBound('minLength'); if (!hasType('string')) rejectUnless('minLength', false); }
  if (has('maxLength')) { stringBound('maxLength'); if (!hasType('string')) rejectUnless('maxLength', false); }
  if (has('minItems')) { stringBound('minItems'); if (!hasType('array')) rejectUnless('minItems', false); }
  if (has('maxItems')) { stringBound('maxItems'); if (!hasType('array')) rejectUnless('maxItems', false); }
  if (hasType('string') && typeof value.minLength === 'number' && typeof value.maxLength === 'number' && value.minLength > value.maxLength) addIssue(ctx, path, 'minLength must not exceed maxLength');
  if (hasType('array') && typeof value.minItems === 'number' && typeof value.maxItems === 'number' && value.minItems > value.maxItems) addIssue(ctx, path, 'minItems must not exceed maxItems');
  ...
  if (has('multipleOf')) {
    if (!validateBound(value.multipleOf, [...path, 'multipleOf'], ctx, 'multipleOf', false) || value.multipleOf <= 0) addIssue(ctx, [...path, 'multipleOf'], 'multipleOf must be greater than zero');
    if (!hasType('number') && !hasType('integer')) rejectUnless('multipleOf', false);
  }

  if (has('format')) {
    if (!hasType('string') || value.format !== 'date-time' && value.format !== 'uuid') addIssue(ctx, [...path, 'format'], 'only date-time and uuid formats are supported for strings');
  }
  for (const name of ['enum', 'const']) {
    if (has(name) && !isJsonValue(value[name])) addIssue(ctx, [...path, name], `${name} must contain portable JSON values`);
  }
  if (Array.isArray(value.enum)) {
    if (value.enum.length === 0 || value.enum.length > 128) addIssue(ctx, [...path, 'enum'], 'enum must contain between 1 and 128 values');
    const seen = new Set(value.enum.map((item) => canonicalJsonValue(item as JsonValue)));
    if (seen.size !== value.enum.length) addIssue(ctx, [...path, 'enum'], 'enum must not contain duplicate values');
  }
}

function canonicalJsonValue(value: JsonValue): string {
  if (Array.isArray(value)) return `[${value.map(canonicalJsonValue).join(',')}]`;
  if (isRecord(value)) return `{${Object.keys(value).sort().map((key) => `${JSON.stringify(key)}:${canonicalJsonValue(value[key] as JsonValue)}`).join(',')}}`;
...
function canonicalizeSchema(value: PortableJsonSchemaNode): PortableJsonSchemaNode {
  ...
  const type = Array.isArray(value.type) ? [...value.type].sort() as NullableSchemaType : value.type;
  const result: PortableJsonSchemaNode = {};
  if (type !== undefined) result.type = type;
  if (type === 'object' || Array.isArray(type) && type.includes('object')) {
    const properties = isRecord(value.properties) ? value.properties : {};
    result.properties = Object.fromEntries(Object.keys(properties).sort().map((key) => [key, canonicalizeSchema(properties[key])]));
    result.required = [...(value.required ?? [])].sort();
    result.additionalProperties = false;
  } else if (value.properties) {
    result.properties = Object.fromEntries(Object.keys(value.properties).sort().map((key) => [key, canonicalizeSchema(value.properties![key])]));
  }
  if (value.items) result.items = canonicalizeSchema(value.items);
  if (value.enum) result.enum = [...value.enum].sort((a, b) => canonicalJsonValue(a).localeCompare(canonicalJsonValue(b)));
  if (value.const !== undefined) result.const = value.const;
  for (const name of ['minLength', 'maxLength', 'minimum', 'maximum', 'exclusiveMinimum', 'exclusiveMaximum', 'multipleOf', 'minItems', 'maxItems', 'format'] as const) {
...
function parsePortableSchema(value: unknown, root: boolean): PortableJsonSchemaNode {
  ...
  if (root && !(parsed.type === 'object' || Array.isArray(parsed.type) && parsed.type.includes('object'))) throw new Error('action input/output schema root must be an object');
  return parsed;
}

const PortableJsonSchemaBase = z.unknown().superRefine((value, ctx) => {
...
export const PortableJsonSchema = PortableJsonSchemaBase as unknown as z.ZodType<PortableJsonSchemaNode>;

const ActionPortSchema = PortableJsonSchemaBase.superRefine((value, ctx) => {
  if (!(value as PortableJsonSchemaNode).type || ((value as PortableJsonSchemaNode).type !== 'object' && !Array.isArray((value as PortableJsonSchemaNode).type))) {
    addIssue(ctx, ['type'], 'action input/output schema root must be an object');
  }
  const type = (value as PortableJsonSchemaNode).type;
  if (Array.isArray(type) ? !type.includes('object') : type !== 'object') addIssue(ctx, ['type'], 'action input/output schema root must be an object');
});

const SafeEntry = z.string().trim().min(1).max(512).refine((value) => {
  if (/^[\\/]/.test(value) || /^[A-Za-z]:/.test(value) || value.includes('\\') || /(^|\/)\.\.?(\/|$)/.test(value) || /[\u0000-\u001f\u007f]/.test(value)) return false;
  return true;
}, 'handler entry must be a safe artifact-relative path');
const SafeExport = z.string().trim().min(1).max(128).regex(/^[A-Za-z_$][A-Za-z0-9_$]*(?:\.[A-Za-z_$][A-Za-z0-9_$]*)*$/);
...
export const PluginAction = z.object({
  action_id: ActionId,
  ...
  timeout_seconds: z.number().int().positive().max(24 * 60 * 60).default(900),
  ...
  if (action.previewable && !action.cloud_capable) {
    ctx.addIssue({ code: z.ZodIssueCode.custom, path: ['previewable'], message: 'previewable actions must be cloud_capable' });
  }
  if (action.previewable && action.execution_semantics === 'side_effect') {
    ctx.addIssue({ code: z.ZodIssueCode.custom, path: ['previewable'], message: 'side_effect actions cannot be previewable' });
  }
});
export type PluginAction = z.infer<typeof PluginAction>;

export const VersionRange = z.string().trim().min(1).max(128).refine((value) => {
...
type ParsedActionSemVer = { major: number; minor: number; patch: number; prerelease: Array<number | string> };
...
function actionRangeUpperBound(version: ParsedActionSemVer, operator: '^' | '~'): ParsedActionSemVer {
...
export function satisfiesActionVersionRange(versionText: string, range: string): boolean {
  ...
  const matches = range.trim().split(/\s+/).every((token) => {
    ...
    switch (match[1] ?? '=') {
      case '^': return comparison >= 0 && compareActionSemVer(version, actionRangeUpperBound(expected, '^')) < 0;
      case '~': return comparison >= 0 && compareActionSemVer(version, actionRangeUpperBound(expected, '~')) < 0;
  ...
  if (matches && version.prerelease.length > 0 && !expectedVersions.some((expected) => expected.prerelease.length > 0)) return false;
  return matches;
}

export const PluginActionDependency = z.object({
...
export const ActionTarget = z.object({
  ...
  action_id: ActionId,
  action_contract_version: StrictSemVer,
  action_surface_sha256: z.string().regex(SHA256_PATTERN, 'action_surface_sha256 must be lowercase hexadecimal'),
}).strict();
export type ActionTarget = z.infer<typeof ActionTarget>;

export const ActionCallChainEntry = z.object({
...
export const CreateActionInvocationRequest = z.object({
  target: ActionTarget,
...
export const ActionInvocationSummary = z.object({
  id: z.string().min(1), team_id: z.string().min(1), kind: ActionInvocationKind, status: ActionInvocationStatus,
  target: ActionTarget, policy_revision: z.number().int().nonnegative(), required_operations: z.array(z.string()),
  root_invocation_id: z.string().min(1), parent_invocation_id: z.string().min(1).nullable(), call_chain: z.array(ActionCallChainEntry),
  input: z.record(z.unknown()), output: z.record(z.unknown()).nullable(), deadline_at: z.string().datetime(),
  created_at: z.string().datetime(), started_at: z.string().datetime().nullable(), completed_at: z.string().datetime().nullable(),
  error_code: z.string(), error_message: z.string(),
}).strict();
export type ActionInvocationSummary = z.infer<typeof ActionInvocationSummary>;

export const ArtifactRefV1 = z.object({
  type: z.literal('artifact_ref'),
  artifact_id: z.string().trim().min(1).max(128),
  media_type: z.string().trim().min(1).max(256).refine((value) => !/[\u0000-\u001f\u007f]/.test(value), 'media_type must not contain control characters'),
...
export const PluginActionSurfaceV1 = z.object({
  ...
  execution: z.discriminatedUnion('runtime_type', [
    z.object({ runtime_type: z.enum(['client', 'nodejs']), entry: SafeEntry, export: SafeExport }).strict(),
    z.object({ runtime_type: z.literal('python'), entry: SafeEntry, callable: SafeExport }).strict(),
    z.object({ runtime_type: z.literal('cloud'), adapter: z.literal('cloud') }).strict(),
    z.object({ runtime_type: z.literal('workflow'), entry: SafeEntry, definition_sha256: z.string().regex(SHA256_PATTERN) }).strict(),
  ]),
}).strict();
...
export function projectPluginActionSurfaceV1(runtimeType: ActionRuntimeType, action: PluginAction, workflowIdentity?: { entry: string; definition_sha256: string }): PluginActionSurfaceV1 {
  let execution: PluginActionSurfaceV1['execution'];
  if (runtimeType === 'workflow') {
    if (action.handler || !workflowIdentity) throw new Error('workflow actions require a frozen workflow identity and no package handler');
    execution = { runtime_type: 'workflow', entry: workflowIdentity.entry, definition_sha256: workflowIdentity.definition_sha256 };
    ...
    if (!action.handler || !('callable' in action.handler)) throw new Error('python actions require a callable handler');
    execution = { runtime_type: 'python', entry: action.handler.entry, callable: action.handler.callable };
  } else {
    if (!action.handler || !('export' in action.handler)) throw new Error(`${runtimeType} actions require an export handler`);
    execution = { runtime_type: runtimeType, entry: action.handler.entry, export: action.handler.export };
  }
...
export const canonicalizePluginActionSurfaceV1 = (runtimeType: ActionRuntimeType, action: PluginAction, workflowIdentity?: { entry: string; definition_sha256: string }): PluginActionSurfaceV1 => projectPluginActionSurfaceV1(runtimeType, action, workflowIdentity);

export function canonicalPluginActionSurfaceJson(runtimeType: ActionRuntimeType, action: PluginAction, workflowIdentity?: { entry: string; definition_sha256: string }): string {
  return canonicalJsonValue(projectPluginActionSurfaceV1(runtimeType, action, workflowIdentity) as unknown as JsonValue);
}
...
export function canonicalInlineActionPayloadJson(value: JsonValue): string {
  ...
  if (utf8ByteLength(json) > ACTION_INLINE_PAYLOAD_MAX_BYTES) throw new Error(`inline action payload exceeds ${ACTION_INLINE_PAYLOAD_MAX_BYTES} bytes`);
  return json;
}
