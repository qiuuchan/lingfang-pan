import { z } from 'zod';
import { MarketplaceCategory, MarketplaceQualityTier, PublicPluginCard, type MarketplaceCategory as MarketplaceCategoryValue } from './web-plugin-center.ts';

export const MarketplaceMetricKind = z.enum([
  'INSTALL_SUCCEEDED', 'RUN_SUCCEEDED', 'RUN_FAILED', 'RATING_CHANGED',
  'PURCHASED', 'REFUNDED', 'SECURITY_BLOCKED', 'SECURITY_CLEARED',
]);
export type MarketplaceMetricKind = z.infer<typeof MarketplaceMetricKind>;

export const MarketplaceMetricSource = z.enum(['DESKTOP_HOST', 'CLOUD_RUNTIME', 'WORKFLOW_RUNTIME', 'REGISTRY', 'COMMERCE', 'SECURITY']);
...
export const MarketplaceRefundMetricState = z.enum(['AVAILABLE', 'NOT_APPLICABLE', 'INSUFFICIENT_SAMPLE', 'DATA_UNAVAILABLE']);
export type MarketplaceRefundMetricState = z.infer<typeof MarketplaceRefundMetricState>;

export const MarketplaceDiscoverySection = z.enum(['FEATURED', 'CATEGORY_POPULAR', 'RECENT_QUALITY']);
export type MarketplaceDiscoverySection = z.infer<typeof MarketplaceDiscoverySection>;

export const MARKETPLACE_CATEGORY_LABELS: Readonly<Record<MarketplaceCategoryValue, string>> = Object.freeze({
  AI: 'AI 与助手', PRODUCTIVITY: '效率与办公', DEV: '开发工具', DATA: '数据与可视化',
  MEDIA: '图像与多媒体', FILES: '文件与存储', NETWORK: '网络与接口', SYSTEM: '系统与监控', OTHER: '其他',
});

export const MarketplaceQualityPolicy = z.object({
  version: z.literal(1),
...
export const MarketplaceQualityReasonDetail = z.object({
...
export const MarketplaceQualityMetricSummary = z.object({
...
export const MarketplaceQualitySummary = z.object({
...
export const MarketplaceDiscoveryItem = PublicPluginCard.extend({
  ...
  if (item.quality_tier !== item.quality.tier) ctx.addIssue({ code: z.ZodIssueCode.custom, path: ['quality_tier'], message: 'quality_tier must match quality.tier' });
});
export type MarketplaceDiscoveryItem = z.infer<typeof MarketplaceDiscoveryItem>;

export const MarketplaceDiscoveryHome = z.object({
  policy: MarketplaceQualityPolicy,
...
export const MarketplaceDiscoveryPage = z.object({
...
const CATEGORY_KEYWORDS: ReadonlyArray<readonly [MarketplaceCategoryValue, readonly string[]]> = [
  ['AI', ['ai', 'gpt', 'llm', '助手', '对话', 'chat', '总结', '翻译', '摘要', 'assistant', '写作', '大模型']],
  ['PRODUCTIVITY', ['笔记', 'note', 'todo', '任务', '待办', '日历', '会议', '纪要', '效率', '清单', '看板', 'kanban']],
  ['DEV', ['代码', '开发', 'code', 'dev', 'git', '构建', '编译', '调试', 'debug', 'sdk', '命令行', 'cli', '终端']],
  ['DATA', ['可视化', '数据分析', 'chart', '图表', '统计', '报表', 'excel', '表格', 'csv', 'dashboard']],
  ['MEDIA', ['图片', '图像', '视频', '音频', '音乐', 'image', 'video', 'audio', '剪辑', '压缩', '转码', '水印']],
  ['FILES', ['文件', 'file', '资源管理', '目录', '搜索文件', '同步', '云盘', '备份', 'archive', 'zip', '解压']],
  ['NETWORK', ['网络', '请求', 'http', 'api', '爬虫', '抓取', '代理', 'proxy', '测速', 'dns', '下载器']],
...
export function inferMarketplaceCategory(input: {
  ...
  const text = [input.name, input.description ?? '', input.runtime_type ?? '', ...(input.capabilities ?? [])].join(' ').toLowerCase();
  return CATEGORY_KEYWORDS.find(([, keywords]) => keywords.some((keyword) => text.includes(keyword)))?.[0] ?? 'OTHER';
}
