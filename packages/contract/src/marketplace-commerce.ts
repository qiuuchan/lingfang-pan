...
export const MarketplacePriceVersion = z.string().trim().min(1).max(128).regex(/^pv1\.[A-Za-z0-9_-]{43}$/);
...
export const MarketplaceOrderStatus = z.enum(['PENDING_SETTLEMENT', 'REFUND_REQUESTED', 'SETTLED', 'REFUNDED']);
...
export const MarketplaceCommerceWriterMode = z.enum(['LEGACY', 'DRAINING', 'SETTLEMENT_V2', 'PAUSED']);
export type MarketplaceCommerceWriterMode = z.infer<typeof MarketplaceCommerceWriterMode>;
export const MarketplacePlatformAccountKind = z.enum(['MARKETPLACE_CLEARING', 'MARKETPLACE_REVENUE']);
...
export const MarketplacePriceSplit = z.object({
...
export const MarketplaceDiscountSummary = z.object({
  id: z.string().uuid(),
...
export const MarketplacePriceProjection = z.object({
  ...
  if (value.list_price_cents - value.discount_amount_cents !== value.effective_price_cents) {
    ctx.addIssue({ code: z.ZodIssueCode.custom, path: ['discount_amount_cents'], message: 'list price minus discount must equal effective price' });
  }
  if (value.price_cents !== value.effective_price_cents) {
    ctx.addIssue({ code: z.ZodIssueCode.custom, path: ['price_cents'], message: 'legacy price_cents must project the effective price' });
  }
});
export type MarketplacePriceProjection = z.infer<typeof MarketplacePriceProjection>;

export const MarketplaceOrderSnapshot = z.object({
  id: z.string().uuid(),
  ...
  if (value.list_price_cents - value.discount_amount_cents !== value.price_cents) {
    ctx.addIssue({ code: z.ZodIssueCode.custom, path: ['price_cents'], message: 'order price snapshot is inconsistent' });
  }
  if (value.platform_amount_cents + value.seller_amount_cents !== value.price_cents) {
    ctx.addIssue({ code: z.ZodIssueCode.custom, path: ['platform_amount_cents'], message: 'order split is inconsistent' });
  }
});
export type MarketplaceOrderSnapshot = z.infer<typeof MarketplaceOrderSnapshot>;

export const MarketplacePurchaseRequest = z.object({
...
export const MarketplacePurchaseResponse = z.object({
  entitled: z.boolean(),
...
export const MarketplaceRefundRequest = z.object({
  id: z.string().uuid(),
...
export const MarketplaceOrderListItem = z.object({
  id: z.string().uuid(),
...
export const MarketplaceOrderPage = z.object({
...
const MarketplaceStatementStatusTotals = z.object({ count: z.number().int().nonnegative(), gross_cents: CnyCents, seller_cents: CnyCents }).strict();
export const MarketplaceStatementSummary = z.object({
...
export const MarketplaceStatementPage = MarketplaceOrderPage.extend({
  range: z.object({ from: z.string().datetime(), to: z.string().datetime(), timezone: z.string().min(1).max(100) }).strict(),
  summary: MarketplaceStatementSummary,
}).strict();
export type MarketplaceStatementPage = z.infer<typeof MarketplaceStatementPage>;

const MarketplaceDailyTotals = z.object({ count: z.number().int().nonnegative(), gross_cents: CnyCents, seller_cents: CnyCents }).strict();
export const MarketplaceStatementDaily = z.object({
  range: z.object({ from: z.string().datetime(), to: z.string().datetime(), timezone: z.string().min(1).max(100) }).strict(),
...
export const CreateMarketplaceDiscountRequest = z.object({
  price_cents: CnyCents.min(1),
  starts_at: z.string().datetime(),
  ends_at: z.string().datetime(),
}).strict();
export const MarketplaceCampaign = z.object({
  id: z.string().uuid(),
  ...
  items: z.array(z.object({ package_id: z.string().uuid(), rank: z.number().int().nonnegative() }).strict()).max(100),
}).strict();

export const MarketplaceCampaignAttributionToken = z.object({
  campaign_token: z.string().trim().min(1).max(4096),
  expires_at: z.string().datetime(),
}).strict();
export type MarketplaceCampaignAttributionToken = z.infer<typeof MarketplaceCampaignAttributionToken>;

export const MarketplaceJournalEntry = z.object({
...
export function splitMarketplacePrice(grossCents: number, platformFeeBps = MARKETPLACE_DEFAULT_PLATFORM_FEE_BPS): MarketplacePriceSplit {
...
export function marketplaceJournalNet(entries: readonly MarketplaceJournalEntry[]): number {
  return entries.reduce((sum, entry) => sum + (entry.direction === 'CREDIT' ? entry.amount_cents : -entry.amount_cents), 0);
}
