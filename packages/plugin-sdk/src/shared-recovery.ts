import type { SharedChangeEvent, SharedChangesPage, SharedPage, SharedValue } from '@lingfang/contract';
...
export function inspectSharedCursor(currentCursor: string, incomingCursor: string): SharedCursorDisposition {
...
export function applySharedChange(
  ...
  if (disposition === 'GAP') {
    throw new SharedRecoveryError('shared_change_cursor_gap', `共享变更 cursor 存在缺口：${replica.cursor} -> ${event.cursor}`);
  }
  let values = replica.values;
  if (event.event_kind === 'UPSERT') {
    if (!fetchedValue || fetchedValue.key !== event.key) {
      throw new SharedRecoveryError('shared_change_value_missing', 'UPSERT 变更必须重新读取对应共享值');
    }
    if (compareSharedDecimal(fetchedValue.revision, event.revision) < 0) {
      throw new SharedRecoveryError('shared_change_value_stale', '重新读取的共享值早于变更 revision');
    }
...
export async function recoverSharedReplicaAfterCursorExpiry(input: {
  ...
  if (!Number.isInteger(maxPages) || maxPages < 1) throw new SharedRecoveryError('shared_relist_limit_invalid', 'max_pages 必须是正整数');
  ...
  while (page.next_page_cursor !== null) {
    if (pages >= maxPages) throw new SharedRecoveryError('shared_relist_page_limit', '共享数据全量恢复页数超过限制');
  ...
  for (let changePages = 0; changePages < maxPages; changePages += 1) {
    ...
    if (compareSharedDecimal(pageOfChanges.next_cursor, after) < 0) {
      throw new SharedRecoveryError('shared_change_cursor_regressed', 'changes API 返回了倒退的 cursor');
    }
...
function mergeRelistPage(
  ...
  if (page.snapshot_cursor !== snapshotCursor || page.relist_token !== relistToken) {
    throw new SharedRecoveryError('shared_relist_token_mismatch', '共享数据分页未保持同一 snapshot/relist token');
  }
  return page.values.reduce(mergeSharedValue, replica);
}

function assertValueScope(replica: SharedReplica, value: SharedValue): void {
  if (value.namespace_id !== replica.namespace_id || value.namespace_generation !== replica.namespace_generation) {
    throw new SharedRecoveryError('shared_namespace_scope_mismatch', '共享值不属于当前 namespace generation');
  }
}

function assertChangeScope(replica: SharedReplica, event: SharedChangeEvent): void {
  if (event.namespace_id !== replica.namespace_id || event.namespace_generation !== replica.namespace_generation) {
    throw new SharedRecoveryError('shared_namespace_scope_mismatch', '共享变更不属于当前 namespace generation');
  }
}

function normalizeDecimal(value: string): string {
  if (!/^[0-9]+$/.test(value)) throw new SharedRecoveryError('shared_decimal_invalid', '共享 revision/cursor 必须是十进制字符串');
