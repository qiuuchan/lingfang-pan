...
const RESERVED_TYPES = new Set([
  ...
  'runtime_artifact_grant',
  'runtime_artifact_hold',
]);

export function buildWorkflowPlan(input: unknown): {
  plan?: WorkflowPlan;
  ...
  definition.nodes.forEach((node) =>
    node.depends_on.forEach((dependency) => graph.setEdge(dependency, node.node_id))
  );
  if (!alg.isAcyclic(graph))
    diagnostics.push({
      code: 'workflow_cycle_detected',
      path: 'nodes',
      message: `workflow contains a cycle: ${alg
        .findCycles(graph)
  ...
  definition.nodes.forEach((node, index) =>
    node.input_bindings.forEach((binding, bindingIndex) => {
      if (
        ...
        !node.depends_on.includes(binding.source.node_id)
      )
        diagnostics.push({
          code: 'workflow_mapping_invalid',
          path: `nodes.${index}.input_bindings.${bindingIndex}`,
          message: 'node output source must be an explicit dependency',
        });
      if (binding.source.kind === 'literal' && containsReservedIdentity(binding.source.value))
        diagnostics.push({
          code: 'workflow_mapping_invalid',
          path: `nodes.${index}.input_bindings.${bindingIndex}.source.value`,
          message: 'literal contains a reserved runtime identity',
        });
    })
  );
  definition.output_bindings.forEach((binding, index) => {
    if (binding.source.kind === 'literal' && containsReservedIdentity(binding.source.value))
      diagnostics.push({
        code: 'workflow_mapping_invalid',
        path: `output_bindings.${index}.source.value`,
        message: 'literal contains a reserved runtime identity',
      });
  });
  findPointerConflicts(definition).forEach((path) =>
    diagnostics.push({
      code: 'workflow_mapping_invalid',
      path,
      message: 'target JSON pointers overlap',
    })
  );
  if (diagnostics.length) return { diagnostics };
  const order = alg.topsort(graph);
  const readySets = topologicalReadySets(graph);
  if (readySets.some((set) => set.length > WORKFLOW_MAX_PARALLEL))
    return {
      diagnostics: [
        {
          code: 'workflow_limit_exceeded',
          path: 'nodes',
          message: `workflow requires more than ${WORKFLOW_MAX_PARALLEL} parallel nodes`,
        },
      ],
    };
  return {
    plan: {
      definition,
      order,
      ready_sets: readySets,
      max_parallelism: Math.max(...readySets.map((set) => set.length)),
    },
    diagnostics: [],
  };
}
...
export async function buildWorkflowClosure(
  ...
  resolveWorkflow: (
    releaseId: string
  ) => WorkflowClosureRelease | null | Promise<WorkflowClosureRelease | null>
  ...
  const visit = async (
    workflow: WorkflowClosureRelease,
    path: string[],
    nestedDepth: number
  ): Promise<void> => {
  ...
  return {
    subplans: [...subplans.values()].sort((left, right) =>
      left.workflow_release_id.localeCompare(right.workflow_release_id)
    ),
  ...
  return definition.nodes
    .filter(
      (node) =>
        ...
        node.depends_on.every((id) => states[id] === 'SUCCEEDED')
    )
    .map((node) => node.node_id)
    .sort();
}
export type WorkflowReducerState = {
  status: 'RUNNING' | 'FAILING' | 'CANCELING' | 'SUCCEEDED' | 'FAILED' | 'CANCELED';
  nodes: Record<
    string,
    {
      ...
      attempt: number;
      retry_limit: number;
      execution_semantics: 'read_only' | 'idempotent' | 'side_effect';
    }
  >;
};
export type WorkflowReducerEvent =
  | { type: 'step_succeeded'; node_id: string }
  | { type: 'step_failed'; node_id: string; retryable: boolean }
  | { type: 'cancel_requested' }
  | { type: 'closing_settled' };
export type WorkflowReducerEffect =
  | { type: 'retry'; node_id: string; attempt: number }
  | { type: 'cancel_running'; node_ids: string[] }
  | { type: 'skip_pending'; node_ids: string[] };
export function reduceWorkflowRun(
  state: WorkflowReducerState,
  event: WorkflowReducerEvent
): { state: WorkflowReducerState; effects: WorkflowReducerEffect[] } {
  const next = structuredClone(state);
  const effects: WorkflowReducerEffect[] = [];
  ...
  if (event.type === 'step_succeeded') {
    ...
    if (node?.status === 'RUNNING') node.status = 'SUCCEEDED';
    if (
      next.status === 'RUNNING' &&
      Object.values(next.nodes).every((item) => item.status === 'SUCCEEDED')
    )
      next.status = 'SUCCEEDED';
  }
  if (event.type === 'step_failed') {
    const node = next.nodes[event.node_id];
    if (node?.status !== 'RUNNING') return { state: next, effects };
    node.status = 'FAILED';
    const canRetry =
      event.retryable &&
      node.execution_semantics !== 'side_effect' &&
      node.attempt < node.retry_limit &&
      next.status === 'RUNNING';
    if (canRetry) {
      node.attempt += 1;
      node.status = 'READY';
      effects.push({ type: 'retry', node_id: event.node_id, attempt: node.attempt });
    } else {
      next.status = 'FAILING';
      const pending = Object.entries(next.nodes)
        .filter(([, item]) => item.status === 'PENDING' || item.status === 'READY')
        .map(([id]) => id);
      const running = Object.entries(next.nodes)
        .filter(([, item]) => item.status === 'RUNNING')
        .map(([id]) => id);
      pending.forEach((id) => {
        next.nodes[id].status = 'SKIPPED';
      });
      effects.push(
        { type: 'skip_pending', node_ids: pending },
        { type: 'cancel_running', node_ids: running }
      );
    }
  }
  if (event.type === 'cancel_requested' && next.status === 'RUNNING') {
  ...
  if (
    event.type === 'closing_settled' &&
    Object.values(next.nodes).every(
      (item) => !['PENDING', 'READY', 'RUNNING'].includes(item.status)
    )
  ) {
    if (next.status === 'FAILING') next.status = 'FAILED';
    if (next.status === 'CANCELING') next.status = 'CANCELED';
  }
  ...
  for (const binding of bindings) {
    const value =
      binding.source.kind === 'literal'
        ? structuredClone(binding.source.value)
        : binding.source.kind === 'workflow_input'
          ? pointerGet(workflowInput, binding.source.source_pointer)
          : pointerGet(nodeOutputs[binding.source.node_id], binding.source.source_pointer);
    result = pointerSet(result, binding.target_pointer, value);
  }
  return result;
}

function topologicalReadySets(graph: Graph): string[][] {
  ...
  while (pending.size) {
    const ready = [...pending]
      .filter((node) =>
        (graph.predecessors(node) ?? []).every((dependency) => done.has(dependency))
      )
      .sort();
...
function containsReservedIdentity(value: unknown): boolean {
  ...
  if (typeof record.type === 'string' && RESERVED_TYPES.has(record.type)) return true;
  return Object.values(record).some(containsReservedIdentity);
}
function findPointerConflicts(definition: Definition): string[] {
  const conflicts: string[] = [];
  const check = (bindings: WorkflowBinding[], prefix: string) => {
    const pointers = bindings.map((binding) => binding.target_pointer);
    pointers.forEach((pointer, index) =>
      pointers.slice(index + 1).forEach((other) => {
        if (pointer === other || pointer.startsWith(`${other}/`) || other.startsWith(`${pointer}/`))
          conflicts.push(`${prefix}.${index}.target_pointer`);
      })
    );
  };
  definition.nodes.forEach((node, index) =>
    check(node.input_bindings, `nodes.${index}.input_bindings`)
  );
  check(definition.output_bindings, 'output_bindings');
  return conflicts;
}
...
function pointerSet(root: unknown, pointer: string, value: unknown): unknown {
  ...
  const result = root && typeof root === 'object' ? (root as Record<string, unknown>) : {};
  let current = result;
