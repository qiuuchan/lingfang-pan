...
async function main() {
  ...
  console.log('[auth/me]', r.status, r.status === 200 ? 'OK' : 'FAIL');
  ...
  console.log('[admin/teams]', r.status);
  if (r.status !== 200) ok = false;
  ...
  console.log(
    '[admin/dashboard]',
    r.status,
    r.status === 200 ? (await r.text()).slice(0, 80) : 'FAIL'
  );
  if (r.status !== 200) ok = false;

  // 5) 公开端点
  r = await fetch(API + '/api/platform-info');
  console.log('[platform-info]', r.status);
  if (r.status !== 200) ok = false;
