...
function listPluginFiles(pluginDir) {
  ...
  const hasVendorArchive =
    existsSync(join(pluginDir, 'vendor.tar.gz')) ||
...
async function exportPlugin(pluginId, outDir) {
  ...
  const paths = listPluginFiles(pluginDir).filter(
    (p) => p !== 'manifest.json' && p !== '_meta.json'
  );
  ...
  const entries = [];
  entries.push(
    buildZipEntry('_meta.json', Buffer.from('{"format":"lingfang-plugin","formatVersion":4}'), true)
  );
  entries.push(
    buildZipEntry('manifest.json', Buffer.from(JSON.stringify(manifest, null, 2)), true)
  );
  ...
  console.log(
    `✅ ${pluginId}.lfplugin v4  (${sizeKB} KB, ${entries.length} 条目) → ${relative(REPO_ROOT, outFile)}`
  );
  return {
    id: pluginId,
    name: manifest.name,
