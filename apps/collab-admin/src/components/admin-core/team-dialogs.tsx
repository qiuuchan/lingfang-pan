...
export function CreateTeamDialog({ children, onChanged }: { children: ReactNode; onChanged: () => void }) {
      ...
      <DialogContent className="sm:max-w-md">
        ...
        <div className="space-y-4">
          <div className="space-y-2">
            ...
            <Input id="create-team-name" value={name} onChange={(event) => setName(event.target.value)} />
          </div>
          <div className="space-y-2">
            <Label htmlFor="create-team-balance">初始余额（元）</Label>
            <Input id="create-team-balance" inputMode="decimal" value={initialBalance} onChange={(event) => setInitialBalance(event.target.value)} />
          </div>
        </div>
        <DialogFooter>
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="sm:max-w-md">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <div className="space-y-2">
            <Label>方向</Label>
            <Select value={direction} onValueChange={(value) => setDirection(value as LedgerDirection)}>
          ...
          <div className="space-y-2">
            ...
            <Input id={`team-balance-${team.id}`} inputMode="decimal" value={amount} onChange={(event) => setAmount(event.target.value)} />
          </div>
        </div>
        <div className="space-y-2">
          <Label htmlFor={`team-balance-reason-${team.id}`}>原因</Label>
          <Input id={`team-balance-reason-${team.id}`} value={reason} onChange={(event) => setReason(event.target.value)} />
        </div>
        <DialogFooter>
          <Button type="button" variant="outline" disabled={busy} onClick={() => setOpen(false)}>取消</Button>
          <Button type="button" disabled={busy} onClick={() => void submit()}>{busy ? '提交中…' : '提交'}</Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  );
...
export function AssignTeamAdminDialog({
  ...
  const options = useAsyncResource(
    (signal) => adminCoreApi.userOptions(query, signal),
    [query],
    { enabled: open, isEmpty: (data) => data.items.length === 0 },
  );
  ...
  return (
    <Dialog open={open} onOpenChange={handleOpenChange}>
      ...
      <DialogContent className="sm:max-w-md">
            ...
            <Select value={userId} onValueChange={setUserId}>
              ...
              <SelectContent>
                {options.data.items.map((user) => (
                  <SelectItem key={user.id} value={user.id}>{user.displayName || user.email} · {user.email}</SelectItem>
                ))}
              </SelectContent>
            </Select>
          ) : null}
        ...
        <DialogFooter>
          <Button type="button" variant="outline" onClick={() => setOpen(false)}>取消</Button>
          <Button type="button" onClick={() => void assign()}>指定</Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  );
