...
export function Dashboard({ onNavigate }: { onNavigate?: (view: View, intent?: Omit<GovernanceIntent, 'nonce'>) => void } = {}) {
  ...
  const stats = [
    ...
    { label: '活跃插件包', value: data?.activePluginPackages ?? 0, desc: '可治理插件包', icon: PlugIcon, color: 'text-violet-500' },
  ...
  const quickActions: Array<{ label: string; icon: typeof UsersIcon; view: View; intent?: Omit<GovernanceIntent, 'nonce'> }> = [
    { label: '创建用户', icon: UsersIcon, view: 'users' },
    { label: '创建团队', icon: BoxesIcon, view: 'teams' },
    { label: '治理中心', icon: CheckCircleIcon, view: 'governance', intent: { tab: 'applications', applicationStatus: 'PENDING' } },
  ];
