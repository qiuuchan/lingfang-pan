...
function statusBadge(s: string) {
  ...
  if (s === 'upstream_error' || s === 'no_channel')
...
export function CallLogsView() {
  ...
  useEffect(() => {
    let mounted = true;
    setLoading(true);
    api<{ items: LlmCallLog[]; total: number }>(`/api/admin/billing/call-logs?${qs}`)
      .then((r) => {
        if (mounted) {
          setLogs(r.items);
  ...
  return (
    <Section title="调用日志" description="AI 调用全链路记录，支持多维度查询。">
      <div className="mb-3 flex flex-wrap items-center gap-2">
        <Input
          className="sm:w-48"
          placeholder="团队 ID（精确）"
          value={teamId === 'ALL' ? '' : teamId}
          onChange={(e) => setTeamId(e.target.value.trim() || 'ALL')}
        />
        ...
        <Select value={status} onValueChange={setStatus}>
          ...
          <SelectContent>
            ...
            <SelectItem value="upstream_error">失败</SelectItem>
          ...
          {logs.length ? (
            logs.map((l) => (
              <TableRow key={l.id}>
                ...
                <TableCell>
                  <div className="text-sm">{l.team?.name ?? l.teamId.slice(0, 8)}</div>
                ...
                <TableCell>
                  <Badge variant="secondary">{clientSourceLabel(l.clientSource)}</Badge>
                </TableCell>
                <TableCell className="font-mono text-xs">{l.model}</TableCell>
                <TableCell>
                  {l.poolName ? (
                    <Badge variant="secondary">{l.poolName}</Badge>
                  ) : (
...
function CallLogDetail({ summary }: { summary: LlmCallLog }) {
  ...
  useEffect(() => {
    ...
    api<{ log: LlmCallLog }>(`/api/admin/billing/call-logs/${summary.id}`, {
      signal: controller.signal,
    })
      .then((result) => setDetail(result.log))
      .catch(() => undefined);
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="sm:max-w-md">
        ...
        {!detail ? (
          <div className="py-8 text-center text-sm text-muted-foreground">加载详情…</div>
        ) : (
          <>
            <InfoGrid
              items={[
                ['模型', log.model],
                ['能力', log.capability],
                ['客户端来源（自报）', clientSourceLabel(log.clientSource)],
                ['版本', log.tier ?? '—'],
