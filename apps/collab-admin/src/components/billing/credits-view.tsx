...
export function CreditsView() {
  ...
  const load = useCallback(async () => {
    const result = await api<{ items: TeamRow[]; total: number }>(
      `/api/admin/billing/credits/teams?page=${page}&pageSize=${pageSize}`
    );
  ...
  return (
    <Section
      title="灵石账户"
      description="团队灵石（AI 用量计费货币，独立于人民币余额）总览与调整。"
    >
      <Table>
        <TableHeader>
          <TableRow>
            <TableHead>团队</TableHead>
            <TableHead>余额（灵石）</TableHead>
...
function AdjustDialog({
  ...
  async function submit() {
    ...
    if (
      !(await run(
        () =>
          api(`/api/admin/billing/credits/teams/${team.id}/adjustments`, {
            method: 'POST',
            body,
          }).then(onSaved),
        '灵石已调整'
        ...
        <div className="space-y-3">
          ...
          <div className="space-y-2">
            ...
            <Select value={direction} onValueChange={(v) => setDirection(v as 'CREDIT' | 'DEBIT')}>
              ...
              <SelectContent>
                <SelectItem value="CREDIT">加款</SelectItem>
...
function LedgerDialog({ team, children }: { team: TeamRow; children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    let mounted = true;
    api<{ items: LedgerRow[] }>(
      `/api/admin/billing/credits/teams/${team.id}/ledger?page=1&pageSize=50`
    )
      .then((r) => mounted && setRows(r.items))
      .catch(() => undefined);
    return () => {
      mounted = false;
    };
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="sm:max-w-lg">
        <DialogHeader>
          <DialogTitle>灵石流水 · {team.name}</DialogTitle>
        </DialogHeader>
