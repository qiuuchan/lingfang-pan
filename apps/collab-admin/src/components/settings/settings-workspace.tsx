...
export function SettingsView() {
  const [activeTab, setActiveTab] = useState<
    'basic' | 'email' | 'security' | 'release' | 'search' | 'video'
  >('basic');
  ...
  useEffect(() => {
    ...
    if (activeTab === 'video')
      api<RbflowSettings>('/api/admin/settings/rbflow')
        .then((data) => {
          if (cancelled) return;
  ...
  async function saveGeetestSettings() {
    ...
    try {
      const scenes = GEETEST_SCENE_OPTIONS.filter((opt) => geetestScenesDraft.has(opt.value))
        .map((opt) => opt.value)
        .join(',');
      const entries: Array<{ key: string; value: string }> = [
      ...
      if (geetestCaptchaKeyDraft.length > 0)
        entries.push({ key: 'geetestCaptchaKey', value: geetestCaptchaKeyDraft });
      await api('/api/admin/settings', { method: 'PATCH', body: { settings: entries } });
      // 保存成功后同步本地快照 + 清空 key 草稿（后端已存新 key，下次加载 hasCaptchaKey=true）。
      setGeetest({
        ...geetestDraft,
        geetestScenes: scenes,
        hasCaptchaKey: geetestCaptchaKeyDraft.length > 0 ? true : geetestDraft.hasCaptchaKey,
      });
      setGeetestDraft((prev) => ({
        ...prev,
        geetestScenes: scenes,
        hasCaptchaKey: geetestCaptchaKeyDraft.length > 0 ? true : prev.hasCaptchaKey,
      }));
  ...
  async function testGeetest() {
    if (
      geetestDraft.geetestCaptchaId.trim() !== geetest.geetestCaptchaId ||
      geetestCaptchaKeyDraft.length > 0
    ) {
  ...
  async function saveGiteeSettings() {
    ...
    try {
      ...
      if (giteeAccessTokenDraft.length > 0)
        entries.push({ key: 'giteeAccessToken', value: giteeAccessTokenDraft });
      await api('/api/admin/settings', { method: 'PATCH', body: { settings: entries } });
  ...
  async function saveRbflowSettings() {
    ...
    try {
      ...
      if (rbflowApiKeyDraft.length > 0)
        entries.push({ key: 'rbflowApiKey', value: rbflowApiKeyDraft });
      await api('/api/admin/settings', { method: 'PATCH', body: { settings: entries } });
      setRbflow({
        ...rbflowDraft,
  ...
  async function testRbflow() {
    if (rbflowDraft.rbflowUrl.trim() !== rbflow.rbflowUrl || rbflowApiKeyDraft.length > 0) {
      return toast.error('配置已修改，请先保存再测试');
    }
    ...
    <div className="space-y-6">
      <Tabs value={activeTab} onValueChange={(value) => setActiveTab(value as typeof activeTab)}>
        <TabsList className="max-w-full overflow-x-auto">
          ...
          <TabsTrigger value="release">发布源</TabsTrigger>
      ...
      <motion.div variants={containerVariant} initial="hidden" animate="show" className="space-y-8">
          ...
          >
            <div className="mb-4 flex flex-wrap items-center gap-2 text-xs text-muted-foreground">
              ...
              <span>
                ...
                独立用户名 / 密码未填时，凭据可内嵌在 SMTP 连接 URL（如
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <div className="space-y-2 sm:col-span-2">
                ...
                />
                <p className="text-xs text-muted-foreground">
                  填服务器地址[:端口]，自动识别 SSL。常见服务商： 阿里云{' '}
                  <code className="rounded bg-muted px-1">smtpdm.aliyun.com:465</code>、 QQ{' '}
                  <code className="rounded bg-muted px-1">smtp.qq.com:465</code>、 163{' '}
                  <code className="rounded bg-muted px-1">smtp.163.com:465</code>、 Gmail{' '}
                  <code className="rounded bg-muted px-1">smtp.gmail.com:587</code>。 也可填完整格式{' '}
                  <code className="rounded bg-muted px-1">smtps://host:port</code>。
                </p>
              </div>
              <div className="space-y-2">
            ...
            <div className="mt-4 rounded-xl border bg-muted/20 p-4">
              ...
              <p className="mt-2 text-xs text-muted-foreground">
                点击后后端会通过当前 SMTP
                配置向该邮箱发一封测试邮件。若失败，错误信息会显示在右上角通知（便于排查 SMTP 连接 /
                认证问题）。
              </p>
            </div>
        ...
        <motion.div
          ...
          className={activeTab === 'security' ? undefined : 'hidden'}
        >
          <Section
            title="验证码服务（极验）"
          ...
          >
            <div className="mb-4 flex flex-wrap items-center gap-2 text-xs text-muted-foreground">
              ...
              <span>
                captchaId 公开（前端 platform-info 据此初始化极验组件），captchaKey
                仅后端校验用（绝不公开）。
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <div className="space-y-2">
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setGeetestDraft({ ...geetestDraft, geetestCaptchaId: e.target.value })
                  }
            ...
            <div className="mt-4 rounded-xl border bg-muted/20 p-4">
              ...
              <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
                <p className="flex-1 text-xs text-muted-foreground">
                  点击后后端会读已保存的 captchaId / captchaKey 向极验接口发探测请求，校验连通性与
                  key 有效性。 若失败，错误信息会显示在右上角通知（便于排查 captchaId / key
                  是否正确）。
                </p>
                <Button
                  onClick={testGeetest}
                  disabled={geetestTesting || geetestLoading || geetestSaving}
                  className="sm:mb-[1px]"
                >
        ...
        <motion.div
          variants={cardVariant}
          className={activeTab === 'release' ? undefined : 'hidden'}
        >
          <Section
            title="更新日志源（Gitee）"
          ...
          >
            <div className="mb-4 flex flex-wrap items-center gap-2 text-xs text-muted-foreground">
              ...
              <span>
                令牌在 Gitee「设置 → 私人令牌」生成，须勾选 project 权限。owner / repo
                留空则用默认（yijianruyuan / lingfang）。 更新日志源来自 Gitee release
                notes，下载页版本号来自本地已签名产物，两者可能不一致（属正常）。
              </span>
            </div>
            ...
            <div className="mt-4 rounded-xl border bg-muted/20 p-4">
              ...
              <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
                <p className="flex-1 text-xs text-muted-foreground">
                  点击后后端会读已保存的 owner / repo / token 向 Gitee releases
                  端点发探测请求，校验连通性与令牌有效性。
                  若失败，错误信息会显示在右上角通知（便于排查 token 失效 / 缺权限 / owner-repo
                  错误）。
                </p>
                <Button
                  onClick={testGitee}
                  disabled={giteeTesting || giteeLoading || giteeSaving}
                  className="sm:mb-[1px]"
                >
        ...
        <motion.div variants={cardVariant} className={activeTab === 'video' ? undefined : 'hidden'}>
          ...
          >
            <div className="mb-4 flex flex-wrap items-center gap-2 text-xs text-muted-foreground">
              ...
              <span>
                RBFLow 是封装 RunningHub ComfyUI 工作流的 FastAPI 服务。API-KEY 对应 RBFLow 实例
                .env 的<code className="mx-1 rounded bg-muted px-1">API_KEY</code>
                （服务端转发用，绝不公开）。地址留空则视频插件报「未配置」。
              </span>
            </div>
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <div className="space-y-2 sm:col-span-2">
                ...
                />
                <p className="text-xs text-muted-foreground">
                  平台运营的 RBFLow 实例 Base URL（含端口，不带尾部斜杠）。
                </p>
              </div>
              <div className="space-y-2 sm:col-span-2">
                ...
                {rbflowDraft.hasApiKey && (
                  <RevealSecretButton
                    secretKey="rbflowApiKey"
                    label="查看 API-KEY 明文"
                    hasConfigured={rbflowDraft.hasApiKey}
                  />
                )}
              </div>
            </div>
            ...
            <div className="mt-4 rounded-xl border bg-muted/20 p-4">
              ...
              <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
                ...
                <Button
                  onClick={testRbflow}
                  disabled={rbflowTesting || rbflowLoading || rbflowSaving}
                  className="sm:mb-[1px]"
                >
        ...
        <motion.div
          variants={cardVariant}
          className={activeTab === 'search' ? undefined : 'hidden'}
        >
          <Section
            title="搜索源（AI 联网搜索）"
          ...
          >
            <div className="grid gap-4 sm:grid-cols-2">
              <div className="space-y-2 sm:col-span-2">
                ...
                />
                <p className="text-xs text-muted-foreground">
                  自部署的 SearXNG 实例地址（留空用公共源）。优先级高于 Tavily/Brave。
                </p>
              </div>
              <div className="space-y-2">
                ...
                <Input
                  ...
                  placeholder={
                    searchDraft.hasTavilyApiKey
                      ? '已配置，留空保持不变'
                      : 'tvly-xxxxxx（tavily.com 注册获取）'
                  }
                  disabled={searchLoading}
                  autoComplete="new-password"
                />
                <p className="text-xs text-muted-foreground">
                  {searchDraft.hasTavilyApiKey
                    ? '当前已配置，留空提交保持不变。'
                    : 'tavily.com 注册，免费额度 1000 次/月。专为 AI 搜索优化。'}
                </p>
              </div>
              <div className="space-y-2">
                ...
                <Input
                  ...
                  placeholder={
                    searchDraft.hasBraveApiKey
                      ? '已配置，留空保持不变'
                      : 'BSAxxxxxx（brave.com/search/api 获取）'
                  }
                  disabled={searchLoading}
                  autoComplete="new-password"
                />
                <p className="text-xs text-muted-foreground">
                  {searchDraft.hasBraveApiKey
                    ? '当前已配置，留空提交保持不变。'
                    : 'brave.com/search/api 注册，免费额度 2000 次/月。'}
                </p>
              </div>
            </div>
        ...
        {/* 版本信息 */}
        <motion.div variants={cardVariant} className={activeTab === 'basic' ? undefined : 'hidden'}>
          <Section title="版本信息" description="当前管理端构建版本与技术栈。">
            <InfoGrid
              items={[
                [
                  '版本号',
