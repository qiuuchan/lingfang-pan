...
// 必须与后端 apps/collab-api/src/security.ts 的同名常量保持一致（跨包，故为重复字面量）。
// readCsrfToken() 用它构造正则而非再写一遍字面量：曾经两处各写一份，改 cookie 名时
// 只改一处就会让 x-csrf-token 静默读空 —— 写请求全部被后端 403，且没有任何编译期提示。
