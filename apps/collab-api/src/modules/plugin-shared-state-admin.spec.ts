...
function service(prisma: Record<string, unknown>, auth = { ensureTeamAdmin: vi.fn(async () => ({ teamId: 'team-1' })) }) {
...
describe('PluginSharedStateService team-admin operations', () => {
  it('returns metadata only and scopes the list to the authenticated team', async () => {
    ...
    const result = await service({ pluginSharedNamespace: { findMany } }).adminListNamespaces('admin-1');
    expect(findMany).toHaveBeenCalledWith(expect.objectContaining({ where: { teamId: 'team-1' } }));
    expect(result.namespaces).toEqual([expect.objectContaining({
      namespace_id: 'ns-1', generation: 2, active_schema_version: 1, used_bytes: 128,
      next_value_revision: '7', next_change_cursor: '9', deleted_at: null,
    })]);
  ...
  it('does not create an invocation principal or accept a runtime token for admin export', async () => {
    ...
    for await (const _line of exported.lines) { /* consume stream */ }
    expect(findUnique).toHaveBeenCalledWith(expect.objectContaining({ where: { id: 'ns-1', teamId: 'team-1', deletedAt: null } }));
    expect(findMany).toHaveBeenCalled();
    expect(exported).not.toHaveProperty('token');
  });

  it('requires a team admin before metadata access', async () => {
    const auth = { ensureTeamAdmin: vi.fn(async () => { throw Object.assign(new Error('forbidden'), { status: 403 }); }) };
    await expect(service({ pluginSharedNamespace: { findMany: vi.fn() } }, auth).adminListNamespaces('member-1')).rejects.toMatchObject({ status: 403 });
  });
});
