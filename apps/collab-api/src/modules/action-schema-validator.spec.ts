...
describe('assertActionValue', () => {
  ...
  it('rejects missing, unknown and out-of-range fields with stable phase codes', () => {
    ...
    expect(() => assertActionValue(schema, { prompt: 'x', extra: true }, 'input')).toThrow(
      /不是允许字段/
    );
    try {
      assertActionValue(schema, { prompt: 'x', count: 8 }, 'output');
    } catch (error) {
      expect(error).toMatchObject({ code: 'action_output_invalid' });
    }
  });
});
...
describe('shared action adapter conformance fixtures', () => {
  const fixtures = JSON.parse(
    readFileSync(
      new URL('../../../desktop/action-adapter-conformance/fixtures.json', import.meta.url),
      'utf8'
    )
  ) as {
  ...
  for (const fixture of fixtures.cases) {
    it(`uses authoritative restricted schemas for ${fixture.id}`, () => {
      if (fixture.id === 'bad_input' || fixture.id === 'bad_artifact') {
        try {
          assertActionValue(fixtures.input_schema, fixture.input, 'input');
        } catch (error) {
          expect(error).toMatchObject({ code: fixture.expected_error });
          return;
        }
      ...
      if (fixture.id === 'bad_output') {
        try {
          assertActionValue(fixtures.output_schema, { ok: 'yes', echoed: 'broken' }, 'output');
        } catch (error) {
          expect(error).toMatchObject({ code: fixture.expected_error });
          return;
        }
      ...
      if (fixture.expected_output)
        expect(() =>
          assertActionValue(fixtures.output_schema, fixture.expected_output, 'output')
        ).not.toThrow();
    });
  }
});
