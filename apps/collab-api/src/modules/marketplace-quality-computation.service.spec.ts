...
import {
  ...
  type MarketplaceQualityComputationRepository,
  type MarketplaceQualityComputeRequest,
...
function request(
  overrides: Partial<MarketplaceQualityComputeRequest> = {}
): MarketplaceQualityComputeRequest {
  return {
    jobKey: 'quality:v1:daily:2026-07-16:pkg-1:p2:e3',
    kind: 'DAILY',
    ...
    scope: {
      ...
      packageId: 'pkg-1',
      releaseId: 'rel-1',
      currentReleaseActivatedAt: new Date(watermark.getTime() - 8 * day),
      pointerRevision: 2,
...
function repository(
  overrides: Partial<MarketplaceQualityComputationRepository> = {}
): MarketplaceQualityComputationRepository {
  return {
    claim: vi.fn(async () => ({
      id: 'computation-1',
      status: 'RUNNING',
...
describe('MarketplaceQualityComputationService', () => {
  it('uses policy/pointer/eligibility in daily keys and evaluates before commit', async () => {
    ...
    expect(repo.commit).toHaveBeenCalledWith(
      input,
      'computation-1',
      expect.objectContaining({ autoQualified: true, tier: 'QUALITY' })
    );
  });

  it('reuses a terminal computation for the same jobKey without creating another snapshot revision', async () => {
    const commit = vi.fn();
    const service = new MarketplaceQualityComputationService(
      repository({
        claim: vi.fn(async () => ({
          id: 'computation-1',
          status: 'SUCCEEDED',
  ...
  it('records a failed job and preserves the previous listing projection', async () => {
    ...
    const service = new MarketplaceQualityComputationService(
      repository({
        commit: vi.fn(async () => {
          throw new Error('commerce_facts_unavailable');
        }),
        fail,
      })
    );
  ...
  it('gives independent manual requests independent job identities', () => {
    ...
    expect(service.manualJobKey(scope, 'request-a')).not.toBe(
      service.manualJobKey(scope, 'request-b')
    );
  });
});
