...
function mockPrisma() {
  ...
  const tx = {
    user: {
      ...
      findUniqueOrThrow: user.findUniqueOrThrow,
      create: user.create,
    },
    auditLog,
...
describe('AuthService 找回密码 + 重置密码', () => {
  ...
  describe('forgotPassword', () => {
    ...
    it('空邮箱抛 bad_request', async () => {
      await expect(service.forgotPassword({ email: ' ' })).rejects.toMatchObject({
        status: 400,
        code: 'bad_request',
      });
    });
  ...
  describe('resetPassword', () => {
    ...
    it('新密码 <8 位抛 bad_request（先于 token 校验）', async () => {
      await expect(
        service.resetPassword({ token: 'any', newPassword: 'short' })
      ).rejects.toMatchObject({ status: 400, code: 'bad_request' });
    });
    ...
    it('合法 token 成功改密 + tokenVersion++ + 审计', async () => {
      ...
      const token = jwt.sign(
        { sub: 'u1', email: 'a@b.com', scope: 'pwd_reset', tokenVersion: 3 },
    ...
    it('账号非 ACTIVE 时抛 bad_request', async () => {
      ...
      await expect(
        service.resetPassword({ token, newPassword: 'newpass123' })
      ).rejects.toMatchObject({ status: 400, code: 'bad_request' });
      expect(prisma.$transaction).not.toHaveBeenCalled();
    });
    ...
    it('scope 非 pwd_reset 的 token 被拒绝', async () => {
      ...
      const token = jwt.sign({ sub: 'u1', email: 'a@b.com' }, process.env.JWT_SECRET!, {
        expiresIn: '15m',
      });
      await expect(
        service.resetPassword({ token, newPassword: 'newpass123' })
  ...
  describe('login 审计', () => {
    ...
    it('账号非 ACTIVE 时写 auth.login.failed 审计（reason=account_inactive）', async () => {
      ...
      await expect(service.login({ email: 'a@b.com', password: 'pass1234' })).rejects.toMatchObject(
      ...
      expect(prisma.auditLog.create).toHaveBeenCalledWith(
        expect.objectContaining({
          data: expect.objectContaining({
            ...
            actorUserId: 'u1',
            metadata: expect.objectContaining({ remainingMinutes: expect.any(Number) }),
          }),
        })
      );
      ...
      expect(prisma.auditLog.create).toHaveBeenCalledWith(
        expect.objectContaining({
          data: expect.objectContaining({
            action: 'auth.login.locked',
            actorUserId: 'u1',
            metadata: expect.objectContaining({ attempts: 5, lockMinutes: 15 }),
          }),
        })
      );
    });
    ...
    it('阈值/锁定期读 PlatformSetting（可配）', async () => {
      ...
      expect(prisma.auditLog.create).toHaveBeenCalledWith(
        expect.objectContaining({
          data: expect.objectContaining({
            action: 'auth.login.locked',
            metadata: expect.objectContaining({ attempts: 3, lockMinutes: 30 }),
          }),
        })
      );
    });
  ...
  describe('管理端验证码校验', () => {
    ...
    it('非平台管理员不能通过管理端登录入口', async () => {
      ...
      prisma.teamAdminApplication.findFirst = vi.fn(async () => null);
      await expect(
        service.adminLogin({ email: 'a@b.com', password: 'pass1234' })
    ...
    it('管理端 forgot 场景启用且验证通过则发送重置邮件', async () => {
      ...
      expect(geetest.validate).toHaveBeenCalledWith({
        lot_number: 'l',
        captcha_output: 'o',
        pass_token: 'p',
        gen_time: 'g',
  ...
  describe('verifyEmail', () => {
    it('token 无效时抛 bad_request', async () => {
      await expect(service.verifyEmail({ token: 'invalid-token' })).rejects.toMatchObject({
        status: 400,
        code: 'bad_request',
      });
      expect(prisma.user.update).not.toHaveBeenCalled();
    ...
    it('账号非 ACTIVE 时抛 bad_request', async () => {
      const token = jwt.sign(
        { sub: 'u1', email: 'a@b.com', scope: 'email_verify', tokenVersion: 0 },
        process.env.JWT_SECRET!,
        { expiresIn: '1h' }
      );
      prisma.user.findUnique.mockResolvedValue({
    ...
    it('token 内嵌 tokenVersion 与库不一致时抛 bad_request', async () => {
      const token = jwt.sign(
        { sub: 'u1', email: 'a@b.com', scope: 'email_verify', tokenVersion: 3 },
        process.env.JWT_SECRET!,
        { expiresIn: '1h' }
      );
      prisma.user.findUnique.mockResolvedValue({
