...
describe('ActionInvocationController desktop host boundary', () => {
  it('forces DESKTOP caller kind and ignores iframe-supplied caller metadata', async () => {
    ...
    const controller = new ActionInvocationController({ create } as never, { assertDeclaredDependency } as never);
    ...
    await controller.create(request, {
      ...
      preview: true,
      target: { package_id: 'target-package', release_id: 'target-release', sha256: 'b'.repeat(64), action_id: 'generate', action_contract_version: '1.0.0', action_surface_sha256: 'c'.repeat(64) },
      input: { prompt: 'demo' },
    });
    ...
    expect(assertDeclaredDependency).toHaveBeenCalledWith({
      ...
      package_id: 'target-package', release_id: 'target-release', sha256: 'b'.repeat(64), action_id: 'generate', action_contract_version: '1.0.0', action_surface_sha256: 'c'.repeat(64),
    });
  ...
  it('derives nested caller identity from the running parent invocation', async () => {
    ...
    const assertDeclaredDependency = vi.fn().mockResolvedValue(undefined);
    const controller = new ActionInvocationController({ create, nestedCaller } as never, { assertDeclaredDependency } as never);
    ...
    expect(assertDeclaredDependency).toHaveBeenCalledWith({
      ...
      release_id: 'parent-release',
      sha256: 'd'.repeat(64),
    }, 'video_generator', expect.objectContaining({ package_id: 'target-package' }));
  ...
  it('exposes claim/complete/fail as real terminal transitions instead of fake client success', async () => {
    ...
    await controller.fail(request, 'invocation-2', { code: 'action_runtime_unavailable', message: 'client unsupported' });

    expect(service.claimDesktop).toHaveBeenCalledWith('user-1', 'invocation-1');
    expect(service.completeDesktop).toHaveBeenCalledWith('user-1', 'invocation-1', { video_id: 'video-1' });
    expect(service.failDesktop).toHaveBeenCalledWith('user-1', 'invocation-2', 'action_runtime_unavailable', 'client unsupported');
  });
});
