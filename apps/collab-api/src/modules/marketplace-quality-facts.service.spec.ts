  ...
  it('returns DATA_UNAVAILABLE before persistent SETTLEMENT_V2 activation without reading orders', async () => {
    ...
    const adapter = new PrismaMarketplaceCommerceFactsAdapter({
      marketplaceCommerceState: {
        findUnique: vi
          .fn()
          .mockResolvedValue({ writerMode: 'DRAINING', settlementV2ActivatedAt: null }),
      },
      purchase: { findMany },
    } as never);
    await expect(adapter.read('package-1', watermark)).resolves.toMatchObject({
    ...
    const adapter = new PrismaMarketplaceCommerceFactsAdapter({
      marketplaceCommerceState: {
        findUnique: vi.fn().mockResolvedValue({
          writerMode: 'PAUSED',
          settlementV2ActivatedAt: new Date('2026-01-01T00:00:00Z'),
        }),
      },
        ...
        findMany: vi.fn().mockResolvedValue([
          {
            ...
            settlementVersion: 'SETTLEMENT_V2',
            status: 'SETTLED',
            createdAt: new Date(watermark.getTime() - 20 * day),
            refundableUntil: new Date(watermark.getTime() - 10 * day),
            refundedAt: null,
            ...override,
          },
        ]),
  ...
  it('replays append-only ratings and metric facts at one watermark, including security and anomaly signals', async () => {
    ...
    const ratingFindMany = vi.fn().mockResolvedValue([
      { teamId: 'team-1', score: 5, revision: 2, recordedAt: new Date(watermark.getTime() - day) },
      {
        ...
        score: 1,
        revision: 1,
        recordedAt: new Date(watermark.getTime() - 2 * day),
      },
      { teamId: 'team-2', score: 4, revision: 1, recordedAt: new Date(watermark.getTime() - day) },
      {
        ...
        score: 5,
        revision: 1,
        recordedAt: new Date(watermark.getTime() - day),
      },
    ]);
    ...
    const prisma = {
      marketplaceListing: {
        findUnique: vi.fn().mockResolvedValue({
          ...
          priceCents: 0,
          currentReleaseActivatedAt: new Date(watermark.getTime() - 10 * day),
          pointerRevision: 2,
          eligibilityRevision: 3,
          eligibilityGateDigest: 'a'.repeat(64),
          listingEligibleSince: new Date(watermark.getTime() - 20 * day),
          releaseEligibleSince: new Date(watermark.getTime() - 10 * day),
          status: 'ACTIVE',
    ...
    expect(metricFindMany).toHaveBeenCalledWith(
      expect.objectContaining({
        where: expect.objectContaining({ recordedAt: { lte: watermark } }),
      })
    );
    expect(ratingFindMany).toHaveBeenCalledWith(
      expect.objectContaining({ where: { packageId: 'package-1', recordedAt: { lte: watermark } } })
    );
