...
export class MarketplaceSettlementCutoverService implements OnModuleInit, OnModuleDestroy {
  ...
  async assertWriterFence(
    expectedGeneration: number,
    allowedModes: Array<'SETTLEMENT_V2' | 'PAUSED'> = ['SETTLEMENT_V2']
  ) {
    const row = await this.prisma.marketplaceCommerceState.findUnique({
      where: { id: 'singleton' },
    });
    if (
      !row ||
      row.writerGeneration !== expectedGeneration ||
      !allowedModes.includes(row.writerMode as 'SETTLEMENT_V2' | 'PAUSED')
    ) {
  ...
  async beginDraining(expectedGeneration: number) {
    ...
    if (changed.count !== 1)
      throw new AppError(409, 'marketplace_cutover_state_conflict', '市场结算不能进入排空状态');
    return this.state();
  }
  ...
  async activate(expectedGeneration: number, now = new Date()) {
    ...
    if (!reconciliation.ok) {
      throw new AppError(
        409,
        'marketplace_cutover_reconciliation_failed',
        '市场结算对账未通过，不能激活'
      );
    }
    const activated = await this.prisma.marketplaceCommerceState.updateMany({
      ...
      data: {
        ...
        settlementV2ActivatedAt: row.settlementV2ActivatedAt ?? now,
      },
    });
    if (activated.count !== 1)
      throw new AppError(409, 'marketplace_cutover_state_conflict', '市场结算激活状态已变化');
  ...
  async pause(expectedGeneration: number, reason = '') {
    ...
    if (changed.count !== 1)
      throw new AppError(409, 'marketplace_cutover_state_conflict', '市场结算暂停状态已变化');
    return this.state();
  }
  ...
  async resume(expectedGeneration: number) {
    ...
    if (changed.count !== 1)
      throw new AppError(409, 'marketplace_cutover_state_conflict', '市场结算恢复状态已变化');
    return this.state();
  }
  ...
  async backfillLegacy(
  ...
  ): Promise<MarketplaceBackfillReport> {
    ...
    return this.serializable(async (tx) => {
      ...
      for (const row of rows) {
        const gross = Math.max(0, Number(row.priceCents ?? 0));
        report.gross_cents += gross;
        ...
        const valid =
          row.status === 'SETTLED' &&
          row.listPriceCents === gross &&
          row.sellerAmountCents === gross &&
          row.platformAmountCents === 0 &&
          row.platformFeeBps === 0 &&
          row.settledAt;
        if (!valid) report.null_or_invalid_orders += 1;
        if (
          !row.entitlement ||
          row.entitlement.status !== 'ACTIVE' ||
          row.entitlement.purchaseId !== row.id
        )
          report.entitlement_mismatch += 1;
        if (dryRun) continue;
        const data: Record<string, unknown> = {
          settlementVersion: 'LEGACY_V1',
          status: 'SETTLED',
          listPriceCents: gross,
          discountAmountCents: 0,
          platformFeeBps: 0,
          platformAmountCents: 0,
          sellerAmountCents: gross,
          settledAt: row.settledAt ?? row.createdAt,
        };
        if (sellerTeamId && !row.sellerTeamId) data.sellerTeamId = sellerTeamId;
        const needsOrderUpdate = Object.entries(data).some(
          ([key, value]) => (row as any)[key] !== value
        );
        if (needsOrderUpdate) {
          const changed = await client.purchase.updateMany({
            where: { id: row.id, settlementVersion: 'LEGACY_V1' },
            data,
          });
        ...
        if (row.packageId) {
          const existing =
            row.entitlement ??
            (await client.pluginEntitlement.findUnique({
              where: { teamId_packageId: { teamId: row.buyerTeamId, packageId: row.packageId } },
            }));
          if (!existing) {
            await client.pluginEntitlement.create({
              data: {
                teamId: row.buyerTeamId,
            ...
            const changed = await client.pluginEntitlement.updateMany({
              where: { id: existing.id, purchaseId: null },
              data: {
                purchaseId: row.id,
                status: 'ACTIVE',
                activatedAt: existing.activatedAt ?? row.createdAt,
              },
            });
            if (changed.count === 1) report.mutated += 1;
          }
  ...
  async reconcile(
    options: { preCutover?: boolean } = {}
  ): Promise<MarketplaceReconciliationReport> {
    const report = await this.readReconciliation(options);
    const state = await this.state();
    if (state) {
      await this.prisma.marketplaceCommerceState.updateMany({
        where: { id: 'singleton' },
        data: {
          lastReconciliationAt: new Date(),
          lastReconciliationStatus: report.ok ? 'OK' : 'FAILED',
          lastReconciliationReport: report as unknown as Prisma.InputJsonValue,
        },
      });
    }
    return report;
  ...
  async runSettlementJob(now = new Date(), limit = 100) {
    ...
    await this.prisma.marketplaceCommerceState.updateMany({
      where: { id: 'singleton' },
      data: {
        lastSettlementJobAt: startedAt,
        lastSettlementJobStatus: 'RUNNING',
        lastSettlementJobError: '',
      },
    });
    try {
      ...
      await this.prisma.marketplaceCommerceState.updateMany({
        where: { id: 'singleton' },
        data: {
          lastSettlementJobAt: new Date(),
          lastSettlementJobStatus: 'SUCCEEDED',
          lastSettlementJobScanned: result.scanned,
          lastSettlementJobSettled: result.settled,
          lastSettlementJobSkipped: result.skipped,
          lastSettlementJobError: '',
        },
      });
      ...
      const message =
        error instanceof Error ? error.message.slice(0, 500) : 'settlement_job_failed';
      await this.prisma.marketplaceCommerceState.updateMany({
        where: { id: 'singleton' },
        data: {
          lastSettlementJobAt: new Date(),
          lastSettlementJobStatus: 'FAILED',
          lastSettlementJobError: message,
        },
      });
      throw error;
    }
  ...
  async settlementJobStatus() {
    ...
    const overdue = await this.prisma.purchase.count({
      where: {
    ...
    const refundReview = await this.prisma.purchase.count({
      where: { settlementVersion: 'SETTLEMENT_V2', status: 'REFUND_REQUESTED' },
    });
    const oldest = await this.prisma.purchase.findFirst({
      where: {
        settlementVersion: 'SETTLEMENT_V2',
        status: 'PENDING_SETTLEMENT',
        settleAt: { lte: new Date() },
    ...
    return {
      last_run_at: state.lastSettlementJobAt?.toISOString() ?? null,
      status: state.lastSettlementJobStatus ?? null,
      scanned: state.lastSettlementJobScanned,
      settled: state.lastSettlementJobSettled,
      skipped: state.lastSettlementJobSkipped,
      error: state.lastSettlementJobError,
      due_count: overdue,
      refund_review_count: refundReview,
      oldest_overdue_at: oldest?.settleAt?.toISOString() ?? null,
    };
  }

  private async readReconciliation(options: {
    preCutover?: boolean;
  }): Promise<MarketplaceReconciliationReport> {
    const client = this.prisma as AnyClient;
    const rows = await client.purchase.findMany({
      where: options.preCutover ? {} : { settlementVersion: 'SETTLEMENT_V2' },
      include: {
        package: { select: { ownerTeamId: true } },
        entitlement: true,
        ledgerEntries: true,
      },
    });
    ...
    const entitlementViolations: string[] = [];
    let gross = 0;
    let pending = 0;
    let review = 0;
    let settled = 0;
    let refunded = 0;
    let clearingExpected = 0;
    let revenueExpected = 0;
    let legacy = 0;
    let v2 = 0;
    let unresolvedSeller = 0;
    let unresolvedRelease = 0;
    for (const row of rows) {
      if (row.settlementVersion === 'LEGACY_V1') legacy += 1;
      else v2 += 1;
      const amount = Number(row.priceCents ?? 0);
      gross += amount;
      ...
      if (row.settlementVersion === 'LEGACY_V1') {
        if (
          row.status !== 'SETTLED' ||
          row.sellerAmountCents !== amount ||
          row.platformAmountCents !== 0
        )
          invalidOrders.push(row.id);
        if (
          row.packageId &&
          (row.entitlement?.status !== 'ACTIVE' || row.entitlement?.purchaseId !== row.id)
        )
          entitlementViolations.push(row.id);
        continue;
      }
      if (
        row.listPriceCents - row.discountAmountCents !== amount ||
        row.platformAmountCents + row.sellerAmountCents !== amount ||
        row.platformFeeBps < 0 ||
        row.platformFeeBps > 10000
      )
        invalidOrders.push(row.id);
      if (row.status === 'PENDING_SETTLEMENT') {
        pending += amount;
        clearingExpected += amount;
      }
      ...
      if (row.status === 'SETTLED') {
        settled += amount;
        revenueExpected += row.platformAmountCents;
      }
      if (row.status === 'REFUNDED') refunded += amount;
      if (
        (row.status === 'PENDING_SETTLEMENT' || row.status === 'REFUND_REQUESTED') &&
        row.entitlement?.status !== 'ACTIVE'
      )
        entitlementViolations.push(row.id);
      if (row.status === 'REFUNDED' && row.entitlement?.status !== 'REVOKED')
        entitlementViolations.push(row.id);
      ...
      >([
        [
          ...
          { amount, direction: 'DEBIT', teamId: row.buyerTeamId, platformAccountId: null },
        ],
        [
          'PLATFORM_PURCHASE_CLEARING_CREDIT',
          { amount, direction: 'CREDIT', teamId: null, platformAccountId: CLEARING_ACCOUNT_ID },
        ],
      ]);
      if (row.status === 'SETTLED') {
        expected.set('PLATFORM_SETTLEMENT_CLEARING_DEBIT', {
          amount,
          direction: 'DEBIT',
          teamId: null,
          platformAccountId: CLEARING_ACCOUNT_ID,
        });
        expected.set('SELLER_SETTLEMENT_CREDIT', {
          amount: row.sellerAmountCents,
          direction: 'CREDIT',
          teamId: row.sellerTeamId,
          platformAccountId: null,
        });
        expected.set('PLATFORM_SETTLEMENT_CREDIT', {
          amount: row.platformAmountCents,
          direction: 'CREDIT',
          teamId: null,
          platformAccountId: REVENUE_ACCOUNT_ID,
        });
      }
      if (row.status === 'REFUNDED') {
        expected.set('BUYER_REFUND_CREDIT', {
          amount,
          direction: 'CREDIT',
          teamId: row.buyerTeamId,
          platformAccountId: null,
        });
        expected.set('PLATFORM_REFUND_CLEARING_DEBIT', {
          amount,
          direction: 'DEBIT',
          teamId: null,
          platformAccountId: CLEARING_ACCOUNT_ID,
      ...
      const badLedger =
        ...
        [...expected].some(([kind, wanted]) => {
          const entry: any = indexed.get(kind);
          return (
            !entry ||
            Number(entry.amountCents) !== wanted.amount ||
            entry.direction !== wanted.direction ||
            (entry.teamId ?? null) !== wanted.teamId ||
            (entry.platformAccountId ?? null) !== wanted.platformAccountId
          );
        });
      if (badLedger) ledgerViolations.push(row.id);
    }
    const accounts = await client.marketplacePlatformAccount.findMany({
      where: { id: { in: [CLEARING_ACCOUNT_ID, REVENUE_ACCOUNT_ID] } },
      select: { id: true, balanceCents: true },
    });
    const accountMap = new Map<string, number>(
      accounts.map((row: any) => [String(row.id), Number(row.balanceCents)])
    );
    ...
    if (clearingActual === null || revenueActual === null)
      reasonCodes.push('platform_account_missing');
    else if (clearingActual !== clearingExpected || revenueActual !== revenueExpected)
      reasonCodes.push('platform_account_balance_mismatch');
    const ok = reasonCodes.length === 0 && (!options.preCutover || legacy === rows.length);
    return {
      ok,
      settlement_version: legacy && v2 ? 'MIXED' : v2 ? 'SETTLEMENT_V2' : 'LEGACY_V1',
      purchase_count: rows.length,
      gross_cents: gross,
      pending_cents: pending,
      refund_requested_cents: review,
      settled_cents: settled,
      refunded_cents: refunded,
      clearing_expected_cents: clearingExpected,
      clearing_actual_cents: clearingActual,
      revenue_expected_cents: revenueExpected,
      revenue_actual_cents: revenueActual,
      invalid_orders: invalidOrders,
      ledger_violations: ledgerViolations,
      entitlement_violations: entitlementViolations,
      unresolved_seller: unresolvedSeller,
      unresolved_release: unresolvedRelease,
      reason_codes: reasonCodes,
    };
  }

  private async ledgerSnapshot(client: AnyClient) {
    const rows = await client.balanceLedger.findMany({
      where: { marketplaceEntryKind: { not: null } },
      orderBy: { id: 'asc' },
      select: {
        id: true,
        amountCents: true,
        direction: true,
        marketplaceEntryKind: true,
        purchaseId: true,
      },
    });
    ...
    return {
      ...
      debit_cents: rows
        ...
        .reduce((sum: number, row: any) => sum + Number(row.amountCents), 0),
      credit_cents: rows
