...
type ExactRouteTarget = Pick<
  ActionTarget,
  'release_id' | 'action_id' | 'action_contract_version' | 'action_surface_sha256'
> & { environment: Environment };

export function cloudRoutingBucket(runId: string, nodePath: string, generation: number): number {
  return (
    createHash('sha256').update(`${runId}\0${nodePath}\0${generation}`).digest().readUInt32BE(0) %
    100
  );
}
...
export class CloudActionRoutingService {
  ...
  async update(
    ...
    releaseId: string,
    actionId: string,
    input: UpdateCloudActionRoutingDto
  ) {
    const membership = await this.managementContext(userId);
    const target = {
      release_id: releaseId,
      action_id: actionId,
      action_contract_version: input.action_contract_version,
      action_surface_sha256: input.action_surface_sha256,
      environment: input.environment,
    };
    await this.assertOwnedRelease(membership.teamId, releaseId);
    const stable = await this.readyDeployment(
      membership.teamId,
      target,
      input.stable_deployment_id
    );
    const candidateId = input.candidate_deployment_id || null;
    const candidatePercent = input.candidate_percent ?? 0;
    if (candidatePercent > 0 && !candidateId)
      throw new AppError(
        400,
        'cloud_routing_conflict',
        'candidate_percent 大于 0 时必须指定 candidate deployment'
      );
    if (candidateId === stable.id)
      throw new AppError(400, 'cloud_routing_conflict', 'stable 与 candidate 不能相同');
    if (candidateId) await this.readyDeployment(membership.teamId, target, candidateId);
    const where = {
      releaseId_actionId_actionContractVersion_actionSurfaceSha256_environment: {
        releaseId,
        actionId,
        actionContractVersion: input.action_contract_version,
        actionSurfaceSha256: input.action_surface_sha256,
        environment: input.environment,
      },
    } as const;
    ...
    if (!current) {
      ...
      try {
        route = await this.prisma.cloudActionRouting.create({
          data: {
            releaseId,
            actionId,
            actionContractVersion: input.action_contract_version,
        ...
        if (error instanceof Prisma.PrismaClientKnownRequestError && error.code === 'P2002')
          throw conflict('Cloud routing generation 已变化');
        throw error;
      }
    } else {
      const changed = await this.prisma.cloudActionRouting.updateMany({
        where: { id: current.id, generation: input.expected_generation },
        data: {
          stableDeploymentId: stable.id,
          candidateDeploymentId: candidateId,
          candidatePercent,
  ...
  async freeze(
    ...
    const release = await this.prisma.pluginRelease.findFirst({
      ...
      select: { package: { select: { ownerTeamId: true } } },
    });
    ...
    if (!route)
      throw new AppError(409, 'cloud_endpoint_not_ready', 'Action 尚未配置 Cloud routing');
    const bucket = cloudRoutingBucket(runId, nodePath, route.generation);
    const selected =
      route.candidateDeployment && route.candidatePercent > bucket
        ? route.candidateDeployment
        : route.stableDeployment;
    if (
      !this.matchesReadyDeployment(selected, release.package.ownerTeamId, {
    ...
    return {
      ...
      routing_generation: route.generation,
      environment,
      policy_decision_id: '',
    };
  }
  ...
  private async assertOwnedRelease(
    teamId: string,
    releaseId: string,
    packageId?: string,
    sha256?: string
  ...
  private async readyDeployment(teamId: string, target: ExactRouteTarget, id: string) {
    ...
    if (!deployment)
      throw new AppError(
        ...
        'routing 只能引用同一精确 Action 的 READY deployment'
      );
    return deployment;
  }

  private matchesReadyDeployment(
    deployment: any,
    teamId: string,
    target: ExactRouteTarget
  ): boolean {
    return Boolean(
      deployment &&
  ...
  private publicRoute(route: any) {
    return {
      target: {
        ...
        action_contract_version: route.actionContractVersion,
        action_surface_sha256: route.actionSurfaceSha256,
        environment: route.environment,
      },
      stable_deployment_id: route.stableDeploymentId,
      candidate_deployment_id: route.candidateDeploymentId,
      candidate_percent: route.candidatePercent,
