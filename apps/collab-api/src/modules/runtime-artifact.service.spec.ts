...
beforeEach(() => {
  process.env.RUNTIME_ARTIFACT_SIGNING_SECRET = 'x'.repeat(32);
});
describe('RuntimeArtifactService', () => {
  it('creates invocation artifacts with a host-owned object key and signed ref', async () => {
    ...
    const prisma = {
      actionInvocation: { findFirst: vi.fn().mockResolvedValue(invocation) },
    ...
    const service = new RuntimeArtifactService(
      prisma as never,
      { ensureCurrentTeam: vi.fn().mockResolvedValue({ teamId: 't1' }) } as never,
      store as never
    );
    const result = await service.createFromInvocation('u1', 'i1', {
      data_base64: 'UE5H',
      media_type: 'image/png',
    });
    expect(store.promote).toHaveBeenCalledWith(
      expect.any(String),
      expect.stringMatching(/^runtime-artifacts\/t1\//),
    ...
    expect(result).toMatchObject({
      ...
      media_type: 'image/png',
      size_bytes: 3,
      authorization: { team_id: 't1', handle: expect.any(String) },
    });
  });
  it('materializes only a signed ref granted to the current invocation', async () => {
    ...
    const prisma = {
      actionInvocation: {
        findFirst: vi.fn().mockResolvedValue({ id: 'i1', teamId: 't1', kind: 'STANDARD' }),
      },
      runtimeArtifact: {
    ...
    const service = new RuntimeArtifactService(
      ...
      { ensureCurrentTeam: vi.fn().mockResolvedValue({ teamId: 't1' }) } as never,
      {
        download: vi
          .fn()
          .mockResolvedValue({ kind: 'stream', stream: Readable.from([bytes]), sizeBytes: 3 }),
      } as never
    );
    await expect(service.materializeForInvocation('u1', 'i1', ref)).resolves.toMatchObject({
      data_base64: 'UE5H',
      media_type: 'image/png',
      size_bytes: 3,
      sha256: ref.sha256,
    });
    expect(prisma.actionInvocation.findFirst).toHaveBeenCalledWith({
      where: expect.objectContaining({ principalUserId: 'u1', status: 'RUNNING' }),
    });
  });
  it('atomically binds workflow input only when a live concrete source grant exists', async () => {
    ...
    const tx = {
      ...
      actionInvocation: {
        findFirst: vi.fn().mockResolvedValue({ id: 'inv1', principalUserId: 'u1' }),
      },
      ...
      runtimeArtifactGrant: {
        findMany: vi
          .fn()
          .mockResolvedValue([
            { id: 'source-invocation-grant', targetKind: 'INVOCATION', targetId: 'inv1' },
          ]),
        findUnique: vi.fn().mockResolvedValue(null),
        create: grantCreate,
      },
    ...
    await expect(
      service.bindWorkflowInputsTx(tx as never, {
        runId: 'run1',
        teamId: 't1',
        principalUserId: 'u1',
        kind: 'STANDARD',
    ...
    expect(grantCreate).toHaveBeenCalledWith({
      data: expect.objectContaining({ targetKind: 'WORKFLOW_RUN', targetId: 'run1' }),
    });
    expect(holdCreate).toHaveBeenCalledWith({
      data: expect.objectContaining({ purpose: 'RUN_INPUT', holderId: 'run1:RUN_INPUT:/image' }),
    });
  });
  it('does not treat same-team ownership as workflow input authority', async () => {
    ...
    const tx = {
      ...
      runtimeArtifactGrant: { findMany: vi.fn().mockResolvedValue([]) },
    };
    await expect(
      signer.bindWorkflowInputsTx(tx as never, {
        runId: 'run1',
        teamId: 't1',
        principalUserId: 'u1',
        kind: 'STANDARD',
  ...
  it('requires initiator or team admin plus a live final-output grant to download', async () => {
    const store = {
      download: vi.fn().mockResolvedValue({ kind: 'stream', stream: {}, sizeBytes: 12 }),
    };
    const prisma = {
      workflowRun: {
        findFirst: vi.fn().mockResolvedValue({
          ...
          resultRetainUntil: new Date('2099-01-01'),
        }),
      },
      runtimeArtifact: {
        findFirst: vi.fn().mockResolvedValue({
    ...
    const member = new RuntimeArtifactService(
      prisma as never,
      { ensureCurrentTeam: vi.fn().mockResolvedValue({ teamId: 't1', role: 'MEMBER' }) } as never,
      store as never
    );
    await expect(member.authorizeWorkflowResult('stranger', 'run1', 'a1')).rejects.toMatchObject({
      code: 'forbidden',
    });
    const admin = new RuntimeArtifactService(
      prisma as never,
      {
        ensureCurrentTeam: vi.fn().mockResolvedValue({ teamId: 't1', role: 'TEAM_ADMIN' }),
      } as never,
      store as never
    );
    await expect(admin.authorizeWorkflowResult('admin', 'run1', 'a1')).resolves.toMatchObject({
      artifact: { id: 'a1' },
    });
    expect(prisma.runtimeArtifactGrant.findFirst).toHaveBeenCalledWith({
      where: expect.objectContaining({ targetId: 'run1:FINAL_OUTPUT' }),
    });
  });
  it('signs refs and detects metadata tampering', async () => {
    const service = new RuntimeArtifactService({} as never, {} as never);
    const ref = service.toRef({
      id: 'a1',
      teamId: 't1',
      mediaType: 'image/png',
      sizeBytes: 12,
    ...
    const resolver = new RuntimeArtifactService(
      prisma as never,
      { ensureCurrentTeam: vi.fn().mockResolvedValue({ teamId: 't1' }) } as never
    );
    await expect(resolver.resolveForInvocation('u1', 'i1', tampered)).rejects.toMatchObject({
      code: 'forbidden',
    });
    expect(prisma.actionInvocation.findFirst).not.toHaveBeenCalled();
  });
  it('refuses to reopen a released hold', async () => {
    const tx = {
      runtimeArtifact: { findUnique: vi.fn().mockResolvedValue({ id: 'a1', status: 'ACTIVE' }) },
      runtimeArtifactHold: {
        findUnique: vi.fn().mockResolvedValue({
          id: 'h1',
          releasedAt: new Date(),
          retainUntil: new Date(Date.now() + 10000),
        }),
      },
    };
    const service = new RuntimeArtifactService(
      { $transaction: (fn: any) => fn(tx) } as never,
      {} as never
    );
    await expect(
      service.acquireHold(
  ...
  it('composite kind lookup prevents a PREVIEW artifact from receiving a STANDARD grant', async () => {
    ...
    expect(tx.runtimeArtifact.findUnique).toHaveBeenCalledWith({
      where: { id_executionKind: { id: 'a1', executionKind: 'STANDARD' } },
    });
  });
  it('binds a source-authorized STANDARD artifact to a canonical shared value grant and hold', async () => {
    ...
    const tx = {
      actionInvocation: {
        findUnique: vi
          .fn()
          .mockResolvedValue({ id: 'i1', teamId: 't1', kind: 'STANDARD', status: 'RUNNING' }),
      },
      runtimeArtifact: {
        findFirst: vi.fn().mockResolvedValue({
          id: 'a1',
      ...
      runtimeArtifactHold: {
        findUnique: vi.fn().mockResolvedValue(null),
        create: vi.fn(async ({ data }) => ({ id: 'hold', ...data })),
      },
    };
    const service = new RuntimeArtifactService({} as never, {} as never);
    await expect(
      service.bindSharedValueTx(
        tx as never,
        'i1',
        ref,
        'ns:1:key:2',
    ...
    ).resolves.toMatchObject({ grant: { targetKind: 'SHARED_VALUE', targetId: 'ns:1:key:2' } });
    expect(tx.runtimeArtifactHold.create).toHaveBeenCalledWith({
      data: expect.objectContaining({ holderKind: 'SHARED_VALUE', holderId: 'ns:1:key:2' }),
    });
  });
  it('never accepts PREVIEW invocation as a shared value source', async () => {
    const tx = {
      actionInvocation: {
        findUnique: vi
          .fn()
          .mockResolvedValue({ id: 'i1', teamId: 't1', kind: 'PREVIEW', status: 'RUNNING' }),
      },
    };
    const service = new RuntimeArtifactService({} as never, {} as never);
    const ref = service.toRef({
    ...
    await expect(
      service.bindSharedValueTx(tx as never, 'i1', ref, 'target', {}, new Date(Date.now() + 60_000))
    ).rejects.toMatchObject({ code: 'forbidden' });
  });
  it('rejects binding an artifact whose platform shared-retention cap already elapsed', async () => {
    ...
    await expect(
      service.bindSharedValueTx(tx as never, 'i1', ref, 'target', {}, new Date(Date.now() + 60_000))
    ).rejects.toMatchObject({ code: 'shared_artifact_expired' });
  ...
  it('repairs missing canonical shared grant and hold without requiring the source invocation', async () => {
    ...
    const tx = {
      runtimeArtifact: {
        findUnique: vi
          .fn()
          .mockResolvedValue({ id: 'a1', status: 'ACTIVE', retainUntil: new Date(0) }),
      ...
      runtimeArtifactHold: {
        findUnique: vi.fn().mockResolvedValue(null),
        create: vi.fn(async ({ data }) => ({ id: 'h1', ...data })),
      },
    };
    const service = new RuntimeArtifactService({} as never, {} as never);
    await expect(
      service.reconcileSharedValueTx(
        tx as never,
        'a1',
        'ns:1:key:2',
        { value_revision: '2' },
    ...
    expect(tx.runtimeArtifact.updateMany).toHaveBeenCalledWith(
      expect.objectContaining({ data: { retainUntil } })
    );
  });
  it('does not exchange a shared grant when its retention hold is missing or released', async () => {
    const tx = {
      actionInvocation: {
        findUnique: vi
          .fn()
          .mockResolvedValue({ id: 'reader', teamId: 't1', kind: 'STANDARD', status: 'RUNNING' }),
      },
  ...
  it('reconciles a crash-window HANDOFF_PENDING hold into a canonical EDGE hold', async () => {
    const pending = {
      ...
      scopeDigest: 'scope',
      holderKey: 'pending-key',
      retainUntil: new Date('2099-01-08T00:00:00.000Z'),
      releasedAt: null,
      ...
      workflowStepAttempt: {
        findUnique: vi.fn().mockResolvedValue({
          ...
          run: {
            ...
            frozenPlan: {
              nodes: [
                { node_id: 'image', depends_on: [] },
                { node_id: 'video', depends_on: ['image'] },
              ],
            },
            resultRetainUntil: new Date('2099-01-08T00:00:00.000Z'),
          },
        }),
      },
      $transaction: vi.fn(async (fn: (client: typeof tx) => Promise<unknown>) => fn(tx)),
    ...
    expect(destinationCreate).toHaveBeenCalledWith({
      data: expect.objectContaining({
        artifactId: 'artifact-1',
        holderKind: 'WORKFLOW_RUN',
        holderId: 'run-1:attempt-1:video',
        purpose: 'EDGE',
      }),
    });
    expect(releasePending).toHaveBeenCalledWith({
      where: { id: 'pending-1', releasedAt: null },
  ...
  it('pins a signed workflow output ref before terminal projection', async () => {
    ...
    const service = new RuntimeArtifactService({} as never, {} as never);
    await expect(
      service.acquireHandoffPendingTx(tx as never, {
  ...
  it('releases transient run holds but preserves final output retention on success', async () => {
    ...
    const service = new RuntimeArtifactService({} as never, {} as never);
    await service.releaseRunHoldsTx(
      { runtimeArtifactHold: { updateMany } } as never,
      'run-1',
      true
    );
    expect(updateMany).toHaveBeenCalledWith({
      where: {
        holderKind: 'WORKFLOW_RUN',
        holderId: { startsWith: 'run-1:' },
        purpose: { in: ['RUN_INPUT', 'EDGE', 'HANDOFF_PENDING'] },
        releasedAt: null,
      },
      data: { releasedAt: expect.any(Date) },
    });
