...
const schedule = {
  ...
  generation: 2,
  status: 'ACTIVE',
  schedulerKey: 'schedule-schedule-1-g2',
  kind: 'DAILY',
  timeZone: 'UTC',
  localTime: '10:00',
  dayOfWeek: null,
...
describe('AutomationScheduleFireProcessor', () => {
  it('does not execute historical Cloud schedules when configured', async () => {
    ...
    const processor = new AutomationScheduleFireProcessor(
      { automationSchedule: { findFirst } } as never,
      { startScheduled } as never,
      { enabled: true, schedulesEnabled: true } as never
    );
    await expect(
      processor.process({
        kind: 'ONCE',
    ...
    ).resolves.toEqual({ outcome: 'DEPRECATED', run_id: null });
  ...
  it('ignores a stale generation or scheduler key without creating a run', async () => {
    const startScheduled = vi.fn();
    const processor = new AutomationScheduleFireProcessor(
      { automationSchedule: { findFirst: vi.fn(async () => null) } } as never,
      { startScheduled } as never
    );
    await expect(
      processor.process({
    ...
    ).resolves.toEqual({ outcome: 'STALE', run_id: null });
    expect(startScheduled).not.toHaveBeenCalled();
  });
  ...
  it('uses prevMillis and repeatJobKey and returns an existing occurrence idempotently', async () => {
    ...
    await expect(
      processor.process({
        ...
        prev_millis: Date.parse('2026-07-20T10:00:00.000Z'),
        repeat_job_key: schedule.schedulerKey,
      })
  ...
  it('creates one scheduled run and advances recurring nextRunAt', async () => {
    ...
    expect(startScheduled).toHaveBeenCalledWith(
      'user-1',
      schedule,
      new Date('2026-07-20T10:00:00.000Z'),
      'schedule-1:g2:2026-07-20T10:00:00.000Z'
    );
    expect(updateMany).toHaveBeenCalledWith(
      expect.objectContaining({ data: { nextRunAt: new Date('2026-07-21T10:00:00.000Z') } })
    );
    vi.useRealTimers();
  });
});
