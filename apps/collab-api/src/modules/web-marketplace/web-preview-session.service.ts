...
export class WebPreviewSessionService {
  ...
  async create(userId: string, packageId: string) {
    ...
    if (detail.preview_mode === 'STATIC_DESKTOP')
      throw new AppError(409, 'web_preview_unavailable', '该插件仅支持桌面运行');
    const nonce = randomBytes(32).toString('base64url');
    const expiresAt = new Date(Date.now() + 5 * 60 * 1000);
    const session = await this.prisma.webPreviewSession.create({
      data: {
        userId,
  ...
  async consume(userId: string, sessionId: string, nonce: string) {
    ...
    const session = await this.prisma.webPreviewSession.findFirst({
      where: { id: sessionId, userId, teamId: membership.teamId },
    });
    if (!session) throw notFound('预览会话不存在');
    if (session.expiresAt <= new Date())
      throw new AppError(410, 'web_preview_session_expired', '预览会话已过期');
    if (digest(nonce) !== session.nonceSha256)
      throw new AppError(403, 'web_preview_nonce_invalid', '预览握手 nonce 无效');
