...
export class WebPreviewSessionController {
  constructor(@Inject(WebPreviewSessionService) private readonly previews: WebPreviewSessionService) {}

  @Post(':packageId/sessions')
  create(@Req() req: Request, @Param('packageId') packageId: string) {
    if (!z.string().uuid().safeParse(packageId).success) throw new AppError(400, 'web_plugin_id_invalid', '插件 packageId 无效');
  ...
  consume(@Req() req: Request, @Param('sessionId') sessionId: string, @Body() body: unknown) {
    const parsed = z.object({ nonce: z.string().min(32).max(256) }).strict().safeParse(body);
    if (!parsed.success || !z.string().uuid().safeParse(sessionId).success) throw new AppError(400, 'web_preview_handshake_invalid', '预览握手参数无效');
