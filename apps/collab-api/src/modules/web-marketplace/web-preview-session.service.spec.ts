...
describe('WebPreviewSessionService', () => {
  it('issues a short nonce once without persisting plaintext', async () => {
    ...
    expect(create).toHaveBeenCalledWith({
      data: expect.objectContaining({ nonceSha256: expect.stringMatching(/^[a-f0-9]{64}$/) }),
    });
  ...
  it('rejects static desktop preview without creating a session', async () => {
    ...
    await expect(service.create('user-1', 'pkg-1')).rejects.toMatchObject({
      code: 'web_preview_unavailable',
    });
  ...
  it('atomically consumes a matching nonce and rejects replay', async () => {
    ...
    await expect(service.consume('user-1', 'session-1', nonce)).resolves.toMatchObject({
      ok: true,
    });
    updateMany.mockResolvedValueOnce({ count: 0 });
    await expect(service.consume('user-1', 'session-1', nonce)).rejects.toMatchObject({
      status: 409,
    });
  });
});
