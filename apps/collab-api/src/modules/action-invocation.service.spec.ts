...
const input = {
  ...
  caller: { kind: 'DESKTOP' as const, id: 'desktop-session' },
};
const row = (overrides = {}) => ({
  id: 'i1',
  teamId: 't1',
  ...
  inputSha256: 'x',
  output: null,
  deadlineAt: new Date(input.deadline_at),
  createdAt: new Date(),
  startedAt: null,
  completedAt: null,
  errorCode: '',
...
describe('ActionInvocationService', () => {
  it('authorizes exactly once and persists an AUTHORIZED invocation', async () => {
    const create = vi
      .fn()
      .mockImplementation(({ data }) => row({ ...data, inputSha256: data.inputSha256 }));
    const authorize = vi.fn().mockResolvedValue({
      decision: { policy_revision: 1 },
      required_operations: ['invoke_action'],
      action: {
        input_schema: {
          type: 'object',
        ...
        execution: { runtime_type: 'nodejs', entry: 'index.js', export: 'generate' },
      },
    });
    const service = new ActionInvocationService(
      { actionInvocation: { findUnique: vi.fn().mockResolvedValue(null), create } } as never,
  ...
  it('returns the existing row without a second authorization for an identical request', async () => {
    const existing = row({
      inputSha256: createHash('sha256').update('{"prompt":"hello"}').digest('hex'),
    });
    const prisma = {
      actionInvocation: { findUnique: vi.fn().mockResolvedValue(existing), create: vi.fn() },
    } as any;
    ...
    expect(prisma.actionInvocation.create).not.toHaveBeenCalled();
  });
    ...
    const service = new ActionInvocationService(
      {
        actionInvocation: {
          updateMany,
          findFirst: vi.fn().mockResolvedValue(row({ status: 'RUNNING', startedAt: new Date() })),
        },
      } as never,
      { ensureCurrentTeam: vi.fn().mockResolvedValue({ teamId: 't1' }) } as never,
      {} as never
    );
    await service.claim('u1', 'i1');
    expect(updateMany).toHaveBeenCalledWith(
      expect.objectContaining({
        where: expect.objectContaining({ id: 'i1', teamId: 't1', status: 'AUTHORIZED' }),
      })
    );
  });
  ...
  it('rejects exact target cycles before authorization', async () => {
    ...
    const authorize = vi.fn();
    const service = new ActionInvocationService(
      { actionInvocation: { findFirst: vi.fn().mockResolvedValue(parent) } } as never,
      { ensureCurrentTeam: vi.fn().mockResolvedValue({ teamId: 't1' }) } as never,
      { authorize } as never
    );
    await expect(
      service.create('u1', { ...input, parent_invocation_id: 'parent-1' })
    ).rejects.toMatchObject({ code: 'action_cycle_detected' });
    expect(authorize).not.toHaveBeenCalled();
  });
  ...
  it('rejects a nested chain deeper than the portable limit', async () => {
    const chain = Array.from({ length: 8 }, (_, index) => ({
      ...
      target: {
        ...
        action_surface_sha256: String(index).padStart(64, '0'),
      },
    }));
    const parent = row({
      id: 'invocation-7',
      status: 'RUNNING',
      principalUserId: 'u1',
      rootInvocationId: 'invocation-0',
      parentInvocationId: 'invocation-6',
      callChain: chain,
    });
    const service = new ActionInvocationService(
      { actionInvocation: { findFirst: vi.fn().mockResolvedValue(parent) } } as never,
    ...
    const nestedTarget = {
      ...
      action_id: 'encode',
      action_surface_sha256: 'f'.repeat(64),
    };
    await expect(
      service.create('u1', { ...input, target: nestedTarget, parent_invocation_id: 'invocation-7' })
    ).rejects.toMatchObject({ code: 'action_depth_exceeded' });
  });
  ...
  it('serializes nested creation at the root and rejects excess active concurrency', async () => {
    const parent = row({
      ...
      parentInvocationId: 'root-1',
      callChain: [
        { invocation_id: 'root-1', target: { ...target, release_id: 'root-release' } },
        { invocation_id: 'parent-1', target },
      ],
    });
    const tx = {
      actionInvocation: {
    ...
    await expect(
      service.create('u1', { ...input, target: nestedTarget, parent_invocation_id: 'parent-1' })
    ).rejects.toMatchObject({ code: 'action_concurrency_exceeded' });
    expect(tx.actionInvocation.updateMany).toHaveBeenCalledWith(
      expect.objectContaining({ where: { id: 'root-1', teamId: 't1' } })
    );
  ...
  it('terminalizes a workflow invocation and its step with one transaction CAS', async () => {
    ...
    const service = new ActionInvocationService(
      ...
      { ensureCurrentTeam: vi.fn().mockResolvedValue({ teamId: 't1' }) } as never,
      {} as never,
      { acquireHandoffPendingTx } as never
    );
    await service.completeWorkflowAttempt('u1', 'i1', 'attempt-1', { value: 'ok' });
    expect(transaction).toHaveBeenCalledOnce();
    expect(acquireHandoffPendingTx).toHaveBeenCalledWith(
      tx,
      expect.objectContaining({
        invocationId: 'i1',
        runId: 'run-1',
    ...
    expect(invocationUpdate).toHaveBeenCalledWith(
      expect.objectContaining({
        where: expect.objectContaining({ id: 'i1', status: 'RUNNING' }),
    ...
    expect(attemptUpdate).toHaveBeenCalledWith(
      expect.objectContaining({
        where: expect.objectContaining({
          id: 'attempt-1',
          actionInvocationId: 'i1',
          status: 'RUNNING',
        }),
        data: expect.objectContaining({ status: 'SUCCEEDED', leaseTokenSha256: null }),
      })
    );
  });
  it('aborts the workflow terminal transaction when either CAS loses', async () => {
    ...
    const service = new ActionInvocationService(
      ...
      { ensureCurrentTeam: vi.fn().mockResolvedValue({ teamId: 't1' }) } as never,
      {} as never,
      { acquireHandoffPendingTx: vi.fn().mockResolvedValue(0) } as never
    );
    await expect(
      service.completeWorkflowAttempt('u1', 'i1', 'attempt-1', {})
