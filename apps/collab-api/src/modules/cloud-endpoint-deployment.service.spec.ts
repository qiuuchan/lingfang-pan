...
import { CloudActionDeploymentService, CLOUD_ENDPOINT_VERIFY_TYPE } from './cloud-action-deployment.service';
import { CloudEndpointSecretCipher } from './cloud-endpoint-secret-cipher';
import { signCloudResponse } from './cloud-signature';

const target = {
  package_id: 'pkg-1', release_id: 'rel-1', sha256: 'a'.repeat(64), action_id: 'image.generate',
  action_contract_version: '1.0.0', action_surface_sha256: 'b'.repeat(64), environment: 'PRODUCTION' as const,
};
...
function harness(options: { cloudCapable?: boolean; teamId?: string } = {}) {
  ...
  const tx: any = {
    ...
    cloudActionDeployment: {
      create: vi.fn(async ({ data }: any) => {
        const row = { supersedesDeploymentId: null, lastHealthAt: null, lastHealthOk: null, lastHealthErrorCode: '', createdAt: now, updatedAt: now, ...data };
        rows.set(row.id, row);
        return row;
      }),
      findFirst: vi.fn(async ({ where }: any) => {
        const candidates = [...rows.values()];
        return candidates.find((row) => (!where.id || row.id === where.id) && (!where.teamId || row.teamId === where.teamId) && (!where.status || row.status === where.status)) ?? null;
      }),
      findMany: vi.fn(async ({ where }: any) => [...rows.values()].filter((row) => row.teamId === where.teamId && row.releaseId === where.releaseId && row.actionId === where.actionId)),
      updateMany: vi.fn(async ({ where, data }: any) => {
        const row = rows.get(where.id);
        const allowed = typeof where.status === 'string' ? row?.status === where.status : where.status?.in?.includes(row?.status);
        if (!row || row.teamId !== where.teamId || !allowed) return { count: 0 };
        Object.assign(row, data, { updatedAt: now });
        return { count: 1 };
      }),
    },
    pluginRelease: { findFirst: vi.fn(async ({ where }: any) => where.package?.ownerTeamId === (options.teamId ?? 'team-1') ? { id: 'rel-1' } : null) },
  ...
  const actions: any = { resolve: vi.fn(async () => ({ action: { cloud_capable: options.cloudCapable ?? true } })) };
    ...
    request: vi.fn(async (request: any) => {
      ...
      return {
        ...
        headers: {
          ...
          'x-lingfang-response-signature': signCloudResponse({ statusCode: 200, timestamp, nonce, invocationId, deploymentId, body: responseBody }, responseSecret),
        },
        body: responseBody,
  ...
  return { service, prisma, auth, actions, http, rows, setResponseSecret: (value: string) => { responseSecret = value; } };
}
...
describe('CloudActionDeploymentService', () => {
  it('creates DRAFT without network, returns the secret once, and exposes only a host', async () => {
    const h = harness();
    const result = await h.service.create('user-1', { target, deployment_key: 'primary', endpoint_url: 'https://api.example.com/v1/action?mode=cloud' });
    expect(result.deployment).toMatchObject({ status: 'DRAFT', endpoint_host: 'api.example.com', target });
  ...
  it('verifies a signed exact-target challenge before transitioning to READY', async () => {
    const h = harness();
    const created = await h.service.create('user-1', { target, deployment_key: 'primary', endpoint_url: 'https://api.example.com/v1/action' });
    ...
    expect(JSON.parse(request.body.toString('utf8'))).toMatchObject({ type: CLOUD_ENDPOINT_VERIFY_TYPE, deployment_id: created.deployment.id, target });
  ...
  it('rotates into a superseding DRAFT while leaving the READY deployment intact, then disables only the owned row', async () => {
    const h = harness();
    const created = await h.service.create('user-1', { target, deployment_key: 'primary', endpoint_url: 'https://api.example.com/action' });
    h.setResponseSecret(created.endpoint_secret);
    ...
    expect(rotated.deployment).toMatchObject({ status: 'DRAFT', supersedes_deployment_id: created.deployment.id, secret_version: 2 });
  ...
  it('rejects non-cloud-capable exact actions before writing a deployment', async () => {
    ...
    await expect(h.service.create('user-1', { target, deployment_key: 'primary', endpoint_url: 'https://api.example.com/action' })).rejects.toMatchObject({ code: 'cloud_endpoint_target_mismatch' });
  ...
  it('does not expose another team deployment by id', async () => {
    ...
    await expect(h.service.disable('user-2', created.deployment.id)).rejects.toMatchObject({ code: 'not_found' });
  });
  ...
  it('retires only after routing and nonterminal execution bindings are closed', async () => {
    ...
    await expect(h.service.retire('user-1', created.deployment.id)).rejects.toMatchObject({ status: 409, code: 'cloud_deployment_in_use' });
    h.prisma.actionInvocation.count.mockResolvedValueOnce(1);
    await expect(h.service.retire('user-1', created.deployment.id)).rejects.toMatchObject({ code: 'cloud_deployment_in_use' });
    ...
    await expect(h.service.retire('user-1', created.deployment.id)).resolves.toMatchObject({ deployment: { status: 'RETIRED' } });
  });
});
