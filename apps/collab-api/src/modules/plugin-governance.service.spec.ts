...
function policy(operations: Array<'invoke_action' | 'run_workflow'> = ['invoke_action', 'run_workflow']) {
  return {
...
describe('PluginGovernanceService', () => {
  it('loads release, entitlement, grants, and policy once for a compound requiredOperations set', async () => {
    const h = harness();
    const result = await h.service.authorizeRelease('member-1', {
  ...
  it.each([
    ...
    await expect(h.service.authorizeRelease(userId, {
      releaseId: release.id,
      packageId: release.packageId,
      sha256: release.sha256,
    }, ['invoke_action'])).rejects.toMatchObject({ code: 'forbidden' });
    expect(h.prisma.pluginGrant.findMany).toHaveBeenCalledOnce();
