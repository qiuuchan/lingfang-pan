...
describe('CloudExecutionQuotaService', () => {
  const config = resolveAutomationConfig({
    ...
    AUTOMATION_TEAM_MAX_ACTIVE_RUNS: '2',
    AUTOMATION_ACTION_MAX_ACTIVE_INVOCATIONS: '3',
    AUTOMATION_TEAM_MAX_USAGE_PER_MINUTE: '5',
    AUTOMATION_ACTION_MAX_USAGE_PER_MINUTE: '2',
  });

  it('rejects team/action usage with the stable cloud_quota_exceeded code', async () => {
    const prisma = {
      actionInvocation: { count: vi.fn().mockResolvedValueOnce(1).mockResolvedValueOnce(1) },
      cloudUsageEvent: { count: vi.fn().mockResolvedValueOnce(3).mockResolvedValueOnce(2) },
    } as any;
    const service = new CloudExecutionQuotaService(prisma, config, {} as any);
    await expect(
      service.assertInvocationQuota({ id: 'i1', teamId: 't1', releaseId: 'r1', actionId: 'a1' })
  ...
  it('uses one atomic Redis acquisition and releases the endpoint lease', async () => {
    ...
    const release = await service.acquireEndpoint(
      { id: 'd1', maxConcurrency: 2, rateLimitPerMinute: 10, timeoutMs: 30_000 },
      'i1'
    );
    expect(redis.eval).toHaveBeenCalledWith(
      expect.stringContaining('ZREMRANGEBYSCORE'),
      2,
      expect.stringContaining('d1:leases'),
      expect.stringContaining('d1:minute:'),
      expect.any(Number),
      expect.any(Number),
      2,
      10,
