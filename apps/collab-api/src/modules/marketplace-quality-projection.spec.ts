...
import {
  canProjectQualitySnapshot,
  isNewerQualitySnapshot,
  marketplaceQualityGateDigest,
  projectMarketplaceQualityGateTx,
} from './marketplace-quality-projection';
...
describe('marketplace quality projection CAS', () => {
  ...
  it('orders snapshots by watermark then computation revision', () => {
    ...
    expect(
      isNewerQualitySnapshot(
        { factWatermark: new Date('2026-07-15T00:00:00Z'), computationRevision: 99n },
        current
      )
    ).toBe(false);
  });
  ...
  it('rejects a late job after release, epoch or gate facts change', () => {
    const base = {
      expectedReleaseId: 'rel-1',
      expectedPointerRevision: 2,
      expectedEligibilityRevision: 3,
      expectedGateDigest: 'a'.repeat(64),
      currentReleaseId: 'rel-1',
      currentPointerRevision: 2,
      currentEligibilityRevision: 3,
      currentGateDigest: 'a'.repeat(64),
  ...
  it('atomically starts pointer activation and both eligibility epochs for a newly approved listing', async () => {
    ...
    const tx: any = {
      ...
      marketplaceListingEligibilityEpoch: {
    ...
    expect(listing).toMatchObject({
      currentReleaseActivatedAt: now,
      listingEligibleSince: now,
      releaseEligibleSince: now,
      pointerRevision: 1,
      eligibilityRevision: 1,
    });
    expect(activationCreate).toHaveBeenCalledWith({
  ...
  it('preserves the listing epoch but resets the release epoch when the current pointer changes', async () => {
    ...
    let listing: any = {
      ...
      currentReleaseActivatedAt: new Date('2026-06-10T00:00:00Z'),
      pointerRevision: 2,
      eligibilityRevision: 4,
      eligibilityGateDigest: 'a'.repeat(64),
      listingEligibleSince: listingSince,
      releaseEligibleSince: new Date('2026-06-10T00:00:00Z'),
      qualityTier: 'QUALITY',
    ...
    const tx: any = {
      ...
      marketplaceListingReleaseActivation: {
        findFirst: vi.fn(async () => ({ releaseId: 'release-1', pointerRevision: 2 })),
    ...
    expect(epochUpdate).toHaveBeenCalledWith(
      expect.objectContaining({ where: expect.objectContaining({ kind: 'RELEASE' }) })
    );
  ...
  it('closes both epochs and clears the active quality projection when a hard gate fails', async () => {
    ...
    const tx: any = {
      ...
      marketplaceListingReleaseActivation: {
        findFirst: vi.fn(async () => ({ releaseId: 'release-1', pointerRevision: 1 })),
        create: vi.fn(),
      },
