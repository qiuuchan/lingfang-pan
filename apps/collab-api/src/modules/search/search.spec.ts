...
describe('normalizeUrl', () => {
  it('去尾斜杠 + 去 hash + 去追踪参数 + host 小写', () => {
    expect(normalizeUrl('https://Example.com/a/?utm_source=x&q=1#frag')).toBe(
...
describe('provider 结果归一化', () => {
  ...
  it('SearxngProvider 解析 results[] → {title,url,snippet,source}，过滤无 url/title', async () => {
    ...
    const items = await new SearxngProvider('https://s.example', 'self').search(
      'q',
      8,
      new AbortController().signal
    );
    expect(items).toEqual([
      { title: 'T1', url: 'https://a.com', snippet: 'c1', source: 'searxng:self' },
    ]);
  });
  ...
  it('SearxngProvider 非 2xx 抛错', async () => {
    ...
    await expect(
      new SearxngProvider('https://s.example').search('q', 8, new AbortController().signal)
    ).rejects.toThrow('502');
  });
});
...
describe('BingHtmlProvider', () => {
  ...
  it('limit 截断生效', () => {
    const html = [
      '<ol><li class="b_algo"><h2><a href="https://a.com">A</a></h2></li>',
      '<li class="b_algo"><h2><a href="https://b.com">B</a></h2></li></ol>',
    ].join('');
  ...
  it('解码 HTML 实体与去标签（标题/摘要含 <b>、&amp; 等）', () => {
    const html =
      '<li class="b_algo"><h2><a href="https://x.com">A &amp; B <b>bold</b></a></h2><p>cat &amp; dog</p></li>';
    const items = parseBingHtml(html, 5, 'bing');
  ...
  it('fetch 成功时返回解析结果', async () => {
    vi.stubGlobal(
      ...
      vi.fn(
        async () =>
          new Response('<li class="b_algo"><h2><a href="https://r.com">R</a></h2><p>s</p></li>', {
            status: 200,
  ...
  it('fetch 非 2xx 抛错', async () => {
    ...
    await expect(
      new BingHtmlProvider().search('q', 8, new AbortController().signal)
    ).rejects.toThrow('503');
  });
});
...
function makeService(settings: Record<string, string>) {
  ...
  const prisma = {
    platformSetting: {
      findMany: vi.fn(async () => Object.entries(settings).map(([key, value]) => ({ key, value }))),
    },
  };
  const cache = {
...
describe('SearchService 聚合容错', () => {
  ...
  it('部分源失败仍返回其余源结果，并把失败源标记不健康', async () => {
    ...
    vi.stubGlobal(
      ...
      vi.fn(async (url: string) => {
        ...
        // searxng（自建或公共）都返回一条
        return new Response(
          JSON.stringify({ results: [{ title: 'Hit', url: 'https://hit.com', content: 's' }] }),
          { status: 200 }
        );
      })
    );
  ...
  it('结果命中缓存时直接复用', async () => {
    ...
    const fetchMock = vi.fn(
      async () =>
        new Response(
          JSON.stringify({ results: [{ title: 'C', url: 'https://c.com', content: '' }] }),
          { status: 200 }
        )
    );
...
describe('decodeBingUrl', () => {
  ...
  it('/search?url= 形态解出直链', () => {
    expect(decodeBingUrl('https://www.bing.com/search?q=x&url=https%3A%2F%2Freal.com%2Fp')).toBe(
      'https://real.com/p'
    );
  });
  it('/ck/a?...&u=a1<base64url> 形态解出真实 URL', () => {
    ...
    expect(decodeBingUrl(`https://www.bing.com/ck/a?u=${u}`)).toBe(
...
describe('GitHubProvider', () => {
  ...
  it('解析 GitHub Search API 响应 → {title,url,snippet,source}', async () => {
      ...
      items: [
        {
          ...
          description: 'A feature-rich downloader',
    ...
    vi.stubGlobal(
      ...
      vi.fn(
        async () =>
          new Response(JSON.stringify(apiResp), {
            status: 200,
  ...
  it('403 限速抛错（让上层跳过回落其它源）', async () => {
    ...
    await expect(new GitHubProvider().search('x', 8, new AbortController().signal)).rejects.toThrow(
      '403'
    );
  });
});
