...
export class BillingController {
  ...
  listPools(@Query() query: BillingPageQuery) {
    return this.pools.adminList(query);
  }
  ...
  updatePool(@Req() req: Request, @Param('id') id: string, @Body() body: PoolUpdateDto) {
    return this.pools.adminUpdate(requireUser(req).id, id, {
      name: body.name,
  ...
  listChannels(
    @Query('kind') kind: 'CHAT' | 'IMAGE' | undefined,
    @Query() query: BillingPageQuery
  ) {
  ...
  channelDetail(@Param('id') id: string) {
    return this.channels.adminDetail(id);
  }
  ...
  createChannel(@Req() req: Request, @Body() body: ChannelUpsertDto) {
    return this.channels.adminCreate(requireUser(req).id, {
      ...body,
  ...
  updateChannel(@Req() req: Request, @Param('id') id: string, @Body() body: ChannelUpsertDto) {
    return this.channels.adminUpdate(requireUser(req).id, id, {
      ...body,
      upstreamKey: body.upstreamKey ?? '',
    });
  }
  ...
  async listPricing(
    @Query() query: BillingPageQuery & { capability?: string; tier?: 'FAST' | 'PREMIUM' }
  ) {
    ...
    const [items, total] = await this.prisma.$transaction([
      this.prisma.modelPricing.findMany({
        where,
        orderBy: [{ capability: 'asc' }, { model: 'asc' }],
        skip,
        take: pageSize,
      }),
  ...
  async upsertPricing(@Req() req: Request, @Body() body: PricingUpsertDto) {
    ...
    await this.audit(requireUser(req).id, 'admin.pricing.upserted', row.id, {
  ...
  async updatePricing(
    @Req() req: Request,
    @Param('id') id: string,
    @Body() body: PricingUpsertDto
  ) {
    const existing = await this.prisma.modelPricing.findUnique({
      where: { id },
      select: { id: true },
    });
    ...
    try {
      ...
      await this.audit(requireUser(req).id, 'admin.pricing.updated', row.id, {
        capability: row.capability,
        model: row.model,
        tier: row.tier,
        pricePerUnit: row.pricePerUnit,
      ...
      if ((error as { code?: string }).code === 'P2002')
        throw new AppError(409, 'conflict', '同能力、模型与版本的定价已存在');
      throw error;
    }
  }
  ...
  async creditTeams(@Query() query: BillingPageQuery & { status?: 'ACTIVE' | 'SUSPENDED' }) {
    ...
    return {
      items: rows.map((row) => ({
        ...
        slug: row.slug,
        status: row.status,
        balance: row.creditAccount?.balance ?? 0,
  ...
  async adjustCredits(
    ...
    await this.audit(requireUser(req).id, 'admin.credit.adjusted', teamId, {
      amount: body.amount,
      direction: body.direction,
      reason: body.reason,
      balance,
  ...
  async callLogDetail(@Param('id') id: string) {
    const log = await this.prisma.llmCallLog.findUnique({
      where: { id },
      include: {
        team: { select: { name: true } },
        user: { select: { email: true } },
