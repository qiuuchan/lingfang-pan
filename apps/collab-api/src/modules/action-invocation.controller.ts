...
export class ActionInvocationController {
  ...
  async create(@Req() req: Request, @Body() body: any) {
    ...
    const dependencyId =
      typeof caller?.dependency_id === 'string' ? caller.dependency_id.trim() : '';
    ...
    const parentInvocationId =
      typeof body?.parent_invocation_id === 'string' ? body.parent_invocation_id.trim() : '';
    const exactCaller = parentInvocationId
      ? await this.invocations.nestedCaller(user.id, parentInvocationId)
      : caller;
    if (
      !exactCaller ||
      typeof exactCaller.package_id !== 'string' ||
      typeof exactCaller.release_id !== 'string' ||
      !/^[a-f0-9]{64}$/.test(String(exactCaller.sha256 || ''))
    )
      throw badRequest('Desktop Action caller identity 无效');
    await this.actions.assertDeclaredDependency(exactCaller, dependencyId, target.data);
    const callerId =
      parentInvocationId ||
      `desktop-plugin:${createHash('sha256')
        .update(
          JSON.stringify({
            package_id: exactCaller.package_id,
            release_id: exactCaller.release_id,
            sha256: exactCaller.sha256,
          })
        )
        .digest('hex')}`;
    const {
      desktop_caller: _ignored,
      caller: _untrustedCaller,
      preview: _untrustedPreview,
      parent_invocation_id: _untrustedParent,
      ...input
    } = body ?? {};
    return this.invocations.create(user.id, {
      ...input,
      target: target.data,
      caller: parentInvocationId
        ? { kind: 'ACTION', id: callerId }
        : { kind: 'DESKTOP', id: callerId },
  ...
  complete(@Req() req: Request, @Param('id') id: string, @Body() body: { output?: unknown }) {
    if (!body?.output || typeof body.output !== 'object' || Array.isArray(body.output))
      throw badRequest('Action output 必须是 JSON 对象');
    return this.invocations.completeDesktop(
      requireUser(req).id,
      id,
  ...
  fail(
    ...
    if (!ActionErrorCode.safeParse(code).success || !message)
      throw badRequest('Action failure code/message 无效');
