...
function harness(attempt = row()) {
  const prisma: any = {
    workflowStepAttempt: {
      findUnique: vi.fn(async ({ select }: any) => (select ? { status: attempt.status } : attempt)),
    },
  };
  const invocations: any = {
    claim: vi.fn(async () => ({})),
    completeCloudWorkflowAttempt: vi.fn(async () => ({ ok: true })),
    failCloudWorkflowAttempt: vi.fn(async () => ({ ok: true })),
    cancelCloudWorkflowAttempt: vi.fn(async () => ({ ok: true })),
  };
  const gateway: any = {
    invoke: vi.fn(async () => ({
      output: { ok: true },
  ...
  return {
    processor: new CloudActionWorkerProcessor(prisma, invocations, gateway),
    prisma,
    invocations,
    gateway,
  };
...
describe('CloudActionWorkerProcessor', () => {
  it('claims through ActionInvocationService, invokes only the gateway, then settles success', async () => {
    const h = harness();
    await expect(h.processor.process(job)).resolves.toEqual({
      outcome: 'SUCCEEDED',
      attempt_id: 'attempt-1',
    });
    expect(h.invocations.claim).toHaveBeenCalledWith('user-1', 'invocation-1');
    expect(h.gateway.invoke).toHaveBeenCalledTimes(1);
    expect(h.invocations.completeCloudWorkflowAttempt).toHaveBeenCalledWith(
      'user-1',
      'invocation-1',
      'attempt-1',
      { ok: true },
      { requestBytes: 12, responseBytes: 8, endpointHttpStatus: 200 }
    );
  });
  ...
  it('marks uncertain side effects failed without any automatic retry or second gateway call', async () => {
    ...
    expect(h.gateway.invoke).toHaveBeenCalledTimes(1);
    expect(h.invocations.failCloudWorkflowAttempt).toHaveBeenCalledWith(
      'user-1',
      'invocation-1',
      'attempt-1',
      expect.objectContaining({ code: 'cloud_result_unknown', outcome: 'RESULT_UNKNOWN' })
    );
  ...
  it('rejects tampered job relationships before claim or gateway execution', async () => {
    const h = harness();
    await expect(
      h.processor.process({ ...job, plan_sha256: 'b'.repeat(64) })
