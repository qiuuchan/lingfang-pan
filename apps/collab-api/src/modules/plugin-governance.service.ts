...
export class PluginGovernanceService {
  ...
  async authorizeRelease(
    userId: string,
    expected: { releaseId: string; packageId?: string; sha256?: string },
    requiredOperations: Operation[],
    options: {
      enforce?: boolean;
      policyOverride?: PluginGovernanceFacts['policy'];
      action?: {
        action_id: string;
    ...
    release: NonNullable<Awaited<ReturnType<PluginGovernanceService['loadRelease']>>>;
  }> {
    ...
    if (
      (expected.packageId && release.packageId !== expected.packageId) ||
      (expected.sha256 && release.sha256 !== expected.sha256)
    )
      throw new AppError(
        409,
        'plugin_release_mismatch',
        '本机插件版本与平台发行版不一致，请重新下载'
      );
    const source =
      release.package.ownerTeamId === membership.teamId
        ? ('team' as const)
        : ('marketplace' as const);
    const entitlementAllowed =
      source === 'team' ||
      (release.package.listing?.status === 'ACTIVE' && release.package.listing.priceCents === 0) ||
            ...
            where: {
              ...
              OR: [
                ...
                ...(membership.teamRoleId
                  ? [{ subjectKind: 'ROLE' as const, subjectId: membership.teamRoleId }]
                  : []),
              ],
            },
    ...
    const decision = evaluatePluginGovernance({
      resource: {
        ...
        runtime_type: typeof manifest.runtime_type === 'string' ? manifest.runtime_type : 'client',
        package_policy_surface_sha256: release.packagePolicySurfaceSha256,
      ...
      policy:
        options.policyOverride === undefined
          ? (active.document as PluginGovernanceFacts['policy'])
          : options.policyOverride,
      platformAllowed:
        release.status === 'PUBLISHED' &&
        release.aiPolicyVersion === PLUGIN_AI_POLICY_VERSION &&
        release.aiPolicyStatus === 'PASSED' &&
        (source === 'team' || release.marketReviewStatus === 'APPROVED'),
      entitlementAllowed,
      userGrant: grantEffect('USER'),
      roleGrant: grantEffect('ROLE'),
    });
    if (!decision.allowed && options.enforce !== false) {
      await this.prisma.auditLog.create({
        data: {
          actorUserId: userId,
          action: 'plugin.policy.decision_denied',
          targetType: 'PluginRelease',
          targetId: release.id,
          metadata: decision as object,
        },
      });
      ...
      if (
        ...
        throw new AppError(
          409,
          'plugin_ai_policy_required',
          '插件发行版尚未通过当前 AI 使用政策检查'
        );
  ...
  private loadRelease(releaseId: string) {
    return this.prisma.pluginRelease.findUnique({
      where: { id: releaseId },
