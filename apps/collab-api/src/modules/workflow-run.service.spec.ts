...
const plan = {
  ...
  output_bindings: [],
  desktop_executor: { session_id: 's1', inventory_sha256: 'e'.repeat(64) },
};
const runRow = (attempts: unknown[] = []) => ({
  id: 'run1',
  teamId: 't1',
  principalUserId: 'u1',
  workflowReleaseId: 'wr1',
  executionScope: 'PRODUCTION',
  executionTarget: 'DESKTOP',
  status: 'RUNNING',
  requestScopeSha256: 'f'.repeat(64),
  idempotencyKey: 'k1',
  requestDigest: '1'.repeat(64),
  inputDigest: '2'.repeat(64),
  rootLogicalExecutionId: 'run1',
...
describe('WorkflowRunService', () => {
  it('authorizes root once and creates image READY with video/music pending', async () => {
    ...
    const findFirst = vi.fn().mockResolvedValue(runRow());
    const prisma = {
      workflowRelease: {
      ...
      workflowRun: { findUnique: vi.fn().mockResolvedValue(null), create, findFirst },
      $transaction: vi.fn(async (fn: any) =>
        fn({
          workflowRun: { create },
          automationOutbox: { create: vi.fn() },
          automationSchedule: { updateMany: vi.fn() },
        })
      ),
    };
    ...
    const service = new WorkflowRunService(
      ...
      { authorizeRelease } as never,
      {} as never,
      {
        validate: vi.fn().mockResolvedValue({ id: 's1', inventorySha256: 'd'.repeat(64) }),
      } as never
    );
    ...
    const attempts = create.mock.calls[0][0].data.attempts.create as Array<{
      nodeId: string;
      status: string;
    }>;
    expect(attempts.map((item) => [item.nodeId, item.status])).toEqual([
      ['image', 'READY'],
      ['video', 'PENDING'],
      ['music', 'PENDING'],
    ]);
  ...
  it('creates a new attempt for a retryable read-only failure', async () => {
    const attempt = {
      ...
      run: { id: 'run1', status: 'RUNNING', rootLogicalExecutionId: 'root1' },
    };
    ...
    const prisma = {
      ...
      workflowRun: { findFirst: vi.fn().mockResolvedValue(runRow()) },
    };
    const service = new WorkflowRunService(
      prisma as never,
      { ensureCurrentTeam: vi.fn().mockResolvedValue({ teamId: 't1' }) } as never,
      {} as never,
      { failWorkflowAttempt: vi.fn().mockResolvedValue({}) } as never,
      {} as never
    );
    await service.failStep('u1', 'a1', 'action_execution_failed', 'failed');
    expect(create).toHaveBeenCalledWith({
      data: expect.objectContaining({ attempt: 1, status: 'READY', requestKey: 'run1:image:1' }),
    });
  });
  it('lists team runs through the snake_case summary serializer', async () => {
    const findMany = vi.fn().mockResolvedValue([runRow()]);
    const service = new WorkflowRunService(
      { workflowRun: { findMany } } as never,
      { ensureCurrentTeam: vi.fn().mockResolvedValue({ teamId: 't1' }) } as never,
      {} as never,
      {} as never,
      {} as never
    ...
    expect(findMany).toHaveBeenCalledWith(
      expect.objectContaining({ where: { teamId: 't1', status: 'RUNNING' }, take: 21 })
    );
    expect(result).toEqual({
      runs: [
        expect.objectContaining({
          ...
          workflow_release_id: 'wr1',
          execution_target: 'DESKTOP',
          attempt_counts: expect.any(Object),
  ...
  it('returns a non-writing preflight plan and stable diagnostics', async () => {
    ...
    const authorizeRelease = vi.fn().mockResolvedValue({ decision: { policy_revision: 2 } });
    const service = new WorkflowRunService(
      { workflowRelease: { findUnique: vi.fn().mockResolvedValue(workflow) } } as never,
      { ensureCurrentTeam: vi.fn().mockResolvedValue({ teamId: 't1' }) } as never,
      { authorizeRelease } as never,
      {} as never,
      {} as never
    );
    const result = await service.preflight('u1', {
      workflow_release_id: 'wr1',
      sha256: release.sha256,
  ...
  it('fires a schedule with one compound policy decision and atomically writes run plus outbox', async () => {
    const create = vi.fn().mockResolvedValue({ id: 'scheduled-run' });
    ...
    const prisma = {
      workflowRelease: { findUnique: vi.fn().mockResolvedValue(workflow) },
      workflowRun: {
      ...
      $transaction: vi.fn(async (fn: any) =>
        fn({
          workflowRun: { create },
          automationOutbox: { create: outboxCreate },
          automationSchedule: { updateMany: vi.fn().mockResolvedValue({ count: 1 }) },
        })
      ),
    };
    const authorizeRelease = vi.fn().mockResolvedValue({ decision: { policy_revision: 2 } });
    const cloudRouting = {
      freeze: vi.fn(
        async (
          ...
          _run: string,
          node: string
        ) => ({ node_path: node, deployment_id: `dep-${node}`, routing_generation: 1, environment })
      ),
    };
    const service = new WorkflowRunService(
      prisma as never,
      ...
      {} as never,
      {} as never,
      cloudRouting as never
    );
    await service.startScheduled(
      'u1',
      {
        id: 'schedule-1',
        teamId: 't1',
        generation: 4,
        workflowReleaseId: 'wr1',
        workflowReleaseSha256: release.sha256,
        inputJson: {},
        kind: 'DAILY',
      },
    ...
    expect(authorizeRelease.mock.calls[0][2]).toEqual([
      'execute_cloud',
      'run_workflow',
      'trigger_schedule',
    ]);
    ...
    expect(outboxCreate).toHaveBeenCalledWith({
      data: expect.objectContaining({ kind: 'ENQUEUE_RUN', generation: 4 }),
    });
  });
  it('creates a nested child from the immutable subplan and inherits trusted execution context', async () => {
    ...
    const subplan = {
      workflow_release_id: 'child-workflow',
      workflow_release_sha256: '9'.repeat(64),
    ...
    const service = new WorkflowRunService(
      { workflowRun: { findUnique: vi.fn().mockResolvedValue(null), create } } as never,
      {} as never,
      {} as never,
      {} as never,
      {} as never
    ...
    const parent = {
      ...runRow(),
      frozenPlan: rootPlan,
      rootLogicalExecutionId: 'root-logical',
      authorizationDecision: { inherited: true },
      cloudBindings: [],
    };

    const childRunId = await (service as any).createChildRun(
      parent,
      parentAttempt,
      { prompt: 'demo' },
      subplan,
      ...
      data: expect.objectContaining({
        ...
        attempts: {
          create: [
            expect.objectContaining({
              nodeId: 'leaf',
              fullNodePath: 'child/leaf',
              effectKey: 'root-logical:child/leaf',
            }),
          ],
        },
      }),
