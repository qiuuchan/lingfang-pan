...
export type CloudActionWorkerResult = {
  outcome: 'SUCCEEDED' | 'FAILED' | 'CANCELED' | 'TIMED_OUT' | 'RESULT_UNKNOWN' | 'STALE';
  attempt_id: string;
};
...
export class CloudActionWorkerProcessor {
  ...
  async process(
    job: Extract<CloudActionJob['data'], { run_id: string }>,
    signal?: AbortSignal
  ): Promise<CloudActionWorkerResult> {
    ...
    if (
      attempt.runId !== job.run_id ||
      attempt.actionInvocationId !== job.invocation_id ||
      attempt.run.planSha256 !== job.plan_sha256 ||
      attempt.run.executionTarget !== 'CLOUD'
    )
      throw new AppError(
        409,
        'cloud_endpoint_target_mismatch',
        'Cloud action job 与冻结运行不匹配'
      );
    if (!attempt.run.principalUserId)
      throw new AppError(409, 'cloud_endpoint_target_mismatch', 'Cloud workflow 缺少执行主体');
    if (attempt.status !== 'RUNNING') return { outcome: 'STALE', attempt_id: attempt.id };
    const userId = attempt.run.principalUserId;
    if (
      this.config &&
      (!this.config.enabled ||
        (attempt.run.triggerKind === 'SCHEDULE'
          ? !this.config.schedulesEnabled
          : !this.config.cloudManualEnabled))
    ) {
      await this.invocations.failCloudWorkflowAttempt(userId, job.invocation_id, attempt.id, {
        code: 'cloud_disabled',
        message: 'Cloud 自动化已由平台暂停',
        outcome: 'FAILED',
      });
      return { outcome: 'FAILED', attempt_id: attempt.id };
    }
    if (
      attempt.run.status === 'CANCELING' ||
    ...
    if (attempt.run.deadlineAt <= new Date()) {
      await this.invocations.failCloudWorkflowAttempt(userId, job.invocation_id, attempt.id, {
        code: 'cloud_timeout',
        message: 'Cloud action 已超过运行 deadline',
        outcome: 'TIMED_OUT',
      });
    ...
    try {
      ...
      const latest = await this.prisma.workflowStepAttempt.findUnique({
        where: { id: attempt.id },
        select: { status: true },
      });
      if (latest && latest.status !== 'RUNNING')
        return { outcome: 'STALE', attempt_id: attempt.id };
      throw error;
    }
    try {
      const result = await this.gateway.invoke(job.invocation_id, signal);
      await this.invocations.completeCloudWorkflowAttempt(
        userId,
        job.invocation_id,
        attempt.id,
        result.output,
        {
          ...
          endpointHttpStatus: result.endpoint_http_status,
        }
      );
      ...
      const appError =
        error instanceof AppError
          ? error
          : new AppError(502, 'cloud_endpoint_unavailable', 'Cloud endpoint 执行失败');
      if (appError.code === 'cloud_request_cancelled' || signal?.aborted) {
        await this.invocations.cancelCloudWorkflowAttempt(
          userId,
          job.invocation_id,
          attempt.id,
          appError.message
      ...
      const resultUnknown =
        attempt.executionSemantics === 'side_effect' && UNCERTAIN_DELIVERY_CODES.has(appError.code);
      const outcome = resultUnknown
        ? ('RESULT_UNKNOWN' as const)
        : timedOut
          ? ('TIMED_OUT' as const)
          : ('FAILED' as const);
      await this.invocations.failCloudWorkflowAttempt(userId, job.invocation_id, attempt.id, {
        code: resultUnknown ? 'cloud_result_unknown' : appError.code,
        message: resultUnknown
          ? 'side-effect endpoint 可能已收到请求，平台不会自动重试'
          : appError.message,
        outcome,
      });
