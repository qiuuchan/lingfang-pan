...
export interface MarketplaceMetricRepository {
  append(
    fact: MarketplaceMetricFact & { occurredAt: Date }
  ): Promise<{ metric: StoredMarketplaceMetric; created: boolean }>;
}
...
export class PrismaMarketplaceMetricRepository implements MarketplaceMetricRepository {
  ...
  async append(
    ...
    try {
      const create = (client: Prisma.TransactionClient | PrismaService) =>
        client.marketplaceMetricEvent.create({
          data: {
      ...
      if (!(error instanceof Prisma.PrismaClientKnownRequestError) || error.code !== 'P2002')
        throw error;
      const existing = await this.prisma.marketplaceMetricEvent.findUnique({
        where: { idempotencyKey: fact.idempotencyKey },
      });
...
const ALLOWED_SOURCE_BY_KIND: Readonly<
  Record<MarketplaceMetricKind, readonly MarketplaceMetricSource[]>
> = {
...
export class MarketplaceMetricRecorder {
  constructor(
    @Inject(MARKETPLACE_METRIC_REPOSITORY) private readonly repository: MarketplaceMetricRepository
  ) {}

  async record(
    fact: MarketplaceMetricFact,
    now = new Date()
  ): Promise<{ metric: StoredMarketplaceMetric; created: boolean }> {
    validateFact(fact);
...
function sameMetric(metric: StoredMarketplaceMetric, fact: MarketplaceMetricFact): boolean {
  return (
    ...
    metric.source === fact.source &&
    metric.packageId === fact.packageId &&
    metric.releaseId === fact.releaseId &&
    metric.teamId === (fact.teamId ?? null) &&
    metric.sourceRecordId === fact.sourceRecordId &&
...
function validateFact(fact: MarketplaceMetricFact): void {
  for (const [name, value, max] of [
    ['idempotency_key', fact.idempotencyKey, 256],
    ['package_id', fact.packageId, 128],
    ['release_id', fact.releaseId, 128],
    ['source_record_id', fact.sourceRecordId, 256],
  ] as const) {
    if (typeof value !== 'string' || !value.trim() || value.length > max)
      throw new Error(`marketplace_metric_invalid_${name}`);
  }
  if (!ALLOWED_SOURCE_BY_KIND[fact.kind]?.includes(fact.source))
    throw new Error('marketplace_metric_source_mismatch');
  if (fact.value !== undefined && (!Number.isInteger(fact.value) || fact.value < 0))
    throw new Error('marketplace_metric_invalid_value');
  if (
    fact.kind === 'RATING_CHANGED' &&
    (fact.value === undefined || fact.value < 1 || fact.value > 5)
  )
    throw new Error('marketplace_metric_invalid_rating');
  if (fact.kind !== 'RATING_CHANGED' && fact.value !== undefined)
    throw new Error('marketplace_metric_unexpected_value');
}
