...
function order(overrides: Record<string, unknown> = {}) {
  return {
    ...
    priceCents: 90,
    platformFeeBps: 2000,
    platformAmountCents: 18,
    sellerAmountCents: 72,
    settlementVersion: 'SETTLEMENT_V2',
    priceVersion: `pv1.${'a'.repeat(43)}`,
    campaignId: null,
    attributionKind: 'ORGANIC',
    status: 'PENDING_SETTLEMENT',
    createdAt: now,
    settleAt: new Date('2026-07-23T00:00:00.000Z'),
    refundableUntil: new Date('2026-07-23T00:00:00.000Z'),
    settledAt: null,
    refundedAt: null,
    package: { name: '图片插件' },
...
function service(prismaOverrides: Record<string, unknown> = {}) {
  ...
  const auth = {
    ensureCurrentTeam: vi.fn().mockResolvedValue({ teamId: 'team-1' }),
    ensurePlatformAdmin: vi.fn().mockResolvedValue(undefined),
  };
  return {
    queries: new MarketplaceCommerceQueryService(prisma as never, auth as never),
    prisma,
    auth,
  };
}
...
describe('MarketplaceCommerceQueryService', () => {
  it('lists only the current buyer team and projects refund-safe order fields', async () => {
    ...
    expect(purchase.findMany).toHaveBeenCalledWith(
      expect.objectContaining({ where: { buyerTeamId: 'team-1' }, skip: 0, take: 10 })
    );
  ...
  it('builds seller-scoped statement totals from the same frozen order filter', async () => {
    const purchase = {
      ...
      aggregate: vi
        ...
        .mockResolvedValueOnce({
          _count: { _all: 1 },
          _sum: { priceCents: 90, sellerAmountCents: 72 },
        })
        .mockResolvedValue({
          _count: { _all: 0 },
          _sum: { priceCents: null, sellerAmountCents: null },
        }),
    };
    const { queries } = service({ purchase });
    const result = await queries.sellerStatement('seller-user', {
      from: '2026-07-01T00:00:00.000Z',
      to: '2026-08-01T00:00:00.000Z',
      timezone: 'Asia/Shanghai',
    });
    expect(purchase.findMany).toHaveBeenCalledWith(
      expect.objectContaining({ where: expect.objectContaining({ sellerTeamId: 'team-1' }) })
    );
    expect(result.summary).toMatchObject({
      ...
      platform_cents: 18,
      seller_cents: 72,
      by_status: { PENDING_SETTLEMENT: { count: 1 } },
    });
  });
  ...
  it('groups creation, settlement and refund instants by the requested IANA timezone', async () => {
    ...
    const { queries } = service({ purchase });
    const result = await queries.sellerStatementDaily('seller-user', {
  ...
  it('admin refund list is paged and requires the platform-admin boundary', async () => {
    ...
    const marketplaceRefundRequest = {
      findMany: vi.fn().mockResolvedValue([refund]),
      count: vi.fn().mockResolvedValue(1),
      findUnique: vi.fn(),
    };
    const { queries, auth } = service({ marketplaceRefundRequest });
    const result = await queries.adminRefundRequests('admin-user', {
      status: 'PENDING',
      page: 2,
      pageSize: 5,
    });
    expect(auth.ensurePlatformAdmin).toHaveBeenCalledWith('admin-user');
    expect(marketplaceRefundRequest.findMany).toHaveBeenCalledWith(
      expect.objectContaining({ where: { status: 'PENDING' }, skip: 5, take: 5 })
    );
    expect(result.items[0]).toMatchObject({
      id: 'refund-1',
      buyer_team_name: '买家团队',
  ...
  it('campaign report counts attributed, refunded and net orders per package', async () => {
    const marketplaceCampaign = {
      findUnique: vi.fn().mockResolvedValue({
        ...
        canceledAt: null,
        items: [
          { id: 'campaign-item-1', packageId: 'package-1', rank: 0, package: { name: '图片插件' } },
        ],
      }),
    };
    const purchase = {
