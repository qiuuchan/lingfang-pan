...
function harness() {
  ...
  const routing = {
    findUnique: vi.fn(async ({ where, include }: any) => {
      ...
      return {
        ...found,
        stableDeployment: deployments.get(found.stableDeploymentId),
        candidateDeployment: found.candidateDeploymentId
          ? deployments.get(found.candidateDeploymentId)
          : null,
      };
    }),
    create: vi.fn(
      async ({ data }: any) => (route = { id: 'route-1', createdAt: now, updatedAt: now, ...data })
    ),
    updateMany: vi.fn(async ({ where, data }: any) => {
      if (!route || route.id !== where.id || route.generation !== where.generation)
        return { count: 0 };
  ...
  const prisma: any = {
    ...
    cloudActionDeployment: {
      findFirst: vi.fn(async ({ where }: any) => {
        ...
        return row &&
          ...
          row.actionContractVersion === where.actionContractVersion &&
          row.actionSurfaceSha256 === where.actionSurfaceSha256 &&
    ...
    pluginRelease: {
      findFirst: vi.fn(async ({ where }: any) =>
        where.package?.ownerTeamId ? { id: 'rel-1' } : { package: { ownerTeamId: 'author-team' } }
      ),
    },
  };
  const auth: any = {
    ensurePermission: vi.fn(async () => ({})),
    ensureCurrentTeam: vi.fn(async () => ({ teamId: 'author-team' })),
  };
...
describe('CloudActionRoutingService', () => {
  it('creates generation 1 and updates only with the expected generation', async () => {
    const h = harness();
    const created = await h.service.update('u1', target.release_id, target.action_id, {
    ...
    await expect(
      h.service.update('u1', target.release_id, target.action_id, {
        action_contract_version: target.action_contract_version,
        action_surface_sha256: target.action_surface_sha256,
        environment: 'PRODUCTION',
        stable_deployment_id: 'stable',
    ...
    const rolledBack = await h.service.update('u1', target.release_id, target.action_id, {
      action_contract_version: target.action_contract_version,
      action_surface_sha256: target.action_surface_sha256,
      environment: 'PRODUCTION',
      stable_deployment_id: 'stable',
    ...
    expect(rolledBack.routing).toMatchObject({
      generation: 2,
  ...
  it('freezes a deterministic candidate or stable deployment without following later routing changes', async () => {
    const h = harness();
    await h.service.update('u1', target.release_id, target.action_id, {
      action_contract_version: target.action_contract_version,
      action_surface_sha256: target.action_surface_sha256,
      environment: 'PRODUCTION',
      stable_deployment_id: 'stable',
  ...
  it('rejects cross-team or non-ready deployments when managing routing', async () => {
    ...
    await expect(
      h.service.update('u2', target.release_id, target.action_id, {
        action_contract_version: target.action_contract_version,
        action_surface_sha256: target.action_surface_sha256,
        environment: 'PRODUCTION',
        stable_deployment_id: 'stable',
    ...
    ).rejects.toMatchObject({ code: 'cloud_endpoint_not_ready' });
  });
});
