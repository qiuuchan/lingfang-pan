...
import { RuntimeArtifactService, sharedArtifactRenewalMs, sharedArtifactRetentionCap } from './runtime-artifact.service';
...
export class PluginSharedStateService implements OnModuleInit, OnModuleDestroy {
  ...
  async adminDeleteNamespace(actorId: string, namespaceId: string) {
    ...
    return this.serializable(async (tx) => {
      const namespace = await tx.pluginSharedNamespace.findFirst({ where: { id: namespaceId, teamId: membership.teamId } });
      if (!namespace || namespace.deletedAt) throw sharedNotFound('shared_namespace_not_found', '共享命名空间不存在');
      const updated = await this.clearNamespaceTx(tx, namespace, membership.teamId);
      await tx.auditLog.create({ data: { actorUserId: actorId, action: 'plugin_shared.namespace.deleted', targetType: 'PluginSharedNamespace', targetId: namespace.id, metadata: { teamId: membership.teamId, generation: namespace.generation } } });
  ...
  async adminReactivateNamespace(actorId: string, namespaceId: string, input: unknown) {
    ...
    return this.serializable(async (tx) => {
      ...
      if (!namespace.deletedAt) {
        if (namespace.activeSchemaVersion !== request.active_schema_version) throw new AppError(409, 'shared_namespace_active', '共享命名空间仍处于启用状态，不能切换 schemaVersion');
      ...
      if (!declaration.schemas.some((entry) => entry.schema_version === request.active_schema_version)) {
      ...
      const updated = await tx.pluginSharedNamespace.update({
        ...
        data: { generation: { increment: 1 }, deletedAt: null, activeSchemaVersion: request.active_schema_version, usedBytes: 0 },
      });
      await tx.auditLog.create({ data: { actorUserId: actorId, action: 'plugin_shared.namespace.reactivated', targetType: 'PluginSharedNamespace', targetId: namespace.id, metadata: { teamId: membership.teamId, generation: updated.generation } } });
      return publicNamespaceLifecycle(updated);
    });
  }

  async adminMigrateNamespaceValue(actorId: string, namespaceId: string, rawKey: string, input: unknown) {
    const membership = await this.requireAdmin(actorId);
    ...
    if (collectArtifactRefs(migration.value).length > 0) throw new AppError(403, 'shared_artifact_invocation_required', '含 ArtifactRef 的共享值必须由插件 STANDARD invocation 迁移');
    return this.serializable(async (tx) => {
      const namespace = await tx.pluginSharedNamespace.findFirst({ where: { id: namespaceId, teamId: membership.teamId } });
      if (!namespace || namespace.deletedAt) throw sharedNotFound('shared_namespace_not_found', '共享命名空间不存在');
      const declaration = await this.loadNamespaceDeclarationForAdmin(membership.teamId, namespace);
      const sourceSchema = declaration.schemas.find((entry) => entry.schema_version === migration.source_schema_version)?.schema;
      const targetSchema = declaration.schemas.find((entry) => entry.schema_version === migration.target_schema_version)?.schema;
      if (!sourceSchema || !targetSchema) throw new AppError(409, 'shared_schema_version_unsupported', '迁移源或目标 schemaVersion 未声明');
      assertSharedSchemaValue(targetSchema, migration.value, '迁移目标值不符合声明 schema');
      const current = await tx.pluginSharedValue.findUnique({ where: { namespaceId_key: { namespaceId: namespace.id, key } } });
      assertExpectedRevision(current, migration.expected_revision);
      if (!current) throw sharedNotFound('shared_value_not_found', '共享值不存在');
      if (current.schemaVersion !== migration.source_schema_version) throw new AppError(409, 'shared_schema_migration_source_changed', '共享值源 schemaVersion 已变化', { current_schema_version: current.schemaVersion, current_revision: current.revision.toString(), retryable: true });
      ...
      if (namespace.usedBytes + delta > namespace.quotaBytes) throw new AppError(413, 'shared_namespace_quota_exceeded', '共享命名空间容量超过限制');
      const allocated = await tx.pluginSharedNamespace.update({ where: { id: namespace.id }, data: { usedBytes: { increment: delta }, nextValueRevision: { increment: 1n }, nextChangeCursor: { increment: 1n }, }, select: { generation: true, nextValueRevision: true, nextChangeCursor: true } });
      ...
      await this.artifacts.releaseSharedValueTx(tx, sharedArtifactTarget(namespace.id, namespace.generation, key, current.revision));
      await tx.pluginSharedValueArtifact.deleteMany({ where: { namespaceId: namespace.id, key, valueRevision: current.revision } });
      const row = await tx.pluginSharedValue.update({ where: { id: current.id }, data: { namespaceGeneration: allocated.generation, valueJson: toPrismaJson(migration.value), schemaVersion: migration.target_schema_version, valueBytes: serialized.bytes, revision: allocated.nextValueRevision, createdByUserId: actorId, createdByPackageId: null } });
      await tx.pluginSharedNamespace.update({ where: { id: namespace.id }, data: { activeSchemaVersion: migration.target_schema_version } });
      await tx.sharedStateOutbox.create({ data: { teamId: membership.teamId, namespaceId: namespace.id, namespaceGeneration: allocated.generation, cursor: allocated.nextChangeCursor, key, revision: allocated.nextValueRevision, schemaVersion: migration.target_schema_version, eventKind: 'UPSERT' } });
      await tx.auditLog.create({ data: { actorUserId: actorId, action: 'plugin_shared.value.migrated', targetType: 'PluginSharedValue', targetId: row.id, metadata: { teamId: membership.teamId, namespaceId: namespace.id, key, revision: row.revision.toString(), sourceSchemaVersion: migration.source_schema_version, targetSchemaVersion: migration.target_schema_version } } });
  ...
  private async adminNamespace(actorId: string, namespaceId: string, includeDeleted: boolean) {
    const membership = await this.requireAdmin(actorId);
    const namespace = await this.prisma.pluginSharedNamespace.findFirst({ where: { id: namespaceId, teamId: membership.teamId, ...(includeDeleted ? {} : { deletedAt: null }) } });
    if (!namespace) throw sharedNotFound('shared_namespace_not_found', '共享命名空间不存在');
    return namespace;
  }

  private async loadNamespaceDeclarationForAdmin(teamId: string, namespace: { ownerKind: string; ownerId: string; name: string }) {
    const release = await this.prisma.pluginRelease.findFirst({
      where: {
        ...(namespace.ownerKind === 'WORKFLOW' ? { id: namespace.ownerId } : { packageId: namespace.ownerId }),
        status: 'PUBLISHED',
      },
      select: { manifest: true },
    });
    const manifest = release?.manifest && typeof release.manifest === 'object' && !Array.isArray(release.manifest) ? release.manifest as Record<string, unknown> : null;
    const declarations = Array.isArray(manifest?.shared_namespaces) ? manifest.shared_namespaces : [];
    const parsed = declarations.map((entry) => SharedNamespaceDeclaration.safeParse(entry)).find((entry) => entry.success && entry.data.name === namespace.name);
    if (!parsed?.success) throw new AppError(409, 'shared_schema_version_unsupported', '发行版未声明该共享命名空间 schema');
    return parsed.data;
  }

  private async clearNamespaceTx(tx: Prisma.TransactionClient, namespace: { id: string; generation: number; nextValueRevision: bigint; nextChangeCursor: bigint; teamId: string }, teamId: string) {
    const values = await tx.pluginSharedValue.findMany({ where: { namespaceId: namespace.id, namespaceGeneration: namespace.generation }, orderBy: [{ revision: 'asc' }, { key: 'asc' }] });
    ...
    for (const value of values) {
      await this.artifacts.releaseSharedValueTx(tx, sharedArtifactTarget(namespace.id, namespace.generation, value.key, value.revision));
      revision += 1n;
      cursor += 1n;
      await tx.sharedStateOutbox.create({ data: { teamId, namespaceId: namespace.id, namespaceGeneration: nextGeneration, cursor, key: value.key, revision, schemaVersion: value.schemaVersion, eventKind: 'DELETE' } });
    }
    await tx.pluginSharedValue.deleteMany({ where: { namespaceId: namespace.id } });
    return tx.pluginSharedNamespace.update({ where: { id: namespace.id }, data: { generation: nextGeneration, deletedAt: new Date(), usedBytes: 0, nextValueRevision: revision, nextChangeCursor: cursor } });
  }

  onModuleInit() {
    this.artifactReconcileTimer = setInterval(() => void this.reconcileArtifactRetention().catch(() => undefined), ARTIFACT_RECONCILE_INTERVAL_MS);
  ...
  async resolvePrincipal(userId: string, teamId: string | null, invocationId: string): Promise<SharedInvocationPrincipal> {
  ...
  async get(principal: SharedInvocationPrincipal, locator: SharedNamespaceLocator, rawKey: string) {
    ...
    return this.prisma.$transaction(async (tx) => {
      const namespace = await tx.pluginSharedNamespace.findUnique({ where: { teamId_ownerKind_ownerId_name: namespaceIdentity(principal.teamId, target) } });
      if (!namespace || namespace.deletedAt) throw sharedNotFound('shared_namespace_not_found', '共享命名空间不存在');
      const value = await tx.pluginSharedValue.findUnique({ where: { namespaceId_key: { namespaceId: namespace.id, key } }, include: { artifacts: true } });
      if (!value || value.namespaceGeneration !== namespace.generation) throw sharedNotFound('shared_value_not_found', '共享值不存在');
      assertReadableSchema(namespace.activeSchemaVersion, value.schemaVersion);
      const expiresAt = new Date(Date.now() + 15 * 60 * 1000);
      for (const edge of value.artifacts) {
        const targetId = sharedArtifactTarget(namespace.id, namespace.generation, key, value.revision);
        await this.artifacts.exchangeSharedValueTx(tx, principal.invocationId, edge.artifactId, targetId, sharedArtifactScope(namespace.id, namespace.generation, key, value.revision, edge.artifactId, edge.jsonPointer), expiresAt);
      }
      return publicValue(value, value.artifacts);
    }, { isolationLevel: Prisma.TransactionIsolationLevel.Serializable });
  }

  async set(principal: SharedInvocationPrincipal, locator: SharedNamespaceLocator, rawKey: string, input: unknown) {
    const target = normalizeLocator(locator);
    const key = parseKey(rawKey);
    ...
    return this.serializable((tx) => this.writeTx(tx, principal, target, key, write, artifactRefs, serialized));
  }

  async migrate(principal: SharedInvocationPrincipal, locator: SharedNamespaceLocator, rawKey: string, input: unknown) {
    const target = normalizeLocator(locator);
    const key = parseKey(rawKey);
    const migration = parseMigration(input);
    ...
    if (!sourceSchema || !targetSchema) {
      throw new AppError(409, 'shared_schema_version_unsupported', '迁移源或目标 schemaVersion 未声明');
    }
    assertSharedSchemaValue(targetSchema, migration.value, '迁移目标值不符合声明 schema');
    ...
    return this.serializable(async (tx) => {
      ...
      if (current.schemaVersion !== migration.source_schema_version) {
        throw new AppError(409, 'shared_schema_migration_source_changed', '共享值源 schemaVersion 已变化', {
  ...
  private async writeTx(
    ...
    knownNamespace?: Awaited<ReturnType<PluginSharedStateService['namespaceForWrite']>>,
      ...
      const namespace = knownNamespace ?? await this.namespaceForWrite(tx, principal, target, write.schema_version);
      ...
      for (const item of artifactRefs) {
        const targetId = sharedArtifactTarget(namespace.id, allocated.generation, key, allocated.nextValueRevision);
        const scope = sharedArtifactScope(namespace.id, allocated.generation, key, allocated.nextValueRevision, item.ref.artifact_id, item.pointer);
        await this.artifacts.bindSharedValueTx(tx, principal.invocationId, item.ref, targetId, scope, retainUntil);
        await tx.pluginSharedValueArtifact.create({ data: { namespaceId: namespace.id, namespaceGeneration: allocated.generation, key, valueRevision: allocated.nextValueRevision, artifactId: item.ref.artifact_id, jsonPointer: item.pointer, executionKind: 'STANDARD' } });
      }
  ...
  async delete(principal: SharedInvocationPrincipal, locator: SharedNamespaceLocator, rawKey: string, expectedRevision: unknown) {
    const target = normalizeLocator(locator);
    const key = parseKey(rawKey);
  ...
  async reactivateNamespace(principal: SharedInvocationPrincipal, locator: SharedNamespaceLocator, input: unknown) {
    const target = normalizeLocator(locator);
    const request = parseReactivation(input);
    ...
    return this.serializable(async (tx) => {
      ...
      const existing = await tx.pluginSharedNamespace.findUnique({ where: { teamId_ownerKind_ownerId_name: identity } });
      if (!existing) {
      ...
      if (!existing.deletedAt) {
        if (existing.activeSchemaVersion !== request.active_schema_version) {
          throw new AppError(409, 'shared_namespace_active', '共享命名空间仍处于启用状态，不能通过重建切换 schemaVersion');
        }
  ...
  async exportNamespace(principal: SharedInvocationPrincipal, locator: SharedNamespaceLocator) {
    ...
    return { namespaceId: activeNamespace.id, generation: activeNamespace.generation, lines: lines() };
  }
  ...
  async reconcileArtifactRetention(rawLimit: unknown = 100, now = new Date()) {
    ...
    for (const edge of edges) {
      const targetId = sharedArtifactTarget(edge.namespaceId, edge.namespaceGeneration, edge.key, edge.valueRevision);
      const live = !edge.namespace.deletedAt
      ...
      const retainUntil = new Date(Math.min(cap.getTime(), now.getTime() + sharedArtifactRenewalMs()));
      if (retainUntil <= now) {
        await this.prisma.$transaction((tx) => this.artifacts.releaseSharedValueTx(tx, targetId), {
          isolationLevel: Prisma.TransactionIsolationLevel.Serializable,
        });
      ...
      try {
        await this.prisma.$transaction((tx) => this.artifacts.reconcileSharedValueTx(
          tx,
          edge.artifactId,
          targetId,
          sharedArtifactScope(edge.namespaceId, edge.namespaceGeneration, edge.key, edge.valueRevision, edge.artifactId, edge.jsonPointer),
          retainUntil,
        ), { isolationLevel: Prisma.TransactionIsolationLevel.Serializable });
        repaired += 1;
      } catch (error) {
        if (error instanceof AppError && (error.code === 'conflict' || error.code === 'shared_artifact_expired')) {
          await this.prisma.$transaction((tx) => this.artifacts.releaseSharedValueTx(tx, targetId), {
            isolationLevel: Prisma.TransactionIsolationLevel.Serializable,
          });
          expired += 1;
    ...
    const [grantCandidates, holdCandidates] = await Promise.all([
      ...
      this.prisma.runtimeArtifactHold.findMany({
        where: { executionKind: 'STANDARD', holderKind: 'SHARED_VALUE', purpose: 'SHARED_VALUE', releasedAt: null },
        select: { artifactId: true, holderId: true },
        orderBy: { id: 'asc' },
        take: limit,
      }),
    ]);
    const candidates = [...grantCandidates, ...holdCandidates.map((hold) => ({ artifactId: hold.artifactId, targetId: hold.holderId }))]
      .filter((candidate, index, all) => all.findIndex((item) => item.artifactId === candidate.artifactId && item.targetId === candidate.targetId) === index);
    for (const candidate of candidates) {
      const key = `${candidate.artifactId}\0${candidate.targetId}`;
      if (liveTargets.has(key) || await this.sharedTargetExists(candidate.artifactId, candidate.targetId)) continue;
      await this.prisma.$transaction((tx) => this.artifacts.releaseSharedValueTx(tx, candidate.targetId), {
        isolationLevel: Prisma.TransactionIsolationLevel.Serializable,
      });
      released += 1;
    }
  ...
  async changes(principal: SharedInvocationPrincipal, locator: SharedNamespaceLocator, after: unknown, rawLimit: unknown) {
    const target = normalizeLocator(locator);
    ...
    if (cursor > 0n) {
      const retained = await this.prisma.sharedStateOutbox.findFirst({
        where: { namespaceId: namespace.id, namespaceGeneration: namespace.generation, createdAt: { gte: new Date(Date.now() - 7 * 24 * 60 * 60 * 1000) } },
        orderBy: { cursor: 'asc' },
        select: { cursor: true },
      });
      if (retained && cursor + 1n < retained.cursor) {
        throw new AppError(410, 'shared_change_cursor_expired', '共享数据变更游标已过期，请执行全量重建', {
          latest_cursor: namespace.nextChangeCursor.toString(),
        });
      }
    }
    const rows = await this.prisma.sharedStateOutbox.findMany({
      where: { namespaceId: namespace.id, namespaceGeneration: namespace.generation, cursor: { gt: cursor } },
      orderBy: { cursor: 'asc' },
      take: limit,
    });
    return {
  ...
  async list(principal: SharedInvocationPrincipal, locator: SharedNamespaceLocator, input: { pageCursor?: unknown; relistToken?: unknown; limit?: unknown }) {
    const target = normalizeLocator(locator);
    ...
    const token = input.relistToken
      ...
      : { namespaceId: namespace.id, generation: namespace.generation, snapshotCursor: namespace.nextValueRevision.toString() };
  ...
  private async namespaceForWrite(
    ...
    const existing = await tx.pluginSharedNamespace.findUnique({ where: { teamId_ownerKind_ownerId_name: identity } });
    if (existing) {
      if (existing.deletedAt) throw new AppError(409, 'shared_namespace_deleted', '共享命名空间已停用');
      return existing;
    }
    if (!isOwner(principal, locator)) throw sharedNotFound('shared_namespace_not_found', '共享命名空间不存在');
  ...
  private async loadNamespaceDeclaration(principal: SharedInvocationPrincipal, locator: SharedNamespaceLocator) {
    ...
    const manifest = release?.manifest && typeof release.manifest === 'object' && !Array.isArray(release.manifest)
      ? release.manifest as Record<string, unknown>
      : null;
    const declarations = Array.isArray(manifest?.shared_namespaces) ? manifest.shared_namespaces : [];
    const parsed = declarations.map((entry) => SharedNamespaceDeclaration.safeParse(entry)).find((entry) => entry.success && entry.data.name === locator.name);
    if (!parsed?.success) throw new AppError(409, 'shared_schema_version_unsupported', '发行版未声明该共享命名空间 schema');
    return parsed.data;
  }
  ...
  private async sharedTargetExists(artifactId: string, targetId: string) {
    ...
    return edges.some((edge) => !edge.namespace.deletedAt
      && edge.namespace.generation === edge.namespaceGeneration
      && edge.value.namespaceGeneration === edge.namespaceGeneration
      && edge.value.revision === edge.valueRevision
      && sharedArtifactTarget(edge.namespaceId, edge.namespaceGeneration, edge.key, edge.valueRevision) === targetId);
  }
  ...
  private async authorize(
    ...
    if (owner && !result.decision.allowed && result.decision.reason_code !== 'high_risk_not_enabled') {
...
function normalizeLocator(locator: SharedNamespaceLocator): SharedNamespaceLocator {
  ...
  if (!ownerId || ownerId.length > 256 || !name || name.length > 128 || /[\u0000-\u001f\u007f]/u.test(name)) {
...
function parseMigration(input: unknown) {
  ...
  if (!parsed.success) throw new AppError(400, 'shared_schema_migration_invalid', '共享数据 schema 迁移参数无效');
  return parsed.data;
}

function parseReactivation(input: unknown) {
  const parsed = SharedNamespaceReactivate.safeParse(input);
  if (!parsed.success) throw new AppError(400, 'shared_namespace_reactivation_invalid', '共享命名空间重建参数无效');
  return parsed.data;
}
...
function parseRequiredRevision(value: unknown): bigint {
  if (typeof value !== 'string' || !/^[0-9]+$/.test(value)) {
    throw new AppError(400, 'shared_expected_revision_required', '删除共享值必须提供 expected_revision');
  }
  return BigInt(value);
}
...
function parseLimit(value: unknown): number {
  ...
  if (!Number.isInteger(parsed) || parsed < 1 || parsed > 200) throw badRequest('limit 必须位于 1 到 200');
  return parsed;
}

function relistSecret(): string {
  const secret = process.env.SHARED_RELIST_TOKEN_SECRET?.trim() || process.env.JWT_SECRET?.trim();
  if (!secret || secret.length < 16) throw new AppError(503, 'shared_relist_unavailable', '共享数据全量重建服务不可用');
  return secret;
}
...
function verifyRelistToken(value: string, namespaceId: string, generation: number): RelistTokenPayload {
  try {
    ...
    const payload = JSON.parse(Buffer.from(encoded, 'base64url').toString('utf8')) as RelistTokenPayload;
    if (payload.namespaceId !== namespaceId || payload.generation !== generation || !/^[0-9]+$/.test(payload.snapshotCursor)) throw new Error('stale');
    return payload;
  } catch {
    throw new AppError(409, 'shared_namespace_generation_stale', '共享数据全量重建令牌无效或已过期');
  }
}
...
function assertWritableSchema(active: number, requested: number) {
  if (active !== requested) {
    throw new AppError(409, 'shared_schema_version_unsupported', '共享值 schemaVersion 未声明或当前不可写', {
...
function assertReadableSchema(active: number, stored: number) {
  if (active !== stored) {
    throw new AppError(409, 'shared_schema_version_unsupported', '当前调用方不支持该共享值 schemaVersion', {
      active_schema_version: active,
...
type SharedArtifactRefValue = { type: 'artifact_ref'; artifact_id: string; media_type: string; size_bytes: number; sha256: string; authorization: { scope: 'TEAM'; team_id: string; handle: string } };
function collectArtifactRefs(value: unknown, pointer = ''): Array<{ ref: SharedArtifactRefValue; pointer: string }> {
  if (!value || typeof value !== 'object') return [];
  if (!Array.isArray(value) && (value as { type?: unknown }).type === 'artifact_ref') return [{ ref: value as SharedArtifactRefValue, pointer }];
  return Object.entries(value).flatMap(([key, child]) => collectArtifactRefs(child, `${pointer}/${key.replace(/~/g, '~0').replace(/\//g, '~1')}`));
}

function sharedArtifactTarget(namespaceId: string, generation: number, key: string, revision: bigint) { return `${namespaceId}:${generation}:${key}:${revision.toString()}`; }
function sharedArtifactScope(namespaceId: string, generation: number, key: string, revision: bigint, artifactId: string, jsonPointer: string) { return { namespace_id: namespaceId, namespace_generation: generation, key, value_revision: revision.toString(), artifact_id: artifactId, json_pointer: jsonPointer }; }
...
function publicValue(row: StoredValue, artifacts: Array<{ namespaceId: string; namespaceGeneration: number; key: string; valueRevision: bigint; artifactId: string; jsonPointer: string; executionKind: string }> = []) {
  return {
    ...
    updated_at: row.updatedAt.toISOString(),
    artifacts: artifacts.map((edge) => ({ namespace_id: edge.namespaceId, namespace_generation: edge.namespaceGeneration, key: edge.key, value_revision: edge.valueRevision.toString(), artifact_id: edge.artifactId, json_pointer: edge.jsonPointer, execution_kind: edge.executionKind })),
  };
}
...
function redactArtifactHandles(value: Prisma.JsonValue): Prisma.JsonValue {
  ...
  if (record.type === 'artifact_ref' && record.authorization && typeof record.authorization === 'object' && !Array.isArray(record.authorization)) {
  ...
  return Object.fromEntries(Object.entries(record).map(([key, child]) => [key, redactArtifactHandles(child)]));
}
