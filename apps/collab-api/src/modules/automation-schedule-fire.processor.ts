...
export class AutomationScheduleFireProcessor {
  constructor(
    ...
    @Inject(WorkflowRunService) private readonly runs: WorkflowRunService,
  ...
  async process(job: ScheduleFireJob): Promise<ScheduleFireResult> {
    ...
    const schedule = await this.prisma.automationSchedule.findFirst({
      where: { id: job.schedule_id, generation: job.generation, status: 'ACTIVE' },
    });
    if (!schedule) return { outcome: 'STALE' as const, run_id: null };
    if (schedule.schedulerKey !== job.scheduler_key)
      return { outcome: 'STALE' as const, run_id: null };
    let scheduledFor: Date;
    let occurrenceKey: string;
    if (job.kind === 'REPEAT') {
      if (
        (schedule.kind !== 'DAILY' && schedule.kind !== 'WEEKLY') ||
        job.repeat_job_key !== schedule.schedulerKey ||
        !Number.isFinite(job.prev_millis)
      )
        return { outcome: 'STALE' as const, run_id: null };
      scheduledFor = new Date(job.prev_millis);
      occurrenceKey = scheduleOccurrenceKey(schedule.id, schedule.generation, scheduledFor);
    } else {
      ...
      if (job.kind === 'ONCE') {
        if (
          schedule.kind !== 'ONCE' ||
          !schedule.runAt ||
          schedule.runAt.getTime() !== scheduledFor.getTime()
        )
          return { outcome: 'STALE' as const, run_id: null };
      } else if (
        (schedule.kind !== 'DAILY' && schedule.kind !== 'WEEKLY') ||
    ...
    const existing = await this.prisma.workflowRun.findUnique({
      where: {
        scheduleId_scheduleGeneration_occurrenceKey: {
    ...
    try {
      const created = await this.runs.startScheduled(
        ...
        const trigger =
          schedule.kind === 'DAILY'
            ? {
                ...
                localTime: String(schedule.localTime),
              }
            : {
