...
import { CreateCloudActionDeploymentDto, UpdateCloudActionRoutingDto } from './dto/cloud-action-deployment.dto';
...
export class CloudActionDeploymentController {
  constructor(@Inject(CloudActionDeploymentService) private readonly deployments: CloudActionDeploymentService, @Inject(CloudActionRoutingService) private readonly routing: CloudActionRoutingService) {}
  ...
  list(@Req() req: Request, @Param('releaseId') releaseId: string, @Param('actionId') actionId: string) {
  ...
  updateRouting(@Req() req: Request, @Param('releaseId') releaseId: string, @Param('actionId') actionId: string, @Body() body: UpdateCloudActionRoutingDto) {
    return this.routing.update(requireUser(req).id, releaseId, actionId, body);
  }

  @Get('cloud-actions/:releaseId/:actionId/routing')
  getRouting(@Req() req: Request, @Param('releaseId') releaseId: string, @Param('actionId') actionId: string, @Query('action_contract_version') version: string, @Query('action_surface_sha256') surface: string, @Query('environment') environment: 'PREVIEW' | 'PRODUCTION' = 'PRODUCTION') {
    return this.routing.get(requireUser(req).id, releaseId, actionId, version, surface, environment);
  }
}
