...
const IDS = {
  legacy: {
    ...
    listing: '11111111-1111-4111-8111-111111111112',
    release: '11111111-1111-4111-8111-111111111113',
  },
  quality: {
    package: '22222222-2222-4222-8222-222222222221',
    listing: '22222222-2222-4222-8222-222222222222',
    release: '22222222-2222-4222-8222-222222222223',
  },
};
...
function row(identity: typeof IDS.legacy, legacyInstallCount: number) {
  ...
  return {
    id: identity.listing,
    packageId,
    currentReleaseId: identity.release,
    priceCents: 0,
    priceRevision: 1,
    status: 'ACTIVE',
    installCount: legacyInstallCount,
    ratingCount: legacyInstallCount,
    ratingSum: legacyInstallCount * 5,
    category: 'AI',
    qualityTier: 'QUALITY',
    qualitySnapshotId: `snapshot-${packageId}`,
    qualityQualifiedAt: new Date('2026-07-15T00:00:00Z'),
    featuredRank: null,
    featuredAt: null,
    featuredUntil: null,
    updatedAt: now,
...
function snapshot(packageId: string, activeTeams: number, installTeams: number, rating: number) {
  return {
    ...
    autoQualified: true,
    listingAgeDays: 20,
    currentReleaseAgeDays: 10,
    activeTeams30d: activeTeams,
    installTeams30d: installTeams,
    observedRuns30d: 50,
    failedRuns30d: 0,
    failureRateBps: 0,
    ratingTeams: 10,
    ratingSum: 45,
    averageRatingTenths: rating,
...
describe('MarketplaceDiscoveryService quality ordering', () => {
  it('orders category popular by immutable snapshot facts and never legacy counters', async () => {
    ...
    expect(result.items.map((item) => item.package_id)).toEqual([
      IDS.quality.package,
      IDS.legacy.package,
    ]);
