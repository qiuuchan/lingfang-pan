...
function candidate(
  packageId: string,
  overrides: Partial<MarketplaceRecommendationCandidate> = {}
...
describe('marketplace recommendation ordering', () => {
  it('orders active featured entries by explicit rank, time and stable package ID', () => {
    const items = [
      candidate('b', {
        tier: 'FEATURED',
        featuredRank: 2,
        featuredAt: new Date('2026-07-15T00:00:00Z'),
      }),
      candidate('c', {
        tier: 'FEATURED',
        featuredRank: 1,
        featuredAt: new Date('2026-07-14T00:00:00Z'),
      }),
      candidate('a', {
        tier: 'FEATURED',
        featuredRank: 1,
    ...
    expect(featuredRecommendations(items, now).map((item) => item.packageId)).toEqual([
      'a',
      'c',
      'b',
    ]);
  ...
  it('orders category popular only by public usage/rating fields and stable package ID', () => {
    ...
    expect(categoryPopularRecommendations(items, 'DEV').map((item) => item.packageId)).toEqual([
      'a',
      'b',
      'c',
      'd',
  ...
  it('recent quality uses the 30-day qualification instant and excludes listed entries', () => {
    const items = [
      ...
      candidate('featured', {
        tier: 'FEATURED',
        qualityQualifiedAt: new Date('2026-07-14T00:00:00Z'),
      }),
    ...
    expect(recentQualityRecommendations(items, now).map((item) => item.packageId)).toEqual([
      'new',
      'featured',
    ]);
  });
