...
export function evaluateMarketplaceQualityV1(input: MarketplaceQualityEvaluationInput): MarketplaceQualityEvaluation {
  if (Number.isNaN(input.factWatermark.getTime())) throw new Error('marketplace_quality_invalid_watermark');
  ...
  if (failedRuns > observedRuns || approvedRefunds > maturedOrders || ratingSum > ratingTeams * 5) throw new Error('marketplace_quality_inconsistent_counts');
  if (input.paid && input.refundMetricState === 'NOT_APPLICABLE' || !input.paid && input.refundMetricState !== 'NOT_APPLICABLE') {
  ...
  const releaseAgeDays = elapsedDays(laterDate(input.releaseEligibleSince, input.currentReleaseActivatedAt), input.factWatermark);
  ...
  const refundRateBps = input.paid && input.refundMetricState === 'AVAILABLE' && maturedOrders >= policy.matured_paid_orders_90d
    ? Math.floor(approvedRefunds * 10_000 / maturedOrders) : null;
  const reasons: MarketplaceQualityReasonDetail[] = [];
  const reason = (code: MarketplaceQualityReasonDetail['code'], actual: number | null, threshold: number | null) => reasons.push({ code, actual, threshold });

  if (!input.hardGateEligible) reason('hard_gate_failed', null, null);
  if (listingAgeDays < policy.listing_age_days) reason('listing_age_insufficient', listingAgeDays, policy.listing_age_days);
  if (releaseAgeDays < policy.current_release_activation_age_days) reason('release_age_insufficient', releaseAgeDays, policy.current_release_activation_age_days);
  if (activeTeams < policy.active_teams_30d) reason('insufficient_active_teams', activeTeams, policy.active_teams_30d);
  if (observedRuns < policy.observed_runs_30d) reason('insufficient_observed_runs', observedRuns, policy.observed_runs_30d);
  else if (failureRateBps !== null && failureRateBps > policy.max_failure_rate_bps) reason('failure_rate_high', failureRateBps, policy.max_failure_rate_bps);
  if (ratingTeams < policy.rating_teams) reason('insufficient_rating_teams', ratingTeams, policy.rating_teams);
  else if (averageRatingTenths !== null && averageRatingTenths < policy.min_average_rating_tenths) reason('average_rating_low', averageRatingTenths, policy.min_average_rating_tenths);
  if (input.paid) {
    if (input.refundMetricState === 'DATA_UNAVAILABLE') reason('refund_data_unavailable', null, null);
    else if (input.refundMetricState === 'INSUFFICIENT_SAMPLE' || maturedOrders < policy.matured_paid_orders_90d) reason('insufficient_matured_paid_orders', maturedOrders, policy.matured_paid_orders_90d);
    else if (refundRateBps !== null && refundRateBps > policy.max_refund_rate_bps) reason('refund_rate_high', refundRateBps, policy.max_refund_rate_bps);
  }
  ...
  return { tier: reasons.length === 0 ? 'QUALITY' : 'LISTED', autoQualified: reasons.length === 0, metrics, reasons };
}
