...
describe('plugin registry model helpers', () => {
  ...
  it('rejects invalid utf-8 and control characters in release source labels', () => {
    expect(() => normalizeReleaseSource({ sourceLabelBase64: Buffer.from([0xff]).toString('base64url') }))
      .toThrow(/来源标签编码无效/);
    expect(() => normalizeReleaseSource({ sourceLabelBase64: Buffer.from('bad\u0000label').toString('base64url') }))
      .toThrow(/非法字符/);
  });

  it('selects the highest strict semver value', () => {
    expect(highestSemVer([
      { id: 'a', version: '1.9.0' },
