...
function harness() {
  ...
  const activeRevision = () => policy?.activeRevisionId
    ? revisions.find((revision) => revision.id === policy.activeRevisionId) ?? null
    : null;
  const tx: any = {
    teamPluginPolicy: {
      findUnique: vi.fn(async () => policy ? { ...policy, activeRevision: activeRevision() } : null),
    ...
    teamPluginPolicyRevision: {
      create: vi.fn(async ({ data }: any) => {
        const revision = { id: `revision-${nextId}`, createdAt: new Date('2026-07-16T00:00:00.000Z'), ...data };
        nextId += 1;
        revisions.push(revision);
        return revision;
      }),
      findFirst: vi.fn(async ({ where }: any) => revisions.find((revision) => revision.policyId === where.policyId && revision.teamId === where.teamId && revision.revision === where.revision) ?? null),
      findMany: vi.fn(async () => [...revisions].sort((left, right) => right.revision - left.revision)),
    },
    auditLog: {
      create: vi.fn(async ({ data }: any) => { audits.push(data); return data; }),
    },
  };
  const prisma: any = {
  ...
  return { service: new PluginGovernancePolicyService(prisma), prisma, tx, getState: () => ({ policy, revisions, audits }) };
}
...
describe('PluginGovernancePolicyService concurrency', () => {
  it('allows only one publish from the same expected revision and keeps history immutable', async () => {
    ...
    expect(state.revisions[0]).toMatchObject({ revision: 1, changeReason: expect.stringMatching(/first|second/) });
  ...
  it('serializes rollback against a concurrent writer and creates a monotonic copy-forward revision', async () => {
    ...
    expect(state.revisions[2]).toMatchObject({ revision: 3, sourceRevisionId: state.revisions[0].id, document });
