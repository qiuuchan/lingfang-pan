...
databaseDescribe(
  `PluginSharedStateService database integration (${process.env.DATABASE_PROVIDER || 'unknown'})`,
  () => {
    beforeAll(async () => {
      prisma = new PrismaService();
      await prisma.$connect();
      await prisma.team.create({
        data: { id: teamId, name: `Shared Integration ${suffix}`, slug: `shared-${suffix}` },
      });
      await prisma.user.create({
        data: {
          id: userId,
          email: `shared-${suffix}@example.test`,
          displayName: 'Shared Integration',
      ...
      const governance = {
        authorizeRelease: vi.fn(async () => ({
          decision: { allowed: true, reason_code: 'allowed', reason: 'allowed' },
        })),
      };
      const artifacts = {
    ...
    it('allows only one concurrent CAS winner and prevents delete-recreate ABA', async () => {
      ...
      const initial = await service.set(principal, locator, 'asset', {
        value: { name: 'initial' },
        schema_version: 1,
      });
      const outcomes = await Promise.allSettled([
        service.set(principal, locator, 'asset', {
          value: { name: 'writer-a' },
          schema_version: 1,
      ...
      const recreated = await service.set(principal, locator, 'asset', {
      ...
      await expect(
        ...
        status: 409,
        code: 'shared_revision_conflict',
        details: { current_revision: recreated.revision, retryable: true },
      });
    }, 30_000);
    ...
    it('preserves namespace identity and allocators across delete and reactivate generations', async () => {
      ...
      const first = await service.set(principal, locator, 'a', {
        value: { name: 'a' },
        schema_version: 1,
      });
      ...
      await expect(
        prisma.pluginSharedValue.count({ where: { namespaceId: before.id } })
      ).resolves.toBe(0);

      const reactivated = await service.reactivateNamespace(principal, locator, {
      ...
      await expect(
        service.set(principal, locator, 'a', {
          value: { name: 'stale-generation' },
          schema_version: 1,
          expected_revision: first.revision,
        })
      ).rejects.toMatchObject({ status: 409, code: 'shared_revision_conflict' });
      const fresh = await service.set(principal, locator, 'a', {
        value: { name: 'fresh-generation' },
        schema_version: 1,
      });
    ...
    it('validates migration schemas, uses CAS, and rolls back active schema on transactional failure', async () => {
      ...
      await expect(
        ...
        status: 409,
        code: 'shared_revision_conflict',
        details: { current_revision: updated.revision },
      });

      const rollbackLocator = locate('migration.rollback');
      const rollbackValue = await service.set(principal, rollbackLocator, 'asset', {
        value: { name: 'small' },
        schema_version: 1,
      });
      ...
      await expect(
        service.migrate(principal, rollbackLocator, 'asset', {
