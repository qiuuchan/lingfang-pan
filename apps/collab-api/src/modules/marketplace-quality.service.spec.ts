...
function setup(overrides: Record<string, any> = {}) {
  ...
  const auth = {
    ensureCurrentTeam: vi.fn().mockResolvedValue({ teamId: ids.team }),
    ensurePlatformAdmin: vi.fn().mockResolvedValue({ id: ids.user }),
  };
  const computations = {
    manualJobKey: vi.fn().mockReturnValue('job-1'),
    dailyJobKey: vi.fn().mockReturnValue('daily-job-1'),
    compute: vi.fn().mockResolvedValue({ computationId: 'c1', snapshotId: 's1', projected: true }),
  };
  ...
  return {
    ...
    facts,
    tx,
    service: new MarketplaceQualityService(
      prisma as never,
      auth as never,
      computations as never,
      tickets as never,
      facts as never
    ),
  };
...
describe('MarketplaceQualityService', () => {
  it('writes the current rating, immutable revision and trusted metric in one transaction', async () => {
    ...
    expect(tx.marketplaceRatingRevision.create).toHaveBeenCalledWith({
    ...
    expect(tx.marketplaceMetricEvent.create).toHaveBeenCalledWith({
      data: expect.objectContaining({
        kind: 'RATING_CHANGED',
        source: 'REGISTRY',
        releaseId: ids.release,
  ...
  it('rejects author-team ratings before writing any projection', async () => {
    ...
    await expect(service.rate(ids.user, ids.package, { score: 5 })).rejects.toMatchObject({
      status: 403,
    });
  ...
  it('requires a successful trusted run before rating a free package', async () => {
    ...
    await expect(service.rate(ids.user, ids.package, { score: 4 })).rejects.toMatchObject({
      status: 403,
    });
  });
  ...
  it('feeds persisted terminal facts into a manual computation and records the admin action', async () => {
    ...
    expect(computations.compute).toHaveBeenCalledWith(
      expect.objectContaining({
        facts: expect.objectContaining({ activeTeams30d: 1, observedRuns30d: 2, failedRuns30d: 1 }),
      })
    );
    expect(facts.load).toHaveBeenCalledWith(ids.package, expect.any(Date));
    expect(prisma.auditLog.create).toHaveBeenCalledWith({
      data: expect.objectContaining({ action: 'admin.marketplace.quality.recomputed' }),
    });
  });
  ...
  it('uses one fixed watermark for a paged daily quality pass', async () => {
    ...
    const { service, facts, computations } = setup({
      marketplaceListing: { findMany: firstPage, findUnique: vi.fn().mockResolvedValue(listing()) },
    });
  ...
  it('sets featured state only through a hard-gated CAS and writes its audit reason', async () => {
    ...
    const current = {
      ...base,
      eligibilityGateDigest: marketplaceQualityGateDigest({
        listingStatus: base.status,
        currentReleaseId: base.currentReleaseId,
        currentReleaseStatus: base.currentRelease.status,
        marketReviewStatus: base.currentRelease.marketReviewStatus,
        aiPolicyVersion: base.currentRelease.aiPolicyVersion,
        aiPolicyStatus: base.currentRelease.aiPolicyStatus,
        securityBlocked: false,
        pointerRevision: base.pointerRevision,
    ...
    await expect(
      service.feature(ids.user, ids.package, {
        reason: '编辑推荐',
        rank: 2,
        until: '2099-01-01T00:00:00.000Z',
      })
    ).resolves.toMatchObject({ tier: 'FEATURED', rank: 2 });
    ...
    expect(auditCreate).toHaveBeenCalledWith({
      data: expect.objectContaining({ action: 'admin.marketplace.quality.featured' }),
    });
  });
  ...
  it('blocks automatic quality without discarding an active manual feature', async () => {
    ...
    const { service } = setup({ $transaction: vi.fn((operation) => operation(marketTx)) });
    await expect(
      service.setQualityBlocked(ids.user, ids.package, true, '异常复核')
    ).resolves.toMatchObject({ blocked: true, tier: 'FEATURED' });
    expect(updateMany).toHaveBeenCalledWith(
      expect.objectContaining({
        data: expect.objectContaining({
          qualityTier: 'FEATURED',
          qualityBlockedReason: '异常复核',
        }),
      })
    );
  });
