...
import { CLOUD_RESPONSE_SIGNATURE_HEADER, cloudRequestHeaders, verifyCloudResponseSignature, type CloudSignatureTarget } from './cloud-signature';
import type { CloudActionTargetDto, CreateCloudActionDeploymentDto } from './dto/cloud-action-deployment.dto';
...
function signatureTarget(target: CloudActionTargetDto | DeploymentRow): CloudSignatureTarget {
  return {
    ...
    actionContractVersion: 'action_contract_version' in target ? target.action_contract_version : target.actionContractVersion,
    actionSurfaceSha256: 'action_surface_sha256' in target ? target.action_surface_sha256 : target.actionSurfaceSha256,
  };
}
...
export class CloudActionDeploymentService {
  ...
  async create(userId: string, input: CreateCloudActionDeploymentDto) {
    ...
    const deployment = await this.prisma.$transaction(async (tx) => {
      const row = await tx.cloudActionDeployment.create({ data: {
        id,
      ...
      await tx.auditLog.create({ data: {
        ...
        targetId: id,
        metadata: { teamId: membership.teamId, releaseId: input.target.release_id, actionId: input.target.action_id, environment: input.target.environment, endpointHost: new URL(endpointUrl).hostname },
      } });
      return row;
    });
  ...
  async list(userId: string, releaseId: string, actionId: string) {
    const membership = await this.managementContext(userId);
    const release = await this.prisma.pluginRelease.findFirst({ where: { id: releaseId, package: { ownerTeamId: membership.teamId } }, select: { id: true } });
    if (!release) throw notFound('插件发行版不存在');
    const rows = await this.prisma.cloudActionDeployment.findMany({ where: { teamId: membership.teamId, releaseId, actionId }, orderBy: { createdAt: 'desc' } });
  ...
  async verify(userId: string, id: string) {
    ...
    const claimed = await this.prisma.cloudActionDeployment.updateMany({ where: { id, teamId: membership.teamId, status: 'DRAFT' }, data: { status: 'VERIFYING' } });
    if (claimed.count !== 1) throw conflict('只有 DRAFT deployment 可以验证');
    try {
      await this.performChallenge(deployment);
      const now = new Date();
      const finished = await this.prisma.cloudActionDeployment.updateMany({ where: { id, teamId: membership.teamId, status: 'VERIFYING' }, data: { status: 'READY', lastHealthAt: now, lastHealthOk: true, lastHealthErrorCode: '' } });
      if (finished.count !== 1) throw conflict('deployment 状态已变化');
      const ready = await this.prisma.cloudActionDeployment.findFirst({ where: { id, teamId: membership.teamId } });
      ...
      await this.prisma.cloudActionDeployment.updateMany({ where: { id, teamId: membership.teamId, status: 'VERIFYING' }, data: { status: 'DRAFT', lastHealthAt: new Date(), lastHealthOk: false, lastHealthErrorCode: code } });
      await this.audit(userId, 'cloud.deployment.verify_failed', id, { teamId: membership.teamId, errorCode: code });
      throw error;
    }
  }
  ...
  async disable(userId: string, id: string) {
    ...
    if (deployment.status !== 'DISABLED') {
      const result = await this.prisma.cloudActionDeployment.updateMany({ where: { id, teamId: membership.teamId, status: { in: ['DRAFT', 'VERIFYING', 'READY'] } }, data: { status: 'DISABLED' } });
    ...
    const row = await this.prisma.cloudActionDeployment.findFirst({ where: { id, teamId: membership.teamId } });
  ...
  async retire(userId: string, id: string) {
    ...
    const [activeRoutes, activeInvocations, activeWorkflowBindings] = await Promise.all([
      this.prisma.cloudActionRouting.count({ where: { OR: [{ stableDeploymentId: id }, { candidateDeploymentId: id }] } }),
      this.prisma.actionInvocation.count({ where: { teamId: membership.teamId, cloudDeploymentId: id, status: { in: ['AUTHORIZED', 'RUNNING'] } } }),
      this.prisma.workflowRunCloudBinding.count({ where: { deploymentId: id, run: { status: { in: ['PENDING', 'RUNNING', 'FAILING', 'CANCELING'] } } } }),
    ]);
    if (activeRoutes > 0 || activeInvocations > 0 || activeWorkflowBindings > 0) {
      throw new AppError(409, 'cloud_deployment_in_use', 'deployment 仍被活动 routing 或非终态执行绑定引用');
    }
    const changed = await this.prisma.cloudActionDeployment.updateMany({
      where: { id, teamId: membership.teamId, status: { in: ['DRAFT', 'VERIFYING', 'READY', 'DISABLED'] } },
      data: { status: 'RETIRED' },
    });
  ...
  async rotateSecret(userId: string, id: string) {
    ...
    const next = await this.prisma.$transaction(async (tx) => {
      const source = await tx.cloudActionDeployment.findFirst({ where: { id, teamId: membership.teamId, status: 'READY' } });
      ...
      await tx.auditLog.create({ data: { actorUserId: userId, action: 'cloud.deployment.secret_rotation_created', targetType: 'CloudActionDeployment', targetId: nextId, metadata: { teamId: membership.teamId, supersedesDeploymentId: id } } });
      return row;
    });
  ...
  private async ownedDeployment(userId: string, id: string) {
    const membership = await this.managementContext(userId);
    const deployment = await this.prisma.cloudActionDeployment.findFirst({ where: { id, teamId: membership.teamId } });
  ...
  private async assertExactOwnedTarget(teamId: string, target: CloudActionTargetDto) {
    const release = await this.prisma.pluginRelease.findFirst({
      where: { id: target.release_id, packageId: target.package_id, sha256: target.sha256, status: 'PUBLISHED', package: { ownerTeamId: teamId, governanceStatus: 'ACTIVE' } },
      select: { id: true },
    });
    ...
    if (resolved.action.cloud_capable !== true) throw new AppError(409, 'cloud_endpoint_target_mismatch', '指定 Action 未声明 Cloud 能力');
  }
  ...
  private async performChallenge(deployment: DeploymentRow) {
    ...
    const body = Buffer.from(JSON.stringify({ type: CLOUD_ENDPOINT_VERIFY_TYPE, challenge_id: challengeId, deployment_id: deployment.id, target }), 'utf8');
    const url = this.http.validateUrl(deployment.endpointUrl);
    const signatureInput = { method: 'POST', canonicalPath: `${url.pathname}${url.search}`, timestamp, nonce, invocationId: challengeId, target: signatureTarget(deployment), deploymentId: deployment.id, body };
    const response = await this.http.request({ url: url.toString(), method: 'POST', headers: cloudRequestHeaders(signatureInput, secret), body, timeoutMs: deployment.timeoutMs, responseLimitBytes: Math.min(deployment.responseLimitBytes, 64 * 1024) });
    if (response.statusCode !== 200) throw new AppError(502, 'cloud_endpoint_unavailable', 'Cloud endpoint 验证失败');
    if (!(singleHeader(response.headers, 'content-type') ?? '').toLowerCase().startsWith('application/json')) throw new AppError(502, 'cloud_endpoint_response_invalid', 'Cloud endpoint 验证响应格式无效');
    const responseSignature = singleHeader(response.headers, CLOUD_RESPONSE_SIGNATURE_HEADER);
    if (!responseSignature || !verifyCloudResponseSignature({ statusCode: response.statusCode, timestamp, nonce, invocationId: challengeId, deploymentId: deployment.id, body: response.body }, responseSignature, secret)) {
      throw new AppError(502, 'cloud_endpoint_signature_invalid', 'Cloud endpoint 响应签名无效');
    }
    let payload: unknown;
    try { payload = JSON.parse(response.body.toString('utf8')); } catch { throw new AppError(502, 'cloud_endpoint_response_invalid', 'Cloud endpoint 验证响应格式无效'); }
    if (!payload || typeof payload !== 'object' || Array.isArray(payload)) throw new AppError(502, 'cloud_endpoint_response_invalid', 'Cloud endpoint 验证响应无效');
    const value = payload as Record<string, unknown>;
    const parsedTarget = CloudActionDeploymentTarget.safeParse(value.target);
    if (value.type !== CLOUD_ENDPOINT_VERIFY_TYPE || value.ok !== true || value.challenge_id !== challengeId || value.deployment_id !== deployment.id || !parsedTarget.success || JSON.stringify(parsedTarget.data) !== JSON.stringify(target)) {
      throw new AppError(502, 'cloud_endpoint_response_invalid', 'Cloud endpoint 验证目标不匹配');
    }
  }

  private async audit(userId: string, action: string, targetId: string, metadata: Record<string, unknown>) {
    await this.prisma.auditLog.create({ data: { actorUserId: userId, action, targetType: 'CloudActionDeployment', targetId, metadata: metadata as Prisma.InputJsonValue } });
  }
}
