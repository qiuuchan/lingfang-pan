...
export class MarketplaceQualityService {
  constructor(
    ...
    @Inject(MarketplaceQualityComputationService)
    private readonly computations: MarketplaceQualityComputationService,
  ...
  policy() {
    return MARKETPLACE_QUALITY_POLICY_V1;
  }
    ...
    return this.prisma.$transaction(
      async (tx) => {
        const current = await tx.marketplaceRating.findUnique({
          where: { packageId_teamId: { packageId, teamId: membership.teamId } },
        });
        ...
        const revisionRow = await tx.marketplaceRatingRevision.create({
          data: {
            ratingId,
            packageId,
            teamId: membership.teamId,
            ...
            score: input.score,
            recordedAt,
            sourceKind: 'TEAM_RATING',
            sourceId,
            actorUserId: userId,
          },
        });
        await tx.marketplaceMetricEvent.create({
          data: {
  ...
  async ownerQuality(userId: string, packageId: string) {
    ...
    const snapshot = listing.qualitySnapshotId
      ? await this.prisma.marketplaceQualitySnapshot.findUnique({
    ...
    return {
      ...
      qualityBlocked: listing.qualityBlockedAt
        ? {
            at: listing.qualityBlockedAt.toISOString(),
            reason: listing.qualityBlockedReason,
          }
        : null,
  ...
  async recompute(actorId: string, packageId: string, requestId?: string) {
    ...
    const result = await this.computePackage(
      packageId,
      'MANUAL',
      factWatermark,
      requestId?.trim() || randomUUID()
    );
    await this.prisma.auditLog.create({
      data: {
        actorUserId: actorId,
  ...
  async feature(
    actorId: string,
    packageId: string,
    input: { reason: string; rank?: number; until?: string }
  ) {
    await this.auth.ensurePlatformAdmin(actorId);
    const reason = requiredReason(input.reason);
    const until = input.until ? new Date(input.until) : null;
    if (until && (!Number.isFinite(until.getTime()) || until <= new Date()))
      throw badRequest('精选结束时间必须位于未来');
    const rank =
      input.rank === undefined ? null : Math.max(0, Math.min(10_000, Math.floor(input.rank)));
    return this.prisma.$transaction(
      async (tx) => {
        ...
        if (!listing || !hardGateEligible(listing))
          throw conflict('只有通过当前市场硬门禁的插件可以设为精选');
        const now = new Date();
        ...
        await tx.auditLog.create({
          data: {
            actorUserId: actorId,
            action: 'admin.marketplace.quality.featured',
            targetType: 'PluginPackage',
            targetId: packageId,
            metadata: { reason, rank, until },
          },
        });
        return {
          packageId,
    ...
    return this.prisma.$transaction(
      async (tx) => {
        ...
        const snapshot = listing.qualitySnapshotId
          ...
          : null;
        const nextTier =
          snapshot?.autoQualified && !listing.qualityBlockedAt
            ? ('QUALITY' as const)
            : ('LISTED' as const);
        const changed = await tx.marketplaceListing.updateMany({
          where: { id: listing.id, updatedAt: listing.updatedAt },
        ...
        await tx.auditLog.create({
          data: {
            actorUserId: actorId,
            action: 'admin.marketplace.quality.unfeatured',
            targetType: 'PluginPackage',
            targetId: packageId,
            metadata: { reason },
          },
    ...
    return this.prisma.$transaction(
      async (tx) => {
        ...
        const featured =
          ...
          (listing.featuredUntil === null || listing.featuredUntil > new Date());
        const nextTier = featured
          ? ('FEATURED' as const)
          : !blocked && snapshot?.autoQualified
            ? ('QUALITY' as const)
            : ('LISTED' as const);
        const changed = await tx.marketplaceListing.updateMany({
          where: { id: listing.id, updatedAt: listing.updatedAt },
          data: blocked
            ? {
                qualityBlockedAt: new Date(),
                qualityBlockedByUserId: actorId,
                qualityBlockedReason: reason,
                qualityTier: nextTier,
              }
            : {
                qualityBlockedAt: null,
                qualityBlockedByUserId: null,
        ...
        await tx.auditLog.create({
          data: {
            actorUserId: actorId,
            action: blocked
              ? 'admin.marketplace.quality.blocked'
              : 'admin.marketplace.quality.unblocked',
            targetType: 'PluginPackage',
            targetId: packageId,
            metadata: { reason },
          },
  ...
  private async ownerListing(userId: string, packageId: string) {
    ...
    if (!pkg || pkg.ownerTeamId !== membership.teamId || !pkg.listing)
      throw notFound('插件包或市场 listing 不存在');
...
function hardGateEligible(listing: any): boolean {
  ...
  return (
    listing.eligibilityGateDigest ===
    marketplaceQualityGateDigest({
