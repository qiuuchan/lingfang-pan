...
function eligible(overrides: Partial<MarketplaceQualityEvaluationInput> = {}): MarketplaceQualityEvaluationInput {
  return {
...
describe('evaluateMarketplaceQualityV1', () => {
  it('qualifies at every exact free-plugin threshold using integer rates', () => {
    ...
    expect(result.metrics).toMatchObject({ failure_rate_bps: 200, average_rating_tenths: 43, refund_metric_state: 'NOT_APPLICABLE' });
  });
  ...
  it('reports each insufficient sample instead of treating empty denominators as zero success', () => {
    const result = evaluateMarketplaceQualityV1(eligible({
      ...
      activeTeams30d: 0, observedRuns30d: 0, failedRuns30d: 0, ratingTeams: 0, ratingSum: 0,
    }));
    ...
    expect(result.reasons.map((item) => item.code)).toEqual([
      'listing_age_insufficient', 'release_age_insufficient', 'insufficient_active_teams',
      'insufficient_observed_runs', 'insufficient_rating_teams',
    ]);
  });
  ...
  it('uses the later release activation/eligibility epoch and enforces failure/rating boundaries', () => {
    ...
    expect(result.reasons.map((item) => item.code)).toEqual(['release_age_insufficient', 'failure_rate_high', 'average_rating_low']);
  });

  it('requires paid refund facts and accepts exactly five percent only with a mature cohort', () => {
    const exact = evaluateMarketplaceQualityV1(eligible({
      paid: true, refundMetricState: 'AVAILABLE', maturedPaidOrders90d: 20, approvedRefunds90d: 1,
    }));
    expect(exact.tier).toBe('QUALITY');
    expect(exact.metrics.refund_rate_bps).toBe(500);

    const high = evaluateMarketplaceQualityV1(eligible({
      paid: true, refundMetricState: 'AVAILABLE', maturedPaidOrders90d: 20, approvedRefunds90d: 2,
    }));
    expect(high.reasons.map((item) => item.code)).toContain('refund_rate_high');

    const unavailable = evaluateMarketplaceQualityV1(eligible({
      paid: true, refundMetricState: 'DATA_UNAVAILABLE', maturedPaidOrders90d: 0, approvedRefunds90d: 0,
    }));
  ...
  it('keeps hard gates, security, anomaly review and manual block independent from metrics', () => {
    const result = evaluateMarketplaceQualityV1(eligible({ hardGateEligible: false, securityIncidents90d: 1, anomalyReviewRequired: true, qualityBlocked: true }));
    expect(result.reasons.map((item) => item.code)).toEqual(['hard_gate_failed', 'security_blocked', 'anomaly_review_required', 'quality_blocked']);
  });

  it('rejects inconsistent aggregate facts', () => {
    expect(() => evaluateMarketplaceQualityV1(eligible({ failedRuns30d: 51 }))).toThrow('inconsistent_counts');
    expect(() => evaluateMarketplaceQualityV1(eligible({ ratingSum: 51 }))).toThrow('inconsistent_counts');
    expect(() => evaluateMarketplaceQualityV1(eligible({ paid: true, refundMetricState: 'NOT_APPLICABLE' }))).toThrow('invalid_refund_state');
  });
});
