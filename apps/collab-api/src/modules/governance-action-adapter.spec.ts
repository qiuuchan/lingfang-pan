...
describe('GovernanceActionAdapter', () => {
  it('resolves and authorizes a compound action exactly once', async () => {
    const resolve = vi
      .fn()
      .mockResolvedValue({ target, action: { execution: { runtime_type: 'cloud' } } });
    const authorizeRelease = vi
      .fn()
      .mockResolvedValue({ decision: { allowed: true }, source: 'team', release: { id: 'r1' } });
    const adapter = new GovernanceActionAdapter(
      { resolve } as never,
    ...
    expect(authorizeRelease).toHaveBeenCalledWith(
      'u1',
      { releaseId: 'r1', packageId: 'p1', sha256: target.sha256 },
      ['execute_cloud', 'invoke_action', 'run_workflow', 'web_preview'],
      {
        action: {
          action_id: 'generate',
          action_contract_version: '1.0.0',
  ...
  it('requires workflow permission from the target runtime for nested ACTION callers', async () => {
    const resolve = vi
      .fn()
      .mockResolvedValue({ target, action: { execution: { runtime_type: 'workflow' } } });
    const authorizeRelease = vi
      .fn()
      .mockResolvedValue({ decision: { allowed: true }, source: 'team', release: { id: 'r1' } });
    const adapter = new GovernanceActionAdapter(
