...
describe('marketplace commerce calculator', () => {
  it('keeps priceVersion opaque and changes it across revision/window facts', () => {
    const base = resolveMarketplacePrice({
      listPriceCents: 100,
      priceRevision: 1,
      discount: null,
      now: active,
    });
    const scheduled = resolveMarketplacePrice({
      listPriceCents: 100,
      priceRevision: 1,
      discount,
      now: before,
    });
    const discounted = resolveMarketplacePrice({
      listPriceCents: 100,
      priceRevision: 1,
      discount,
      now: active,
    });
    const revised = resolveMarketplacePrice({
      listPriceCents: 100,
      priceRevision: 2,
      discount,
      now: active,
    ...
    expect(
      new Set([
        base.price_version,
        scheduled.price_version,
        discounted.price_version,
        revised.price_version,
      ]).size
    ).toBe(4);
  });

  it('validates absolute discount price and the 90-day maximum window', () => {
    expect(() => validateMarketplaceDiscount(100, { ...discount, priceCents: 0 })).toThrow(
      'invalid_price'
    );
    expect(() => validateMarketplaceDiscount(100, { ...discount, priceCents: 100 })).toThrow(
      'invalid_price'
    );
    expect(() =>
      validateMarketplaceDiscount(100, {
        ...discount,
        endsAt: new Date(start.getTime() + 91 * 24 * 60 * 60 * 1000),
      })
    ).toThrow('too_long');
  });
  ...
  it('builds complete balanced journals including a zero platform settlement row', () => {
    ...
    expect(settlement).toContainEqual({
  ...
  it('separates clearing reserve, seller receivable and settlement readiness', () => {
    ...
    expect(
      ...
      clearingReserveCents: 100,
      sellerReceivableCents: 80,
      platformReceivableCents: 20,
      settleable: true,
      heldForRefundReview: false,
    });
    expect(
      projectMarketplaceOrderFunds({
    ...
    expect(
      ...
      clearingReserveCents: 0,
      sellerReceivableCents: 0,
