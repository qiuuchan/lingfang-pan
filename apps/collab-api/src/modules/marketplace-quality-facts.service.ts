...
export class PrismaMarketplaceCommerceFactsAdapter implements MarketplaceCommerceFactsPort {
  ...
  async read(packageId: string, factWatermark: Date): Promise<MarketplaceCommerceQualityFacts> {
    ...
    if (
      ...
      state.writerMode === 'LEGACY' ||
      state.writerMode === 'DRAINING'
    ) {
    ...
    const eligibleBuyerTeamIds = new Set(
      rows
        .filter(
          (row) =>
            row.settlementVersion === 'SETTLEMENT_V2' &&
    ...
    const cohort = rows.filter(
      (row) => row.createdAt >= windowStart && row.createdAt <= factWatermark
    );
    ...
    const matured = cohort.filter(
      (row) => row.refundableUntil !== null && row.refundableUntil <= factWatermark
    );
    if (matured.some((row) => row.status === 'REFUND_REQUESTED')) {
      return { ...unavailableCommerceFacts(), eligibleBuyerTeamIds };
    }
    const approvedRefunds = matured.filter(
      (row) => row.status === 'REFUNDED' && row.refundedAt && row.refundedAt <= factWatermark
    ).length;
    return {
      refundMetricState:
        matured.length >= MARKETPLACE_QUALITY_POLICY_V1.matured_paid_orders_90d
          ? 'AVAILABLE'
          : 'INSUFFICIENT_SAMPLE',
  ...
  async load(packageId: string, factWatermark: Date): Promise<MarketplaceQualityFacts> {
    ...
    const [events, ratingRevisions, commerce] = await Promise.all([
      ...
      listing.priceCents > 0
        ? this.commerce.read(packageId, factWatermark)
        : Promise.resolve({
    ...
    events.forEach((event) => {
      if (event.teamId) factTeamIds.add(event.teamId);
    });
    ...
    const publicEvents = events
      .filter((event) => event.teamId === null || activeTeamIds.has(event.teamId))
      .filter((event) => !isTestFact(event.metadata));
    const recentEvents = publicEvents.filter((event) => event.occurredAt >= thirtyDayStart);
    const runEvents = recentEvents.filter(
      (event) =>
        event.releaseId === listing.currentReleaseId &&
    ...
    const eligibleRatings = latestRatings.filter(
      (rating) =>
        rating.teamId !== listing.package.ownerTeamId &&
        activeTeamIds.has(rating.teamId) &&
        (listing.priceCents > 0
          ? commerce.eligibleBuyerTeamIds.has(rating.teamId)
          : freeUsageTeams.has(rating.teamId))
    );
...
function isTestFact(metadata: unknown): boolean {
  ...
  return (
    value.test_data === true || value.platform_test === true || value.excluded_from_quality === true
  );
}
...
function unresolvedSecurityIncidentIds(
  events: Array<{ kind: string; sourceRecordId: string; metadata: unknown }>
): Set<string> {
...
function anomalyReviewRequired(
  events: Array<{ kind: string; teamId: string | null; occurredAt: Date; metadata: unknown }>,
  ...
  const burstTeams = new Set(
    events
      .filter(
        (event) =>
          event.kind === 'RUN_SUCCEEDED' &&
          event.teamId &&
          event.occurredAt.getTime() >= newest - DAY_MS
      )
      .map((event) => event.teamId as string)
  );
