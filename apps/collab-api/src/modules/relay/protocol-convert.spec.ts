...
describe('openAiToAnthropicRequest', () => {
  ...
  it('assistant tool_calls → tool_use 块；role:tool → tool_result 块', () => {
          ...
          tool_calls: [
            {
              id: 'call_1',
              type: 'function',
              function: { name: 'upload_plugin', arguments: '{"name":"x"}' },
            },
          ],
        },
    ...
    expect(msgs[1]).toEqual({
      role: 'assistant',
      content: [{ type: 'tool_use', id: 'call_1', name: 'upload_plugin', input: { name: 'x' } }],
    });
...
describe('anthropicToOpenAiResponse', () => {
  it('text + tool_use → OpenAI message + tool_calls + finish_reason', () => {
    ...
    const choice = (
      out.choices as { message: Record<string, unknown>; finish_reason: string }[]
    )[0];
...
describe('AnthropicStreamToOpenAi', () => {
  it('message_start→role chunk，text_delta→content chunk，message_stop→finish', () => {
    ...
    expect((start[0].choices as { delta: Record<string, unknown> }[])[0].delta).toEqual({
      role: 'assistant',
      content: '',
    });
    const d1 = c.consume(
      e({ type: 'content_block_delta', index: 0, delta: { type: 'text_delta', text: '你好' } })
    );
    expect((d1[0].choices as { delta: Record<string, unknown> }[])[0].delta).toEqual({
      content: '你好',
    });
  ...
  it('tool_use 流式：content_block_start→tool_calls 头，input_json_delta→arguments 增量', () => {
    ...
    const tcStart = c.consume(
      e({
        type: 'content_block_start',
        index: 0,
        content_block: { type: 'tool_use', id: 'tu_1', name: 'upload_plugin' },
      })
    );
    const delta0 = (
      tcStart[0].choices as {
        delta: { tool_calls: { index: number; id: string; function: { name: string } }[] };
      }[]
    )[0].delta;
    expect(delta0.tool_calls[0]).toMatchObject({
      index: 0,
      id: 'tu_1',
      function: { name: 'upload_plugin', arguments: '' },
    });
    const tcDelta = c.consume(
      e({
