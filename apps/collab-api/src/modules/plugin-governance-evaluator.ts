...
export type PluginPolicyDecision = {
  allowed: boolean;
  required_operations: PluginPolicyOperation[];
  team_id: string;
...
const HIGH_RISK_PLUGIN_POLICY_OPERATIONS = new Set<PluginPolicyOperation>([
  'invoke_action',
  'run_workflow',
  'execute_cloud',
  'manage_schedule',
...
function targetMatches(
  rule: TeamPluginPolicyDocumentV1['rules'][number],
  resource: PluginPolicyResource
): boolean {
  ...
  if (target.kind === 'PACKAGE') {
    return (
      target.package_id === resource.package_id &&
      (!target.approved_surface_sha256 ||
        target.approved_surface_sha256 === resource.package_policy_surface_sha256)
    );
  }
  ...
  return (
    target.workflow_release_id === resource.workflow?.workflow_release_id &&
...
function staleSurfaceReason(
  operation: PluginPolicyOperation,
  policy: TeamPluginPolicyDocumentV1,
  resource: PluginPolicyResource
): ReasonCode | null {
  if (!HIGH_RISK_PLUGIN_POLICY_OPERATIONS.has(operation)) return null;
  const rules = policy.rules.filter(
    (rule) => rule.effect === 'ALLOW' && rule.operations.includes(operation)
  );
  if (
    rules.some(
      (rule) =>
        rule.target.kind === 'PACKAGE' &&
        rule.target.package_id === resource.package_id &&
        rule.target.approved_surface_sha256 &&
        rule.target.approved_surface_sha256 !== resource.package_policy_surface_sha256
    )
  )
    return 'package_surface_changed';
  if (
    rules.some(
      (rule) =>
        rule.target.kind === 'ACTION' &&
        rule.target.package_id === resource.package_id &&
        ...
        rule.target.action_surface_sha256 !== resource.action.action_surface_sha256
    )
  )
    return 'action_surface_changed';
  if (
    rules.some(
      (rule) =>
        rule.target.kind === 'WORKFLOW' &&
        rule.target.workflow_release_id === resource.workflow?.workflow_release_id &&
        rule.target.workflow_plan_sha256 !== resource.workflow.workflow_plan_sha256
    )
  )
    return 'workflow_plan_changed';
  return null;
}
...
function teamRuleResult(operation: PluginPolicyOperation, facts: PluginGovernanceFacts) {
  ...
  if (
    policy.allowed_source_kinds.length > 0 &&
    !policy.allowed_source_kinds.includes(facts.resource.source_kind)
  ) {
    return {
      allowed: false,
      reasonCode: 'team_source_denied' as const,
      matched: [{ layer: 'TEAM' as const, effect: 'DENY' as const }],
    };
  }
  if (
    facts.resource.declared_capabilities.some((capability) =>
      policy.denied_capability_kinds.includes(capability)
    )
  ) {
    return {
      allowed: false,
      reasonCode: 'team_capability_denied' as const,
      matched: [{ layer: 'TEAM' as const, effect: 'DENY' as const }],
    };
  }
  const candidates = policy.rules.filter(
    (rule) => rule.operations.includes(operation) && targetMatches(rule, facts.resource)
  );
  ...
  return {
    ...
    matched: matches
      .filter((rule) => rule.effect === winner)
      .map((rule) => ({ layer: 'TEAM' as const, effect: winner, rule_id: rule.rule_id })),
  };
}
...
function evaluateOperation(operation: PluginPolicyOperation, facts: PluginGovernanceFacts) {
  if (!facts.platformAllowed || !facts.entitlementAllowed) {
    return {
      operation,
      allowed: false,
      reason_code: 'platform_gate_denied' as const,
      matched: [{ layer: 'PLATFORM' as const, effect: 'DENY' as const }],
    };
  }
  const team = teamRuleResult(operation, facts);
  if (team?.allowed === false)
    return { operation, allowed: false, reason_code: team.reasonCode, matched: team.matched };
  if (HIGH_RISK_PLUGIN_POLICY_OPERATIONS.has(operation) && team?.allowed !== true) {
    const staleReason = facts.policy
      ? staleSurfaceReason(operation, facts.policy, facts.resource)
      : null;
    return {
      operation,
      allowed: false,
      reason_code: staleReason ?? 'high_risk_not_enabled',
      matched: [{ layer: 'TEAM' as const, effect: 'DENY' as const }],
    };
  }
  if (facts.userGrant === 'DENY')
    return {
      operation,
      allowed: false,
      reason_code: 'user_grant_denied' as const,
      matched: [{ layer: 'USER_GRANT' as const, effect: 'DENY' as const }],
    };
  if (facts.userGrant === 'ALLOW')
    return {
      operation,
      allowed: true,
      reason_code: 'allowed' as const,
      matched: [{ layer: 'USER_GRANT' as const, effect: 'ALLOW' as const }],
    };
  if (facts.roleGrant === 'DENY')
    return {
      operation,
      allowed: false,
      reason_code: 'role_grant_denied' as const,
      matched: [{ layer: 'ROLE_GRANT' as const, effect: 'DENY' as const }],
    };
  if (facts.roleGrant === 'ALLOW')
    return {
      operation,
      allowed: true,
      reason_code: 'allowed' as const,
      matched: [{ layer: 'ROLE_GRANT' as const, effect: 'ALLOW' as const }],
    };
  if (team?.allowed === true)
    return { operation, allowed: true, reason_code: 'allowed' as const, matched: team.matched };
  return {
    operation,
    allowed: true,
    reason_code: 'allowed' as const,
    matched: [{ layer: 'REQUEST' as const, effect: 'ALLOW' as const }],
  };
}
...
export function evaluatePluginGovernance(facts: PluginGovernanceFacts): PluginPolicyDecision {
  ...
  const operationResults = requiredOperations.map((operation) =>
    evaluateOperation(operation, facts)
  );
