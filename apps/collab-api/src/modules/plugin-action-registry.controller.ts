...
export class PluginActionRegistryController {
  ...
  @Get('plugin-releases/:releaseId/actions') list(@Param('releaseId') releaseId: string) {
    return this.actions.list(releaseId);
  }
  @Post('plugin-actions/resolve') resolve(
    @Body()
    body: {
      package_id: string;
      release_id: string;
    ...
    return this.actions.resolve(body);
  }
}
