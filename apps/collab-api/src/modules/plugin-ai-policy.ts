...
function sourceWithoutComments(content: string, path: string): string[] {
  ...
  const hashComments =
    ['.env', '.py', '.toml', '.yaml', '.yml'].includes(ext) ||
...
function scanManifest(
  ...
  const scanString = (value: string, path: string): void => {
    for (const [pattern, provider] of THIRD_PARTY_ENDPOINTS) {
      if (pattern.test(value)) {
        add({
          ...
          path: 'manifest.json',
          message: `manifest 不得配置 ${provider} 模型端点：${path}`,
        });
      }
    }
    if (/\b(?:sk|rk|pk)-(?:proj-)?[A-Za-z0-9_-]{16,}\b/.test(value)) {
      add({
        ...
        path: 'manifest.json',
        message: `manifest 不得包含硬编码模型密钥：${path}`,
      });
    }
  };
  const visit = (value: unknown, path: string, aiContext: boolean): void => {
    ...
    const capabilityKind =
      typeof object.kind === 'string' && AI_CAPABILITIES.has(object.kind as PluginAiCapability);
    for (const [key, child] of Object.entries(value as Record<string, unknown>)) {
      const lower = key.toLowerCase().replace(/[-_]/g, '');
      const nextPath = path ? `${path}.${key}` : key;
      const nextAiContext =
        aiContext || capabilityKind || /(ai|llm|chat|image|model|openai|anthropic)/i.test(key);
      if (
        ['apikey', 'apiurl', 'baseurl', 'authorization'].includes(lower) ||
      ...
      if (
        ...
        add({
          code: 'ai.model.invalid',
          path: 'manifest.json',
          message: '插件模型档位只允许 fast 或 premium',
        });
      }
  ...
  try {
    ...
    for (const field of [
      ...
      if (!dependencies || typeof dependencies !== 'object' || Array.isArray(dependencies))
        continue;
      Object.keys(dependencies as Record<string, unknown>).forEach((name) =>
...
function bridgeEnvHasFallback(line: string): boolean {
  ...
  const functionFallback = line.match(
    /(?:getenv|environ\.get|get)\s*\([^,]+,\s*(['"])(.*?)\1\s*\)/
  );
...
function detectCapabilities(text: string, required: Set<PluginAiCapability>): void {
  if (
    /sdk\.llm\.chat\s*\(|__lingfangInvoke\s*\(\s*['"]llm\.chat['"]|\.chat\.completions\.create\s*\(|\/v1\/chat\/completions/i.test(
      text
    )
  ) {
  ...
  if (
    /sdk\.image\.generate\s*\(|__lingfangInvoke\s*\(\s*['"]image\.generate['"]|\.images\.(?:generate|generations)\s*\(|\/v1\/images\/generations/i.test(
      text
    )
  ) {
  ...
  if (
    /sdk\.image\.edit\s*\(|__lingfangInvoke\s*\(\s*['"]image\.edit['"]|\.images\.edit\s*\(|\/v1\/images\/edits|\/image\/edit/i.test(
      text
    )
  ) {
...
function looksLikeAiDependency(name: string): boolean {
  return /(?:^|[-/@])(openai|anthropic|groq|mistral|replicate|cohere|ollama|langchain|llama|gemini|generative-ai|ai-sdk)(?:$|[-/])/.test(
    name
  );
}
...
function hasOpenAiBridgeBinding(text: string): boolean {
  ...
  const identifiers = (name: 'URL' | 'TOKEN') =>
    [
      ...text.matchAll(
        new RegExp(
  ...
  for (const variable of identifiers('TOKEN')) {
    if (
      new RegExp(`(?:api_key|apiKey)\\s*[:=][^\\n,}]*\\b${escapeRegExp(variable.name)}\\b`).test(
        text
      )
    ) {
      tokenBound = true;
...
function scanSourceFile(
  ...
  if (file.scanError) {
    const reason =
      file.scanError === 'dependency_too_large'
        ? '依赖声明超过 256 KiB'
  ...
  const aiTextContext =
    /(?:sdk\.(?:llm|image)|llm\.chat|image\.generate|openai|anthropic|chat\.completions|images\.generat|LINGFANG_PLUGIN_BRIDGE_)/i.test(
      text
    );
  ...
  standardOpenAiUsage.value ||=
    /(?:from\s+openai\s+import|import\s+OpenAI\s+from\s+['"]openai['"]|require\s*\(\s*['"]openai['"]|@ai-sdk\/openai)/i.test(
      text
    );
  ...
  if (filename === 'package.json') {
    ...
    for (const name of dependencies.names) {
      ...
      if (
        FORBIDDEN_NODE_PACKAGES.has(name) ||
        (looksLikeAiDependency(name) && !ALLOWED_OPENAI_PACKAGES.has(name))
      ) {
  ...
  if (filename === 'requirements.txt') {
    lines.forEach((line, index) => {
      ...
      if (
        name &&
        (FORBIDDEN_PYTHON_PACKAGES.has(name) || (looksLikeAiDependency(name) && name !== 'openai'))
      ) {
        add({
          code: 'ai.sdk.third_party',
          path,
          line: index + 1,
          message: `不得依赖第三方模型 SDK：${name}`,
        });
      }
    });
  }
  ...
  lines.forEach((line, index) => {
    ...
    if (/\b(?:sk|rk|pk)-(?:proj-)?[A-Za-z0-9_-]{16,}\b/.test(line)) {
      add({
        ...
        path,
        line: lineNumber,
        message: '源码不得包含硬编码模型密钥',
      });
    }
    if (
      /(?:from\s+(?:anthropic|groq|mistralai|replicate|dashscope|google\.generativeai)\s+import|require\s*\(\s*['"](?:@anthropic-ai\/sdk|groq-sdk|replicate|mistralai)['"]|from\s+['"](?:@anthropic-ai\/sdk|groq-sdk|replicate|mistralai)['"])/i.test(
        line
      )
    ) {
      add({
    ...
    if (bridgeEnvHasFallback(line)) {
      add({
        ...
        path,
        line: lineNumber,
        message: '宿主 bridge 环境变量不得配置自定义 fallback',
      });
    }
    if (
      /(?:console\.(?:log|debug|info)|\bprint\s*\(|writeFile|appendFile|localStorage|sessionStorage)/.test(
        line
      ) &&
      /LINGFANG_PLUGIN_BRIDGE_(?:URL|TOKEN)/.test(line)
    ) {
      add({
        code: 'ai.bridge.secret_sink',
        path,
        line: lineNumber,
        message: '不得打印或持久化宿主 bridge 连接信息',
      });
    }
    if (
      aiTextContext &&
      /(?:api[_-]?key|api[_-]?url|base[_-]?url|baseURL|authorization|model[_-]?provider)\s*[:=]/i.test(
        line
      ) &&
      ...
      add({
        ...
        path,
        line: lineNumber,
        message: '插件不得配置模型 Key、URL、provider 或 Authorization',
      });
    }
...
export function checkPluginAiPolicy(input: PluginAiPolicyInput): PluginAiPolicyResult {
  ...
  for (const file of Array.isArray(input.files) ? input.files : []) {
    ...
    if (isEntry && file.binary) {
      scanSourceFile(
        { path: file.path, scanError: 'invalid_utf8' },
        required,
        standardOpenAiUsage,
        bridgeEnv,
        add
    ...
    if (typeof file.content === 'string') {
      ...
      if (totalBytes > PLUGIN_AI_POLICY_MAX_TOTAL_BYTES) {
        scanSourceFile(
          { path: file.path, scanError: 'total_too_large' },
          required,
          standardOpenAiUsage,
          bridgeEnv,
          add
  ...
  if (
    manifestEntry &&
    !(Array.isArray(input.files) && input.files.some((file) => file?.path === manifestEntry))
  ) {
    add({
      code: 'ai.policy.unscannable',
      path: manifestEntry,
      message: 'manifest.entry 缺少可扫描的可执行文本',
    });
  }
  if (standardOpenAiUsage.value) required.add('llm.chat');
  if (
    standardOpenAiUsage.value &&
    (!bridgeEnv.url || !bridgeEnv.token || !standardOpenAiUsage.bound)
  ) {
    add({
      code: 'ai.bridge.custom',
      path: 'manifest.json',
...
export function pluginAiPolicyReason(result: PluginAiPolicyResult): string {
  return result.diagnostics
    ...
    .map(
      (diagnostic) =>
        `${diagnostic.code}:${diagnostic.path}${diagnostic.line ? `:${diagnostic.line}` : ''}`
    )
    .join('; ')
    .slice(0, 1000);
}
