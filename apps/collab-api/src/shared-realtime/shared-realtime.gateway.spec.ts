...
describe('SharedRealtimeGateway', () => {
  it('joins, heartbeats and leaves an exact generation room', async () => {
    ...
    const gateway = new SharedRealtimeGateway(store, authenticator as any, broadcaster, {
      apply: vi.fn(),
    } as any);
    ...
    expect(
      await store.list({ teamId: 'team-1', namespaceId: 'ns-1', namespaceGeneration: 2 })
    ).toHaveLength(1);
  ...
  it('rejects malformed rooms without joining transport rooms', async () => {
    ...
    expect(client.emit).toHaveBeenCalledWith('shared:error', {
      code: 'shared_realtime_room_invalid',
    });
  });
