...
export class SharedRealtimeRedisAdapter implements OnModuleDestroy {
  ...
  async apply(namespace: Namespace): Promise<void> {
    ...
    this.subClient = new IORedis(this.config.redisUrl, { ...options, connectionName: 'lingfang-plugin-shared-socket-subscriber' });
    ...
    try {
      ...
      namespace.server.adapter(createAdapter(handledRedisClient(this.pubClient) as never, handledRedisClient(this.subClient) as never));
    } catch (error) {
      await this.close();
      throw new Error(`shared realtime Redis adapter unavailable: ${error instanceof Error ? error.message : 'unknown error'}`);
    }
  }
