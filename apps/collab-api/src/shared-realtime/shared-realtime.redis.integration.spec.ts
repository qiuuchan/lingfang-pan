...
} from './shared-realtime-broadcaster';
...
const roomV1: SharedPresenceRoom = {
  teamId: 'team-shared',
  namespaceId: 'namespace-shared',
...
redisDescribe('shared realtime Redis multi-instance integration', () => {
  beforeAll(async () => {
    admin = new IORedis(TEST_REDIS_URL!, {
      connectionName: 'lingfang-shared-realtime-integration-admin',
    });
  ...
  afterAll(async () => {
    ...
    for (const node of nodes.splice(0).reverse()) {
      const server = (
        node.gateway as unknown as {
          namespace: { server: { close: (callback: () => void) => void } };
        }
      ).namespace.server;
  ...
  it('shares presence and invalidations across two live Nest/Socket.IO instances', async () => {
    ...
    const firstA = nextEvent<PresenceSnapshot>(
      clientA,
      SHARED_PRESENCE_EVENT,
    ...
    const bothOnA = nextEvent<PresenceSnapshot>(
      clientA,
      SHARED_PRESENCE_EVENT,
      (snapshot) => memberIds(snapshot).join(',') === 'user-a,user-b'
    );
    const bothOnB = nextEvent<PresenceSnapshot>(
      clientB,
      SHARED_PRESENCE_EVENT,
      (snapshot) => memberIds(snapshot).join(',') === 'user-a,user-b'
    );
    ...
    const generationTwoSnapshot = nextEvent<PresenceSnapshot>(
      generationTwo,
      SHARED_PRESENCE_EVENT,
    ...
    const secondInvalidationOnB = nextEvent<{ cursor: string }>(
      clientB,
      SHARED_INVALIDATION_EVENT,
    ...
    await expect(nodeA.presence.list(roomV2)).resolves.toMatchObject([
      { user_id: 'user-generation-2' },
    ]);

    clientA.disconnect();
    const afterDisconnect = await heartbeatUntil(
      clientB,
      (snapshot) => memberIds(snapshot).join(',') === 'user-b'
    );
    ...
    const reconnectedOnB = nextEvent<PresenceSnapshot>(
      clientB,
      SHARED_PRESENCE_EVENT,
      (snapshot) => memberIds(snapshot).join(',') === 'user-a,user-b'
    );
    ...
    const expiringJoined = nextEvent<PresenceSnapshot>(
      expiring,
      SHARED_PRESENCE_EVENT,
    ...
    const expiredError = nextEvent<{ code: string }>(
      expiring,
      'shared:error',
      (event) => event.code === 'shared_presence_expired'
    );
...
async function createNode(): Promise<TestNode> {
  ...
  const fakeAuthenticator = {
    ...
    }): Promise<SharedRealtimeAuthContext> {
      const userId = String(socket.handshake.auth?.user_id || '').trim();
      if (!userId)
        throw Object.assign(new Error('unauthorized'), { code: 'shared_realtime_unauthorized' });
      return {
        userId,
        teamId: roomV1.teamId,
        displayName: userId,
  ...
  const moduleRef = await Test.createTestingModule({
    providers: [
      {
        provide: SHARED_REALTIME_CONFIG,
        useValue: { enabled: true, transport: 'redis', redisUrl: TEST_REDIS_URL },
      },
