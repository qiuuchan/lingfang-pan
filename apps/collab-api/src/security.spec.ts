...
function cookieContext(token: string, options: { path?: string; method?: string; csrf?: string } = {}) {
  const csrf = options.csrf ?? '';
  const request: Record<string, unknown> & { header: (name: string) => string } = {
    path: options.path ?? '/web/session',
    method: options.method ?? 'GET',
    header: (name) => name === 'cookie'
...
describe('JwtAuthGuard signed team context', () => {
  ...
  it('accepts HttpOnly-style cookie authentication only on the web API boundary', async () => {
    const token = jwt.sign({ sub: 'u1', email: 'u@example.com', tokenVersion: 1, teamId: 't1', teamContextVersion: 3 }, process.env.JWT_SECRET!);
    const { guard } = build({ status: 'ACTIVE', tokenVersion: 1, teamContextVersion: 3, platformRole: 'NONE', platformRoleId: null });
  ...
  it('requires a matching CSRF header for cookie-authenticated Web mutations', async () => {
    ...
    await expect(guard.canActivate(missing.context)).rejects.toMatchObject({ status: 403, code: 'csrf_invalid' });
