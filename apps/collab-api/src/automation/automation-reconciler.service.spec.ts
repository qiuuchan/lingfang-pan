...
describe('AutomationReconcilerService', () => {
  const config = resolveAutomationConfig({
    ...
    AUTOMATION_ONCE_MISFIRE_WINDOW_MS: '60000',
  });
  ...
  it('marks an overdue ONCE schedule MISSED and emits a deduplicated owner notification', async () => {
        ...
        findMany: vi.fn(async () => [
          {
            ...
            nextRunAt: runAt,
            generation: 1,
            schedulerKey: 'schedule-s1-g1',
            status: 'ACTIVE',
          },
        ]),
        updateMany: vi.fn(async (args: any) =>
          args.data.status === 'MISSED' ? { count: 1 } : { count: 1 }
        ),
        findUnique: vi.fn(),
      },
    ...
    const service = new AutomationReconcilerService(
      prisma,
      config,
      scheduler,
    ...
    expect(prisma.notification.create).toHaveBeenCalledWith({
      data: expect.objectContaining({
        userId: 'u1',
        type: 'automation_schedule_missed',
        relatedId: 's1',
      }),
    });
  });
  ...
  it('coalesces a due recurring projection into one recovery fire and advances the Redis projection', async () => {
    ...
    const prisma = {
      automationSchedule: {
        ...
        findUnique: vi.fn(async () => ({ generation: 5, status: 'ACTIVE' })),
      },
      workflowRun: { findMany: vi.fn(async () => []) },
    ...
    expect(fire.process).toHaveBeenCalledWith(
  ...
  it('projects consecutive failures and notifies only once for the same failure episode', async () => {
    ...
    const prisma = {
      automationSchedule: {
        findMany: vi.fn(async () => [schedule]),
        updateMany: vi.fn(async () => ({ count: 1 })),
        findUnique: vi.fn(async () => ({ generation: 2, status: 'ACTIVE' })),
      },
      workflowRun: {
        findMany: vi
          .fn()
          .mockResolvedValueOnce([
            ...
            { id: 'f2', status: 'FAILED' },
            { id: 'f1', status: 'FAILED' },
            { id: 'ok0', status: 'SUCCEEDED' },
          ])
          .mockResolvedValueOnce([]),
      },
      notification: {
        findFirst: vi.fn(async () => null),
        create: vi.fn(async () => ({ id: 'n3' })),
      },
    } as any;
    const scheduler = {
      remove: vi.fn(),
    ...
    expect(prisma.automationSchedule.updateMany).toHaveBeenCalledWith(
      expect.objectContaining({ data: { consecutiveFailures: 3 } })
    );
    expect(prisma.notification.create).toHaveBeenCalledWith({
      data: expect.objectContaining({
        type: 'automation_schedule_consecutive_failure:ok0',
        relatedId: 's3',
      }),
    });
  });
