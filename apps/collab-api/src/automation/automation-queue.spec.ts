...
import {
  automationControlJob,
  classifyAutomationRedisPolicy,
  createAutomationQueueAdapter,
  type AutomationQueuePort,
} from './automation-queue';

const row = {
  id: 'outbox-1',
  kind: 'UPSERT_SCHEDULE' as const,
  aggregateId: 'schedule-1',
  generation: 4,
  payload: { secret: 'must-never-enter-a-job', input: { prompt: 'private' } },
  attempts: 1,
...
describe('automation queue boundary', () => {
  it('does not construct Redis/BullMQ for feature-off or API-only roles', async () => {
    ...
    const disabled = createAutomationQueueAdapter(
      resolveAutomationConfig({ AUTOMATION_ENABLED: 'false' }),
      factory
    );
    const apiOnly = createAutomationQueueAdapter(
    ...
    await expect(disabled.checkReadiness()).resolves.toMatchObject({
      status: 'disabled',
      redis: 'not_required',
    });
    await expect(apiOnly.checkReadiness()).resolves.toMatchObject({
      status: 'api_only',
      redis: 'not_required',
    });
  });
  ...
  it('creates an infrastructure adapter only through the injected factory', () => {
    ...
    const config = resolveAutomationConfig({
      ...
      AUTOMATION_REDIS_URL: 'redis://automation.example/15',
    });
  ...
  it('projects outbox rows to ID/generation-only deterministic jobs', () => {
    ...
    expect(first).toMatchObject({
      ...
      data: { schedule_id: 'schedule-1', generation: 4 },
    });
