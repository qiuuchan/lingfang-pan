...
export class AutomationReconcilerService implements OnModuleInit, OnModuleDestroy {
  ...
  async reconcileOnce(
    now = new Date()
  ): Promise<{ schedules: number; runs: number; missed: number; recovered: number }> {
    const result = { schedules: 0, runs: 0, missed: 0, recovered: 0 };
    if (!this.config.enabled || !this.config.schedulesEnabled) {
      const active = await this.prisma.automationSchedule.findMany({
        where: { status: 'ACTIVE' },
    ...
    for (const schedule of schedules) {
      ...
      if (
        schedule.kind === 'ONCE' &&
        schedule.runAt &&
        schedule.runAt.getTime() < now.getTime() - this.config.onceMisfireWindowMs
      ) {
      ...
      if (schedule.nextRunAt && schedule.nextRunAt <= now) {
        ...
        const fire =
          schedule.kind === 'ONCE'
            ? await this.fire.process({
                kind: 'ONCE',
                schedule_id: schedule.id,
                generation: schedule.generation,
                scheduler_key: schedule.schedulerKey,
                scheduled_for: schedule.nextRunAt.toISOString(),
      ...
      if (current?.generation === schedule.generation && current.status === 'ACTIVE')
        await this.scheduler.upsert(schedule.id, schedule.generation, now);
    }
  ...
  private async reconcileRuns(result: { runs: number }): Promise<void> {
    ...
    for (const run of runs) {
      ...
      await this.control
        .process({
          name: run.status === 'CANCELING' ? 'run.cancel' : 'run.coordinate',
          data: { run_id: run.id, generation: run.scheduleGeneration ?? 0 },
        })
        .catch(() => undefined);
    }
  }
  ...
  private async refreshFailureProjection(scheduleId: string, ownerUserId: string): Promise<void> {
    ...
    await this.prisma.automationSchedule.updateMany({
      where: { id: scheduleId, consecutiveFailures: { not: failures } },
    ...
    if (failures >= this.config.consecutiveFailureNotifyThreshold) {
      ...
      await this.notifyOnce(
        ownerUserId,
        `automation_schedule_consecutive_failure:${episodeBoundary}`,
        '自动化计划连续失败',
        `该计划已连续失败 ${failures} 次，请检查工作流节点和 Cloud endpoint。`,
        'AutomationSchedule',
        scheduleId
      );
    }
  ...
  private async notifyOnce(
    ...
    const exists = await this.prisma.notification.findFirst({
      where: { userId, type, relatedType, relatedId },
      select: { id: true },
    });
    if (exists) return;
  ...
  private async tick(): Promise<void> {
    ...
    try {
      ...
      /* readiness exposes dependency failures; the next tick retries from Prisma truth. */
    } finally {
      this.running = false;
    }
  }
