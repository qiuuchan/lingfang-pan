...
export type CloudActionJob = {
  name: 'action.invoke';
  data: { kind?: 'WORKFLOW_ATTEMPT'; run_id: string; attempt_id: string; invocation_id: string; plan_sha256: string }
...
export type AutomationWorkerHeartbeat = { process_id: string; role: 'worker' | 'all'; updated_at: string; ttl_ms: number };
...
export function classifyAutomationRedisPolicy(input: {
  ...
  const persistence = input.appendOnly === 'yes' || Boolean(input.savePolicy?.trim())
    ? 'safe' as const
    : input.appendOnly !== undefined || input.savePolicy !== undefined ? 'unsafe' as const : 'unknown' as const;
  const evictionPolicy = input.maxmemoryPolicy === 'noeviction'
    ? 'noeviction' as const
    : input.maxmemoryPolicy ? 'unsafe' as const : 'unknown' as const;
  return { status: persistence === 'safe' && evictionPolicy === 'noeviction' ? 'ready' : 'degraded', persistence, evictionPolicy };
}

function deterministicJobId(kind: string, aggregateId: string, generation: number): string {
  const digest = createHash('sha256').update(`${kind}\0${aggregateId}\0${generation}`).digest('hex').slice(0, 32);
...
export function automationRedisConnectionName(scope: string, prefix: string): string {
  const safeScope = scope.toLowerCase().replace(/[^a-z0-9-]+/g, '-').slice(0, 36);
...
export function automationControlJob(row: ClaimedAutomationOutbox): AutomationControlJob {
  ...
  switch (row.kind) {
    case 'UPSERT_SCHEDULE':
      return { name: 'schedule.upsert', data: { schedule_id: row.aggregateId, generation }, jobId: deterministicJobId(row.kind, row.aggregateId, generation) };
    case 'REMOVE_SCHEDULE':
      return { name: 'schedule.remove', data: { schedule_id: row.aggregateId, generation }, jobId: deterministicJobId(row.kind, row.aggregateId, generation) };
    case 'ENQUEUE_RUN':
      return { name: 'run.coordinate', data: { run_id: row.aggregateId, generation }, jobId: deterministicJobId(row.kind, row.aggregateId, generation) };
    case 'ENQUEUE_ACTION':
      throw new Error('ENQUEUE_ACTION must be published through the action queue');
    case 'CANCEL_RUN':
      return { name: 'run.cancel', data: { run_id: row.aggregateId, generation }, jobId: deterministicJobId(row.kind, row.aggregateId, generation) };
  }
}
...
class DisabledAutomationQueueAdapter implements AutomationQueuePort {
  ...
  async publishOutbox(): Promise<void> { throw new Error('automation_queue_not_available_for_process_role'); }
  async publishAction(): Promise<void> { throw new Error('automation_queue_not_available_for_process_role'); }
  async close(): Promise<void> {}
  async writeWorkerHeartbeat(): Promise<void> {}
  async listWorkerHeartbeats(): Promise<AutomationWorkerHeartbeat[]> { return []; }
...
export function createAutomationQueueAdapter(config: AutomationConfig, factory: AutomationQueueFactory = (value) => new BullMqAutomationQueueAdapter(value)): AutomationQueuePort {
  if (!config.connectsToRedis || !config.redisUrl) return new DisabledAutomationQueueAdapter(config);
...
export class BullMqAutomationQueueAdapter implements AutomationQueuePort, OnModuleDestroy {
  ...
  constructor(config: AutomationConfig) {
    if (!config.connectsToRedis || !config.redisUrl) throw new Error('BullMQ automation adapter requires an infrastructure process role');
    this.redis = new IORedis(config.redisUrl, {
      lazyConnect: true,
      enableReadyCheck: true,
      maxRetriesPerRequest: null,
      connectionName: automationRedisConnectionName(`automation-${config.processRole}`, config.redisPrefix),
    });
    this.redis.on('error', () => undefined);
    const connection = this.redis;
    this.controlQueue = new Queue(AUTOMATION_CONTROL_QUEUE, { connection, prefix: config.redisPrefix });
    this.actionQueue = new Queue(CLOUD_ACTION_QUEUE, { connection, prefix: config.redisPrefix });
  }

  private heartbeatKey(processId: string): string { return `${this.controlQueue.opts.prefix}:worker-heartbeat:${processId}`; }

  async writeWorkerHeartbeat(heartbeat: AutomationWorkerHeartbeat): Promise<void> {
    await this.redis.set(this.heartbeatKey(heartbeat.process_id), JSON.stringify(heartbeat), 'PX', heartbeat.ttl_ms);
  }
  ...
  async listWorkerHeartbeats(): Promise<AutomationWorkerHeartbeat[]> {
    ...
    return values.flatMap((value) => { try { return value ? [JSON.parse(value) as AutomationWorkerHeartbeat] : []; } catch { return []; } });
  }

  async removeWorkerHeartbeat(processId: string): Promise<void> { await this.redis.del(this.heartbeatKey(processId)); }
  ...
  async checkReadiness(): Promise<AutomationRedisReadiness> {
    ...
    try {
      ...
      return { ...classifyAutomationRedisPolicy({ appendOnly, savePolicy, maxmemoryPolicy: policy }), redis: 'up' };
    } catch {
      await this.redis.del(probeKey).catch(() => undefined);
      return { status: 'degraded', redis: 'down', persistence: 'unknown', evictionPolicy: 'unknown' };
    }
  }
  ...
  private async scanKeys(pattern: string): Promise<string[]> {
    ...
    do { const result = await this.redis.scan(cursor, 'MATCH', pattern, 'COUNT', 100); cursor = result[0]; keys.push(...result[1]); } while (cursor !== '0');
    return keys;
  }
}
