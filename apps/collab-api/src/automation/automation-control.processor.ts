...
export class AutomationControlProcessor {
  ...
  async process(command: AutomationControlCommand) {
    if (command.name === 'schedule.upsert') {
      if (!this.config.enabled || !this.config.schedulesEnabled) throw new AppError(503, 'cloud_disabled', '定时自动化当前已关闭');
      return this.scheduler.upsert(command.data.schedule_id, command.data.generation);
    }
    if (command.name === 'schedule.remove') return this.scheduler.remove(command.data.schedule_id, command.data.generation);

    const run = await this.prisma.workflowRun.findUnique({
      where: { id: command.data.run_id },
      include: { attempts: true },
    });
    if (!run || run.executionTarget !== 'CLOUD' || !run.principalUserId) return { outcome: 'STALE' as const };
    if (run.scheduleGeneration !== null && command.data.generation !== 0 && run.scheduleGeneration !== command.data.generation) return { outcome: 'STALE' as const };
    ...
    if (!this.config.enabled || (run.triggerKind === 'SCHEDULE' ? !this.config.schedulesEnabled : !this.config.cloudManualEnabled)) {
      await this.abortBus.broadcast({ kind: 'KILL_SWITCH', id: '*', reason: 'cloud kill switch enabled' });
    ...
    const attempts = await this.prisma.workflowStepAttempt.findMany({
      where: { runId: run.id, status: 'RUNNING', actionInvocationId: { not: null }, transportJobId: null },
      orderBy: [{ createdAt: 'asc' }, { id: 'asc' }],
    });
    let published = 0;
    for (const attempt of attempts) {
      await this.queue.publishAction({ run_id: run.id, attempt_id: attempt.id, invocation_id: attempt.actionInvocationId!, plan_sha256: run.planSha256 });
      await this.prisma.workflowStepAttempt.updateMany({
        where: { id: attempt.id, runId: run.id, status: 'RUNNING', actionInvocationId: attempt.actionInvocationId, transportJobId: null },
