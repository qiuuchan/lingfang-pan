...
export class AutomationReadinessService {
  ...
  async check(): Promise<
    AutomationRedisReadiness & {
      processRole: AutomationConfig['processRole'];
      workers: {
        required: boolean;
        status: 'not_required' | 'ready' | 'missing';
        count: number;
  ...
  > {
    ...
    const heartbeats =
      base.redis === 'up' ? await this.queue.listWorkerHeartbeats().catch(() => []) : [];
    const latest = heartbeats.reduce<number | null>((value, item) => {
      const time = Date.parse(item.updated_at);
      return Number.isFinite(time) && (value === null || time > value) ? time : value;
    }, null);
    const workers = {
      required,
      status: !required
      ...
      latest_age_ms: latest === null ? null : Math.max(0, Date.now() - latest),
    };
    return {
      ...base,
      ...(required && workers.status !== 'ready' ? { status: 'degraded' as const } : {}),
  ...
  async metrics() {
    const [queue, outboxPending, outboxDead, activeSchedules, readyEndpoints, unhealthyEndpoints] =
      await Promise.all([
        this.check(),
        ...
        this.prisma.cloudActionDeployment.count({
          where: { status: 'READY', lastHealthOk: false },
        }),
      ]);
    return {
      queue,
      abort: this.abortBus.metrics(),
      outbox: { pending: outboxPending, dead: outboxDead },
      schedules: { active: activeSchedules },
      endpoints: { ready: readyEndpoints, unhealthy: unhealthyEndpoints },
