...
const DATASOURCE_PROVIDER_PATTERN =
  /(datasource\s+db\s*\{[\s\S]*?provider\s*=\s*")postgresql("[\s\S]*?\})/;
const MYSQL_STRING_LIST_FIELD_PATTERN =
  /^(\s*[A-Za-z_][A-Za-z0-9_]*\s+)String\[\](\s+@default\(\[\]\))?([^\r\n]*)$/gm;
const MYSQL_LONG_TEXT_FIELD_PATTERN =
  /^(\s*readmeMarkdown\s+String\s+@default\(""\))\s+@db\.Text$/gm;
...
export function renderPrismaSchemaForProvider(schema: string, provider: DatabaseProvider): string {
  ...
  let rendered = schema
    ...
    .replace(
      MYSQL_STRING_LIST_FIELD_PATTERN,
      (_match, prefix: string, defaultClause = '', suffix: string) =>
    ...
    .replace(
      MYSQL_LONG_TEXT_FIELD_PATTERN,
      (_match, prefix: string) =>
        `${prefix.replace('@default("")', '@default(dbgenerated("(\'\')"))')} @db.LongText`
    );
  for (const [pattern, replacement] of MYSQL_BOUNDED_ACTION_FIELDS)
    rendered = rendered.replace(pattern, replacement);
  return rendered;
}

export async function ensurePrismaSchemaForEnv(
