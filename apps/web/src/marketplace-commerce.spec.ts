...
describe('web marketplace commerce', () => {
  ...
  it('rejects stale non-v1 catalog tokens before any purchase request', async () => {
    ...
    expect(() =>
      purchaseWebPlugin(
        { package_id: 'package-1', price_version: 'base_old' } as PublicPluginDetail,
        fetcher
      )
    ).toThrow('市场价格版本无效');
  ...
  it('loads a campaign token and freezes it into the purchase command', async () => {
    ...
    const fetcher = vi
      ...
      .mockImplementationOnce(() =>
        response({ campaign_token: 'ct1.signed', expires_at: '2026-07-16T01:00:00.000Z' })
      )
      .mockImplementationOnce(() => response({ entitled: true }));
    const attribution = await loadCampaignAttributionToken(campaignId, packageId, fetcher);
    await purchaseWebPlugin(
      { package_id: packageId, price_version: `pv1.${'a'.repeat(43)}` } as PublicPluginDetail,
      {
    ...
    expect(fetcher.mock.calls[0][0]).toBe(
      `/api/marketplace/campaigns/${campaignId}/items/${packageId}/attribution-token`
    );
