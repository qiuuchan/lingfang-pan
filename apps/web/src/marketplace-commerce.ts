...
const ObjectResponse = {
  parse(value: unknown) {
    if (!value || typeof value !== 'object' || Array.isArray(value)) throw new Error('市场响应无效');
...
export function purchaseWebPlugin(
  ...
  if (!/^pv1\.[A-Za-z0-9_-]{43}$/.test(detail.price_version)) throw new Error('市场价格版本无效，请刷新后重试');
...
export function submitQualityAppeal(packageId: string, body: string, fetchImplementation?: FetchImplementation) {
  return requestJson(
...
export async function loadWebOrders(fetchImplementation?: FetchImplementation) {
  const result = await requestJson('/api/web/plugins/orders/current?pageSize=50', MarketplaceOrderPage, {}, fetchImplementation);
  return result.items;
}
