...
import type {
  MarketplaceDiscoveryHome as DiscoveryHome,
  MarketplaceOrderListItem,
  MarketplaceOwnerQuality,
  PublicPluginCard as Card,
  PublicPluginDetail as Detail,
...
function Catalog() {
  ...
  useEffect(() => {
    Promise.all([loadCatalog(), loadDiscoveryHome()])
      .then(([page, discovery]) => {
        setData(page);
        setHome(discovery);
      })
  ...
  const sections = home
    ...
    : [];
  const filter = (items: Card[]) =>
    items.filter(
      (item) =>
        (!query.trim() ||
          `${item.name} ${item.summary} ${item.category}`
            .toLowerCase()
            .includes(query.trim().toLowerCase())) &&
        (runtime === 'ALL' || item.runtime_type === runtime)
    );
  return (
    <main>
      <header>
        <span>LINGFANG</span>
        <h1>插件中心</h1>
        <p>发现经过审核、可追溯版本的团队插件。</p>
        <a href="/orders">我的订单</a>
      </header>
      <section className="catalog-filters" aria-label="目录筛选">
        <input
          aria-label="搜索插件"
      ...
      {sections.map((section) => (
        <section className="catalog-section" key={section.key}>
          <h2>{section.title}</h2>
          <div className="grid">
            {filter(section.items).map((item) => (
              <PluginCard item={item} key={item.package_id} />
            ))}
          </div>
        </section>
      ))}
...
function PluginCard({ item }: { item: Card }) {
  const tier = ({ LISTED: '已上架', QUALITY: '优质', FEATURED: '精选' } as const)[
    item.quality_tier
  ];
  return (
    <a className="card" href={`/plugins/${item.package_id}`}>
      <div className="tags">
        <b>{tier}</b>
      ...
      <small>
        v{item.version} · 安装 {item.install_count} ·{' '}
        {formatPrice(item.effective_price_cents ?? item.base_price_cents)}
      </small>
    </a>
  );
}
...
function PreviewPage({ item }: { item: Detail }) {
  ...
  return (
    <section className="preview" data-mode={item.preview_mode}>
      <h2>仅支持桌面运行</h2>
      <p>该插件需要 {item.runtime_type} 本地运行时，Web 端不会执行代码或伪造输出。</p>
      <a className="button" href={`lingfang://plugins/${encodeURIComponent(item.package_id)}`}>
        在桌面端打开
      </a>
    </section>
  );
...
function DetailPage({ id, preview }: { id: string; preview: boolean }) {
  ...
  useEffect(() => {
    loadPluginDetail(id)
      ...
      .catch((cause) => setError(cause instanceof Error ? cause.message : String(cause)));
    loadOwnerQuality(id)
      .then(setOwnerQuality)
      .catch((cause) => {
        if (!(cause instanceof WebApiError) || ![401, 403, 404].includes(cause.status))
  ...
  if (!item)
    return (
      <main>
        <p aria-live="polite">加载插件信息…</p>
      </main>
    );
  if (preview)
    return (
      <main>
        <a href={`/plugins/${id}`}>← 返回详情</a>
        <header>
          <h1>{item.name} · 在线预览</h1>
          <p>根据发行版 runtime 使用真实 Cloud Trial、安全浏览器沙箱或桌面承接。</p>
        </header>
        <PreviewPage item={item} />
      </main>
    );
  const detail = item;
  ...
  async function acquire() {
    ...
    try {
      ...
      setPurchaseState(
        cause instanceof Error ? cause.message : price === 0 ? '获取失败' : '购买失败'
      );
    }
  }
  return (
    <main>
      <a href="/plugins">← 返回插件中心</a>
      <header>
        <div className="tags">
          <b>{item.quality_tier}</b>
          <span>{item.preview_mode}</span>
          {campaignId && <span>Campaign</span>}
        </div>
        <h1>{item.name}</h1>
        <p>{item.summary}</p>
        <div className="actions">
          <a className="button" href={`/plugins/${id}/preview${location.search}`}>
            {item.preview_mode === 'STATIC_DESKTOP' ? '查看运行方式' : '查看在线预览'}
          </a>
          <button
            className="button"
            disabled={purchaseState.endsWith('中…')}
            onClick={() => void acquire()}
          >
            {price === 0 ? '免费获取' : `购买 ${formatPrice(price)}`}
          </button>
        </div>
        {purchaseState && <p aria-live="polite">{purchaseState}</p>}
      </header>
      <div className="layout">
        <article>
          <Markdown source={item.readme_markdown} />
        </article>
        <aside>
          <h3>价格</h3>
          ...
          <p>{item.version}</p>
          <h3>运行时</h3>
          <p>{item.runtime_type}</p>
          <h3>兼容性</h3>
          <p>{item.compatibility.web_compatible ? '支持 Web' : '需要桌面端'}</p>
          {item.preview_actions.length > 0 && (
            <>
              <h3>可试跑 Action</h3>
              <p>{item.preview_actions.map((action) => action.name).join('、')}</p>
            </>
          )}
        </aside>
      </div>
      {ownerQuality && (
        <OwnerQualityPanel
          ...
          onAppeal={(body) => submitQualityAppeal(id, body).then(() => undefined)}
        />
      )}
    </main>
  );
      ...
      <section className="orders">
        {items.map((item) => (
          <article className="card" key={item.id}>
            ...
            <p>
              {formatPrice(item.price_cents)} · {orderStatus(item.status)}
            </p>
            <small>
              {formatDate(item.created_at)}
              {item.refundable_until ? ` · 退款申请截止 ${formatDate(item.refundable_until)}` : ''}
            </small>
          </article>
        ))}
        {!error && items.length === 0 && <p>当前团队暂无插件订单。</p>}
      </section>
    </main>
  );
}
...
function App() {
  ...
  return (
    <>
      ...
      {parts[0] === 'orders' ? (
        <OrdersPage />
      ) : parts[0] !== 'plugins' || !parts[1] ? (
        <Catalog />
      ) : (
        <DetailPage id={parts[1]} preview={parts[2] === 'preview'} />
      )}
    </>
  );
}
