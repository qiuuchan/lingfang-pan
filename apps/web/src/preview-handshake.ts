export type PreviewHandshakeSession = {
  sessionId: string;
  nonce: string;
  expiresAt: number;
  consumed: boolean;
...
export function validatePreviewHandshake(
  session: PreviewHandshakeSession,
  event: PreviewHandshakeEvent,
  ...
  if (
    payload.type !== 'lingfang.preview.handshake.v1' ||
...
export function acceptPreviewHandshake(
  session: PreviewHandshakeSession,
  event: PreviewHandshakeEvent,
  iframeWindow: unknown,
  now = Date.now()
...
export function previewIsolationReportSafe(value: unknown): value is PreviewIsolationReport {
  ...
  const keys = [
    'parent_dom_readable',
    'parent_storage_readable',
    'parent_token_readable',
...
export function previewCapabilityAllowed(capability: string): boolean {
  return PREVIEW_SAFE_CAPABILITIES.has(capability);
}
