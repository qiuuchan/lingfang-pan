...
export function ClientSandboxPreview({ detail }: { detail: PublicPluginDetail }) {
  ...
  useEffect(() => {
    ...
    void prepareWebSession()
      ...
      .catch((cause) => {
        if (!active) return;
        setError(cause instanceof Error ? cause.message : '无法创建 Client 预览会话');
        setStatus('ERROR');
      });
    return () => {
      active = false;
  ...
  useEffect(() => {
    const listener = (event: MessageEvent) => {
      ...
      if (
        !state ||
        !frameWindow ||
        consumeInFlight.current ||
        !validatePreviewHandshake(state, event, frameWindow)
      )
        return;
      ...
      void consumeClientPreviewSession(state.sessionId, state.nonce)
        .then(() => {
          ...
          (event.source as Window).postMessage(
            { type: 'lingfang.preview.ready.v1', session_id: state.sessionId },
            '*',
            [channel.port2]
          );
          setStatus('READY');
        ...
        .catch((cause) => {
          setError(cause instanceof Error ? cause.message : '预览握手失败');
          setStatus('ERROR');
        })
        .finally(() => {
  ...
  try {
    if (session) src = buildPreviewIframeUrl(previewOrigin(), session, window.location.origin);
  } catch (cause) {
    configurationError = cause instanceof Error ? cause.message : '预览源配置无效';
  }
  ...
  return (
    <section
      className="preview client-preview"
      data-mode="CLIENT_SANDBOX"
      data-preview-status={visibleStatus}
    >
      <div className="trial-heading">
        <div>
          <p className="eyebrow">Opaque Client Sandbox</p>
          <h2>浏览器沙箱预览</h2>
        </div>
        <span className={`status status-${visibleStatus.toLowerCase()}`} aria-live="polite">
          {previewStatus(visibleStatus)}
        </span>
      </div>
      <p>
        发行版在独立预览源运行；沙箱不包含 <code>allow-same-origin</code>，不会收到主站 Cookie
        或认证 token。
      </p>
      {visibleError && (
      ...
      {report && (
        <p className="isolation-ok" role="status">
          已验证父页面 DOM、storage、token 与预览源存储均不可读。
        </p>
      )}
    </section>
  );
...
export function previewOrigin(
  configured = import.meta.env.VITE_PLUGIN_PREVIEW_ORIGIN as string | undefined
): string {
...
export function buildPreviewIframeUrl(
  ...
  if (preview.origin === new URL(parentOrigin).origin)
    throw new Error('Client 预览源必须与 Web 主站 origin 分离');
  preview.pathname = `/sessions/${encodeURIComponent(session.session_id)}/index.html`;
  preview.hash = new URLSearchParams({
    session_id: session.session_id,
    nonce: session.channel_nonce,
  }).toString();
...
function bindPreviewCapabilities(port: MessagePort, frame: HTMLIFrameElement): void {
  port.onmessage = (event) => {
    ...
    if (
      request.type !== 'lingfang.preview.capability.v1' ||
      typeof request.request_id !== 'string' ||
      typeof request.capability !== 'string'
    )
      return;
    if (!previewCapabilityAllowed(request.capability)) {
      reply(port, request.request_id, false, undefined, {
        code: 'preview_capability_denied',
        message: '该能力不可在 Web 预览中使用',
      });
      return;
    }
    if (request.capability === 'ui.view') {
      reply(port, request.request_id, true, {
...
function reply(
  ...
  port.postMessage({
    type: 'lingfang.preview.capability.result.v1',
