...
export function createClientPreviewSession(
  packageId: string,
  fetchImplementation?: FetchImplementation
): Promise<PreviewSession> {
  return requestJson(
    `/api/web/plugin-preview/${encodeURIComponent(packageId)}/sessions`,
    WebPreviewSession,
    { method: 'POST' },
    fetchImplementation
  );
...
export function consumeClientPreviewSession(
  sessionId: string,
  nonce: string,
  fetchImplementation?: FetchImplementation
) {
  return requestJson(
    `/api/web/plugin-preview/sessions/${encodeURIComponent(sessionId)}/consume`,
    ConsumeResponse,
    {
      method: 'POST',
