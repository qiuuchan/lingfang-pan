...
export function CloudTrialPanel({ detail }: { detail: PublicPluginDetail }) {
  ...
  const action =
    detail.preview_actions.find((candidate) => candidate.action_id === actionId) ?? null;
  const [inputSource, setInputSource] = useState(() =>
    JSON.stringify(action ? defaultTrialInput(action.input_schema) : {}, null, 2)
  );
    ...
    return (
      <section className="trial-panel">
        ...
        <div className="notice">
          当前发行版没有可公开试跑的只读或幂等 Action，请返回详情刷新版本信息。
        </div>
      </section>
    );
  }
  ...
  return (
    <section className="trial-panel" data-mode="CLOUD_TRIAL" aria-labelledby="cloud-trial-title">
      <div className="trial-heading">
        <div>
          <p className="eyebrow">PREVIEW · 隔离执行</p>
          <h2 id="cloud-trial-title">Cloud 安全试用</h2>
        </div>
        {trial && (
          <span className={`status status-${trial.status.toLowerCase()}`} aria-live="polite">
      ...
      <label className="field">
        ...
        >
          {detail.preview_actions.map((candidate) => (
      ...
      <div className="trial-actions">
        <button className="button" type="button" onClick={start} disabled={active || busy}>
          {busy && !trial ? '正在提交…' : '开始真实试跑'}
        </button>
        {active && (
          <button
    ...
    <div className="trial-result">
      <dl className="trial-meta">
        <div>
          <dt>Invocation</dt>
          <dd>
            <code>{trial.invocation_id}</code>
          </dd>
        </div>
        <div>
          <dt>今日剩余</dt>
          <dd>
            {trial.quota_remaining} / {trial.daily_limit}
          </dd>
        </div>
        <div>
          <dt>并发占用</dt>
          <dd>
            {trial.concurrent_active} / {trial.concurrency_limit}
          </dd>
        </div>
        <div>
          <dt>过期时间</dt>
          <dd>{formatTime(trial.expires_at)}</dd>
        </div>
      </dl>
      {trial.output && (
        <div>
          ...
          <pre className="json-output">
            <code>{JSON.stringify(trial.output, null, 2)}</code>
          </pre>
        </div>
      )}
      {trial.error && (
      ...
      {!trial.output && !trial.error && !isCloudTrialTerminal(trial) && (
        <p className="running-note">页面正在轮询统一 invocation 状态，终态前不会生成占位结果。</p>
      )}
    </div>
  );
}
