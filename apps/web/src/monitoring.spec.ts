...
describe('web 全局错误上报 (monitoring)', () => {
  it('正向：VITE_SENTRY_DSN 配置时，reportError 调用 Sentry.captureException', async () => {
    vi.stubEnv('VITE_SENTRY_DSN', 'https://example@sentry.io/2');
  ...
  it('ErrorBoundary.componentDidCatch 将错误路由到 reportError（DSN 缺失→console 兜底可见）', async () => {
    ...
    boundary.componentDidCatch(err, { componentStack: 'at App' });
    // componentDidCatch → reportError → DSN 缺失时 console.error 兜底可见，
    // 且携带 react 阶段标记（证明确实走了上报路径而非静默丢弃）。
    expect(errorSpy).toHaveBeenCalled();
