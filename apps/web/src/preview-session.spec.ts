...
describe('Client preview session client', () => {
  ...
  it('creates and consumes the one-time nonce only through the CSRF cookie session', async () => {
    const fetcher = vi.fn()
      ...
      .mockImplementationOnce(() => response({ ok: true, session_id: '11111111-1111-4111-8111-111111111111', mode: 'CLIENT_SANDBOX' }));
    await prepareWebSession(fetcher);
    const session = await createClientPreviewSession('33333333-3333-4333-8333-333333333333', fetcher);
