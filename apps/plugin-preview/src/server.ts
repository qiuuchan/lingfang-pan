...
export function createPreviewOriginServer(input: PreviewOriginConfig) {
  ...
  return createServer((request, response) => {
    void handleRequest(config, request, response).catch(() => sendError(response, 502, 'preview_upstream_failed'));
  });
}

async function handleRequest(config: Required<Pick<PreviewOriginConfig, 'internalOrigin' | 'serviceKey' | 'webAppOrigins' | 'fetchImplementation'>> & Pick<PreviewOriginConfig, 'publicOrigin'>, request: IncomingMessage, response: ServerResponse) {
  ...
  if (url.pathname === '/health') return send(response, 200, 'application/json; charset=utf-8', Buffer.from('{"ok":true}'), request.method === 'HEAD');
  if (url.pathname === '/_lingfang/bridge.js') return send(response, 200, 'text/javascript; charset=utf-8', Buffer.from(PREVIEW_BRIDGE_SOURCE), request.method === 'HEAD');
  ...
  const upstreamUrl = new URL(`/api/internal/plugin-preview/sessions/${sessionId}/asset`, config.internalOrigin);
  ...
  if (!upstream.ok) return sendError(response, upstream.status >= 400 && upstream.status < 500 ? upstream.status : 502, 'preview_resource_unavailable');
  ...
  const output = isEntry && contentType.toLowerCase().startsWith('text/html')
    ? injectPreviewBootstrap(body.toString('utf8'), sessionId, entryPath)
    : body;
  if (isEntry) response.setHeader('content-security-policy', contentSecurityPolicy(config.webAppOrigins, output.toString('utf8')));
...
function validateConfig(input: PreviewOriginConfig) {
  if (!input.serviceKey || input.serviceKey.length < 32) throw new Error('PLUGIN_PREVIEW_SERVICE_KEY must contain at least 32 characters');
  const internalOrigin = exactOrigin(input.internalOrigin, 'COLLAB_API_INTERNAL_ORIGIN');
  const webAppOrigins = [...new Set(input.webAppOrigins.map((origin) => exactOrigin(origin, 'PREVIEW_WEB_APP_ORIGINS')))];
  if (webAppOrigins.length === 0) throw new Error('PREVIEW_WEB_APP_ORIGINS must contain at least one origin');
  const publicOrigin = input.publicOrigin ? exactOrigin(input.publicOrigin, 'PLUGIN_PREVIEW_PUBLIC_ORIGIN') : undefined;
  if (publicOrigin && webAppOrigins.includes(publicOrigin)) throw new Error('plugin preview origin must differ from every Web app origin');
  return { ...input, internalOrigin, webAppOrigins, publicOrigin, fetchImplementation: input.fetchImplementation || fetch };
}
...
function decodeHeaderPath(value: string | null): string {
  ...
  try { return decodeURIComponent(value); } catch { return 'index.html'; }
}
...
function inlineScriptHashes(html: string): string[] {
  ...
  while ((match = expression.exec(html))) {
    values.add(`'sha256-${createHash('sha256').update(match[1] || '').digest('base64')}'`);
  }
  return [...values];
}
...
function applySecurityHeaders(response: ServerResponse, webAppOrigins: string[]): void {
  ...
  response.setHeader('permissions-policy', 'camera=(), microphone=(), geolocation=(), clipboard-read=(), clipboard-write=(), payment=(), usb=(), serial=(), fullscreen=()');
...
function sendError(response: ServerResponse, status: number, code: string): void {
  send(response, status, 'application/json; charset=utf-8', Buffer.from(JSON.stringify({ code })), false);
}

function send(response: ServerResponse, status: number, contentType: string, body: Buffer, head: boolean): void {
