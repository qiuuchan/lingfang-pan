...
export function NotificationCenter({
  ...
  const markAllRead = useCallback(async () => {
    ...
    try {
      ...
      setData((prev) =>
        prev
          ? { notifications: prev.notifications.map((n) => ({ ...n, read: true })), unreadCount: 0 }
          : prev
      );
    } catch (e) {
...
function NotificationRow({ item, onRead }: { item: NotificationItem; onRead: () => void }) {
  ...
  return (
    <div
      className={cn(
        'group flex gap-3.5 px-4 py-3.5 transition-colors hover:bg-muted/35',
        ...
        <div className="mt-2 flex items-center justify-between gap-3">
          ...
          {!item.read && (
            <Button
              ...
              size="xs"
              onClick={onRead}
              className="h-6 px-2 text-[10px] text-primary opacity-80 hover:text-primary group-hover:opacity-100"
            >
