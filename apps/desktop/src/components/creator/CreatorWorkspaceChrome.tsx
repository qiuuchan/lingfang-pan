      ...
      {QUICK_TAGS.map((tag) => {
        ...
        return (
          <button
            ...
            className="inline-flex h-8 items-center gap-1.5 rounded-full border border-border/70 bg-card px-3 text-xs font-medium text-muted-foreground shadow-sm transition-colors hover:border-primary/40 hover:bg-muted hover:text-foreground"
          >
...
export function CreatorWorkspaceSidebar({
  ...
  return (
    <aside className="flex w-64 shrink-0 flex-col border-r bg-sidebar/70 text-sidebar-foreground">
      <div className="flex items-center justify-between border-b px-3 py-3">
        <div className="flex min-w-0 items-center gap-2">
          <span className="flex size-8 shrink-0 items-center justify-center rounded-lg border bg-background/80">
            <SparklesIcon className="size-4 text-primary" />
          </span>
      ...
      <div className="flex-1 space-y-4 overflow-y-auto p-3">
        <div className="space-y-1.5">
          <Button variant="default" size="sm" className="w-full justify-start gap-2" disabled={busy} onClick={onNewConversation}>
            <PlusIcon className="size-3.5" />
            新建对话
            {compressedHint > 0 && <Badge variant="outline" className="ml-auto h-4 px-1 text-[10px]">压缩 {compressedHint}</Badge>}
          </Button>
          <Button variant="ghost" size="sm" className="w-full justify-start gap-2" onClick={onOpenSkills}>
            <WrenchIcon className="size-3.5" />
            技能
            {activeSkillCount > 0 && <Badge variant="secondary" className="ml-auto h-4 px-1 text-[10px]">{activeSkillCount}</Badge>}
          </Button>
          {recentPlugins.length > 0 && (
            <Popover>
              <PopoverTrigger render={<Button type="button" variant={referencedPlugin ? 'default' : 'ghost'} size="sm" className="w-full justify-start gap-2" />}>
                <FileCode2Icon className="size-3.5" />
        ...
        <div className="space-y-2">
          <div className="flex items-center justify-between px-2 text-[11px] font-medium text-muted-foreground">
            ...
            {conversations.length > 0 && <Badge variant="secondary" className="h-4 px-1 text-[10px]">{conversations.length}</Badge>}
          </div>
              ...
              return (
                <div
                  ...
                  className={cn(
                    ...
                    active ? 'border-primary/40 bg-primary/10 text-primary' : 'border-transparent hover:border-border hover:bg-muted/60',
                  )}
                >
                  <button
                    type="button"
                  ...
                  {confirming ? (
                    ...
                    <button
                      ...
                      className="inline-flex size-6 shrink-0 items-center justify-center rounded-md text-muted-foreground opacity-0 transition-all hover:bg-destructive/10 hover:text-destructive group-hover:opacity-100"
                    >
