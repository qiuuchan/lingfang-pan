...
export function CreatorWorkspaceSidebar({
  ...
  return (
    ...
    >
      ...
      {/* 历史列表：展开态显示完整条目，折叠态以首字符圆圈列表呈现（与主侧栏插件项一致）。 */}
      <div className="flex-1 space-y-1 overflow-y-auto p-2">
        {!collapsed && conversations.length > 0 && (
          <div className="flex items-center justify-between px-1 text-[11px] font-medium text-muted-foreground">
            ...
            <Badge variant="secondary" className="h-4 bg-accent px-1 text-[10px] text-primary">{conversations.length}</Badge>
          </div>
        )}
        {conversations.length ? conversations.slice(0, collapsed ? 8 : 12).map((conversation) => {
