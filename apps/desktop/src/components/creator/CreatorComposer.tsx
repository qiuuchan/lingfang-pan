...
const CONTROL_CLASS =
  'inline-flex h-8 items-center justify-center gap-1.5 rounded-md px-2 text-xs font-medium text-muted-foreground transition-colors hover:bg-accent hover:text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring disabled:pointer-events-none disabled:opacity-45';
...
export function CreatorComposer({
  ...
  const hasContextChips =
    selectedFiles.length > 0 || referencedPlugin != null || workspacePluginId != null;
          ...
          {hasContextChips && (
            <div className="flex flex-wrap items-center gap-1.5 border-b border-border/70 px-3 py-2">
              {workspacePluginId && (
                <div className="inline-flex min-w-0 max-w-full items-center gap-1 rounded-md bg-muted px-2 py-1 text-xs text-muted-foreground">
                  <button
                    ...
                    title={workspacePath ?? workspacePluginId}
                  >
              ...
              {selectedFiles.slice(0, 4).map((file) => (
                <div
                  key={file.id}
                  className="inline-flex min-w-0 items-center gap-1 rounded-md bg-muted px-2 py-1 text-xs text-muted-foreground"
                >
            ...
            <div className="flex min-w-0 items-center gap-1">
              <Popover open={moreOpen} onOpenChange={setMoreOpen}>
                ...
                <PopoverContent className="w-72 p-2" align="start" side="top" sideOffset={8}>
                  ...
                  <MenuAction icon={PaperclipIcon} onClick={() => runMoreAction(onPickFiles)}>
                    选择文件
                  </MenuAction>
                  <MenuAction icon={FolderOpenIcon} onClick={() => runMoreAction(onPickFolder)}>
                    选择文件夹
                  </MenuAction>
                  {selectedFiles.length > 0 && (
                    <>
                      <MenuAction
                        icon={PackageIcon}
                        disabled={busy}
                        onClick={() => runMoreAction(onImportFiles)}
                      >
                        将已选文件导入为插件
                      </MenuAction>
                      <MenuAction icon={XIcon} onClick={() => runMoreAction(onClearFiles)}>
                        清空已选文件
                      </MenuAction>
                    </>
                  )}
                  <MenuAction icon={SparklesIcon} onClick={() => runMoreAction(onOpenSkills)}>
                    技能（已启用 {activeSkillCount} 个）
                  </MenuAction>
                  <MenuAction
                    icon={WandSparklesIcon}
                    disabled={busy || optimizingPrompt || !input.trim()}
                    onClick={() => runMoreAction(onOptimizePrompt)}
                  >
                  ...
                  <MenuAction
                    icon={MicIcon}
                    disabled={busy}
                    onClick={() => runMoreAction(onToggleVoice)}
                  >
                  ...
                  {workspacePluginId && (
                    <>
                      ...
                      <MenuAction
                        icon={FolderOpenIcon}
                        onClick={() => runMoreAction(onOpenWorkspace)}
                      >
                        打开插件工作区
                      </MenuAction>
                      <MenuAction
                        icon={RefreshCwIcon}
                        onClick={() => runMoreAction(onRefreshWorkspace)}
                      >
                        刷新插件工作区
                      </MenuAction>
                    </>
                        ...
                        {recentPlugins.map((plugin) => {
                          ...
                          return (
                            <button
                              ...
                              onClick={() =>
                                runMoreAction(() =>
                                  onSelectReferencedPlugin(active ? null : plugin)
                                )
                              }
                              className={cn(
                                'flex w-full items-center gap-2 rounded-md px-2.5 py-2 text-left text-sm transition-colors hover:bg-accent',
            ...
            <div className="flex shrink-0 items-center gap-1">
              {canInspectContext && contextUsagePct != null && (
                <button
                  ...
                  className={cn(CONTROL_CLASS, contextUsagePct >= 80 && 'text-warning')}
                  title={`查看上下文详情${contextUsageLabel ? ` · ${contextUsageLabel}` : ''}${compressHint ? ` · ${compressHint}` : ''}`}
                >
                  <GaugeIcon className="size-3.5" />
                  <span className="font-mono tabular-nums">
                    {contextUsagePct === 0 ? '<1%' : `${contextUsagePct}%`}
                  </span>
                </button>
              )}
              {busy ? (
                <button
                  ...
                  title="停止"
                  aria-label="停止"
                  className="inline-flex size-9 items-center justify-center rounded-md bg-foreground text-background transition-opacity hover:opacity-85 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
                >
                ...
                <button
                  ...
                  title="发送"
                  aria-label="发送"
                  className="inline-flex size-9 items-center justify-center rounded-md bg-primary text-primary-foreground transition-colors hover:bg-primary/90 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring disabled:pointer-events-none disabled:opacity-40"
                >
