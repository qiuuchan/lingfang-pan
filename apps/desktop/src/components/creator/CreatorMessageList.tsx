      ...
      <div ref={scrollRef as React.Ref<HTMLDivElement>} className="min-h-0 flex-1 overflow-y-auto px-4 py-8 [scrollbar-gutter:stable] sm:px-7 sm:py-10">
        <div className={cn(CREATOR_COLUMN_CLASS, 'flex flex-col gap-7')}>
          {turns.map((turn, turnIndex) => {
            ...
            if (turn.role === 'user') {
              return (
                <div key={turnIndex} className="flex justify-end animate-in fade-in slide-in-from-bottom-2 duration-300">
            ...
            return (
              <div key={turnIndex} className="flex w-full items-start gap-3 animate-in fade-in slide-in-from-bottom-2 duration-300">
                    ...
                    if (part.type === 'text') {
                      ...
                      return (
                        <div key={`text-${partIndex}`} className="creator-assistant-bubble group relative py-0.5 pr-8 text-[15px] leading-7 text-foreground">
                          ...
                          <div className="break-words"><Markdown>{part.content}</Markdown></div>
                        </div>
                      );
                    }
