...
import { Building2Icon, KeyRoundIcon, LogOutIcon, MailIcon, ShieldCheckIcon, UserRoundIcon } from 'lucide-react';
    ...
    <div className="mx-auto flex max-w-lg flex-col gap-4">
      ...
      <div className="flex items-center justify-between gap-4 rounded-xl border border-destructive/15 bg-destructive/5 p-4">
        <div className="min-w-0">
          ...
          <p className="mt-0.5 text-xs leading-relaxed text-muted-foreground">本机登录状态将被清除，不会删除你的账户数据。</p>
        </div>
        <Button variant="destructive" className="shrink-0" onClick={() => { resetSession(); onClose(); }}>
          <LogOutIcon />退出登录
        </Button>
      </div>
    </div>
    ...
    <div className="overflow-hidden rounded-xl border bg-card shadow-sm">
      <div className="flex items-center gap-4 border-b bg-muted/30 p-5">
        ...
        <span className="inline-flex shrink-0 items-center gap-1.5 rounded-full border bg-background px-2.5 py-1 text-xs font-medium text-foreground shadow-sm">
          <ShieldCheckIcon className="size-3.5 text-primary" />{roleLabel}
        </span>
      </div>
      <div className="grid gap-px bg-border sm:grid-cols-2">
        <SummaryItem icon={<Building2Icon />} label="当前团队" value={session.tenantName || '暂未加入团队'} />
...
function SummaryItem({ icon, label, value, mono = false }: { icon: ReactNode; label: string; value: string; mono?: boolean }) {
  return (
    <div className="flex min-w-0 items-center gap-3 bg-card px-4 py-3">
      <span className="flex size-8 shrink-0 items-center justify-center rounded-full bg-muted text-muted-foreground [&_svg]:size-4">{icon}</span>
      <div className="min-w-0">
        <p className="text-[11px] text-muted-foreground">{label}</p>
        <p className={`truncate text-xs font-medium ${mono ? 'font-mono' : ''}`} title={value}>{value}</p>
      </div>
    </div>
  );
}
...
function ProfileForm({
  ...
  return (
    <section className="rounded-xl border bg-card p-4 shadow-sm">
      <SectionHeading icon={<UserRoundIcon />} title="基本资料" description="这些信息会展示在你的团队空间中。" />
      <div className="mt-4 grid gap-3">
        <div className="grid gap-1.5">
          <Label htmlFor="account-name">昵称</Label>
          <Input id="account-name" className="h-9" value={displayName} onChange={(event) => onDisplayNameChange(event.target.value)} placeholder="显示名称" />
        </div>
        <div className="grid gap-1.5">
          <Label htmlFor="account-email">邮箱</Label>
          <Input id="account-email" className="h-9" type="email" value={email} onChange={(event) => onEmailChange(event.target.value)} placeholder="name@example.com" />
        </div>
...
function PasswordForm({
  ...
  return (
    <section className="rounded-xl border bg-card p-4 shadow-sm">
      <SectionHeading icon={<KeyRoundIcon />} title="登录密码" description="建议使用至少 8 位且不易猜测的新密码。" />
      <div className="mt-4 grid gap-3">
        <div className="grid gap-1.5">
          <Label htmlFor="account-password">新密码</Label>
          <Input id="account-password" className="h-9" type="password" autoComplete="new-password" value={password} onChange={(event) => onPasswordChange(event.target.value)} placeholder="输入至少 8 位新密码" />
        </div>
        <LoadingButton variant="outline" className="justify-self-end" loading={saving} onClick={onSave}>
          {saving ? '重置中…' : '更新密码'}
        </LoadingButton>
      </div>
    </section>
