// CliRuntimeTab.tsx — 设置页 Tab1：脚本运行环境管理。
//
// 职责：
// - 渲染 2 行（nodejs/python），每行展示
...
import { RUNTIME_META } from '@/lib/install-cli';
import { dragRegionProps } from '@/lib/window-drag';
import type { InstallTarget, ProbeResult, RuntimeInstallTarget } from '@/lib/cli-types';
    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="flex items-center justify-between gap-2">
        <div className="text-sm text-muted-foreground">
          自动检测本机已安装的 Node.js 与 Python，未安装时可一键自动安装（仅 Windows）。
        </div>
...
function InstallConfirmDialog({
  ...
  const meta = target ? RUNTIME_META[target as RuntimeInstallTarget] : null;
