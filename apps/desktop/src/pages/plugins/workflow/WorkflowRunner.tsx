...
const SESSION_DIAGNOSTICS = new Set(['workflow_executor_session_invalid', 'workflow_inventory_changed', 'workflow_installation_mismatch']);
...
function statusLabel(status: string): string {
  return ({ PENDING: '准备中', RUNNING: '运行中', FAILING: '失败收口中', SUCCEEDED: '已完成', FAILED: '失败', CANCELING: '取消中', CANCELED: '已取消' } as Record<string, string>)[status] ?? status;
}
    ...
    <Card size="sm" className={selected ? 'ring-2 ring-primary' : available ? 'ring-success/30' : undefined}>
      ...
      <CardContent className="space-y-2">
        <div className="flex flex-wrap items-center gap-2">
          <Badge variant={available ? 'default' : 'secondary'}>{available ? '可用' : '不可用'}</Badge>
          {selected && <Badge variant="outline">当前选择</Badge>}
        </div>
        <p className="text-xs leading-relaxed text-muted-foreground">{reason}</p>
        {onSelect && <Button type="button" variant={selected ? 'secondary' : 'outline'} size="sm" className="w-full" disabled={disabled || selected || !available} onClick={onSelect}>{selected ? '已选择' : `选择${title}`}</Button>}
      </CardContent>
    </Card>
  );
}

function DefinitionError({ diagnostics }: { diagnostics: Array<{ code: string; path: string; message: string }> }) {
  return (
...
export function WorkflowRunner({ plugin, onReady, onAdoptedDraft }: { plugin: LoadedPlugin; onReady: () => void; onAdoptedDraft: (draft: LoadedPlugin) => void }) {
  const decoded = useMemo(() => decodeWorkflowDefinition(plugin.files, plugin.entry), [plugin.entry, plugin.files]);
...
function RunnableWorkflow({ plugin, decoded, onAdoptedDraft }: { plugin: LoadedPlugin; decoded: Extract<ReturnType<typeof decodeWorkflowDefinition>, { ok: true }>; onAdoptedDraft: (draft: LoadedPlugin) => void }) {
  ...
  useEffect(() => {
    if (!plugin.releaseId) { setUpgradeSuggestions([]); return; }
    let active = true;
    void getWorkflowUpgradeSuggestions(plugin.releaseId)
      .then((response) => { if (active) setUpgradeSuggestions(response.suggestions); })
      .catch(() => { if (active) setUpgradeSuggestions([]); });
    return () => { active = false; };
  }, [plugin.releaseId]);
  ...
  useEffect(() => {
    ...
    let active = true;
    void createDesktopWorkflowSession().then(() => active && setDesktopSessionReady(true)).catch((caught) => active && setPreflightError(errorMessage(caught, '桌面执行器 session 创建失败')));
    return () => { active = false; void revokeDesktopWorkflowSession().catch(() => undefined); };
  }, [exactIdentity, executionTarget, support.desktop.available]);

  const refreshPreflight = useCallback(async () => {
    if (!plugin.releaseId || !plugin.releaseSha256 || !selectedSupport.available || executionTarget === 'DESKTOP' && !desktopSessionReady) return;
    setPreflightLoading(true);
    setPreflightError('');
    try {
      setPreflight(await preflightWorkflowRun({
  ...
  }, [desktopSessionReady, executionTarget, input, plugin.releaseId, plugin.releaseSha256, selectedSupport.available]);

  useEffect(() => {
    if (!exactIdentity || !selectedSupport.available || executionTarget === 'DESKTOP' && !desktopSessionReady) return;
    const timer = window.setTimeout(() => { void refreshPreflight(); }, 350);
    return () => window.clearTimeout(timer);
  }, [desktopSessionReady, exactIdentity, executionTarget, refreshPreflight, selectedSupport.available]);

  const refreshRun = useCallback(async () => {
    if (!run || pollInFlight.current) return;
    pollInFlight.current = true;
    try { setRun(await getWorkflowRun(run.id)); }
    catch (caught) { setPreflightError(errorMessage(caught, '刷新工作流运行失败')); }
  ...
  useEffect(() => {
    ...
    void drive();
    const timer = window.setInterval(() => { void drive(); }, 750);
  ...
  useEffect(() => {
    if (executionTarget !== 'DESKTOP' || !desktopSessionReady || run && TERMINAL_RUN_STATUSES.has(run.status)) return;
    const timer = window.setInterval(() => {
      void heartbeatDesktopWorkflowSession().catch((caught) => setPreflightError(errorMessage(caught, '桌面执行器 heartbeat 失败')));
    }, 20_000);
    return () => window.clearInterval(timer);
  }, [desktopSessionReady, executionTarget, run]);

  useEffect(() => {
    if (!run || !TERMINAL_RUN_STATUSES.has(run.status) || run.execution_target !== 'DESKTOP') return;
  ...
  const start = async () => {
    ...
    if (!selectedSupport.available || !plugin.releaseId || !plugin.releaseSha256 || executionTarget === 'DESKTOP' && !desktopSessionReady) return;
    setStarting(true);
    setPreflightError('');
    try {
  ...
  const cancel = async () => {
    if (!run) return;
    setCanceling(true);
    try { setRun(await cancelWorkflowRun(run.id)); toast.success('已请求取消工作流'); }
    catch (caught) { toast.error(errorMessage(caught, '取消工作流失败')); }
    ...
    <div className="min-h-0 flex-1 overflow-y-auto bg-muted/30 p-4 sm:p-6">
      <div className="mx-auto max-w-6xl space-y-5">
        <section className="flex flex-col gap-3 rounded-xl border bg-background p-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex min-w-0 items-start gap-3"><GitBranchIcon className="mt-0.5 size-8 shrink-0 text-primary" /><div className="min-w-0"><h2 className="truncate text-lg font-semibold">{plugin.name}</h2><p className="text-sm text-muted-foreground">{decoded.definition.nodes.length} 个精确 Action 节点 · {decoded.levels.length} 个阶段 · 最大并行 {decoded.maxParallelism}</p></div></div>
          <div className="flex flex-wrap gap-2">
            <LoadingButton variant="outline" size="sm" loading={preflightLoading} disabled={!exactIdentity || !selectedSupport.available || executionTarget === 'DESKTOP' && !desktopSessionReady} onClick={() => void refreshPreflight()}><RefreshCwIcon className="size-4" />重新预检</LoadingButton>
            {run && !TERMINAL_RUN_STATUSES.has(run.status) ? <LoadingButton variant="destructive" size="sm" loading={canceling} onClick={() => void cancel()}><SquareIcon className="size-3.5" />取消运行</LoadingButton> : <LoadingButton size="sm" loading={starting} disabled={!canStart} title={selectedSupport.reason} onClick={() => void start()}><PlayIcon className="size-4" />{executionTarget === 'CLOUD' ? '开始 Cloud 运行' : '开始本地运行'}</LoadingButton>}
          </div>
        </section>
        ...
        {upgradeSuggestions.length > 0 && (
          <Card size="sm">
            <CardHeader><CardTitle>可升级依赖</CardTitle><CardDescription>以下版本仍满足发布时声明的范围与 Action 契约。平台不会修改当前发行版；采纳后需创建、验证并发布新的工作流版本。</CardDescription></CardHeader>
            <CardContent className="space-y-2">
              {upgradeSuggestions.map((suggestion) => (
                <div key={suggestion.node_id} className="flex flex-col gap-1 rounded-lg border bg-muted/30 p-3 text-sm sm:flex-row sm:items-center sm:justify-between">
                  <div><p className="font-medium">{suggestion.node_id} · {suggestion.suggested_target.action_id}</p><p className="text-xs text-muted-foreground">{suggestion.current_version} → {suggestion.suggested_version} · 范围 {suggestion.declared_version_range}</p></div>
                  <Badge variant="outline">仅建议</Badge>
                </div>
              ))}
              <LoadingButton className="w-full sm:w-auto" loading={adoptingUpgrades} onClick={() => void adoptUpgrades()}>采纳到草稿</LoadingButton>
            </CardContent>
          </Card>
        )}
        ...
        {!run && (
          <Card>
            <CardHeader><CardTitle>运行输入</CardTitle><CardDescription>当前将在{executionTarget === 'CLOUD' ? ' Cloud' : '本地桌面端'}执行；字段来自整体 input schema，大文件只接受 ArtifactRef。</CardDescription></CardHeader>
            <CardContent><WorkflowInputForm schema={decoded.definition.input_schema} value={input} issues={issues} disabled={starting} onChange={(next) => { setInput(next); setIssues([]); }} /></CardContent>
          </Card>
        )}
        ...
        <Card>
          <CardHeader><CardTitle>只读执行图</CardTitle><CardDescription>点击节点查看精确 release、Action 契约、重试和结构化错误；图中不提供拖拽或改线入口。</CardDescription></CardHeader>
          <CardContent><WorkflowDag definition={decoded.definition} levels={decoded.levels} attempts={attemptsForDag(run)} /></CardContent>
        </Card>
      </div>
    </div>
  );
