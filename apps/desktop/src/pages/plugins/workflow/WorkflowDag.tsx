...
export function WorkflowDag({ definition, levels, attempts = [] }: { definition: WorkflowDefinitionV1; levels: string[][]; attempts?: WorkflowAttemptView[] }) {
    ...
    <div className="overflow-x-auto pb-2">
      <div className="grid min-w-max auto-cols-[minmax(220px,260px)] grid-flow-col items-start gap-8">
        {levels.map((level, levelIndex) => (
          <div key={levelIndex} className="relative space-y-3">
                ...
                >
                  ...
                  <summary className="cursor-pointer list-none p-3 marker:hidden">
                    <div className="flex items-start justify-between gap-2">
                      <div className="min-w-0"><p className="truncate text-sm font-semibold">{node.node_id}</p><p className="mt-0.5 truncate text-xs text-muted-foreground">{node.target.action_id}</p></div>
                      <StatusIcon status={status} />
                    </div>
                    <div className="mt-3 flex flex-wrap items-center gap-1.5">
                      <Badge variant="secondary">{STATUS_TEXT[status] ?? status}</Badge>
                      {attempt && attempt.attempt > 0 && <Badge variant="outline">重试 {attempt.attempt}</Badge>}
                    </div>
                  </summary>
                  <div className="space-y-2 border-t px-3 py-2.5 text-xs">
                    <p><span className="text-muted-foreground">依赖：</span>{node.depends_on.join(', ') || '无'}</p>
                    <p className="break-all"><span className="text-muted-foreground">Package：</span>{node.target.package_id}</p>
                    <p className="break-all"><span className="text-muted-foreground">Release：</span>{node.target.release_id}</p>
