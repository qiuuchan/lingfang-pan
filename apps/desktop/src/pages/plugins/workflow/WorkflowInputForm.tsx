...
function FieldIssues({ issues, path }: { issues: WorkflowInputIssue[]; path: string[] }) {
  ...
  return <FieldError className="flex flex-col gap-0.5 text-xs">{exact.map((issue, index) => <p key={`${issue.path}-${index}`}>{issue.message}</p>)}</FieldError>;
}
...
function SchemaField({
  ...
  if (!required && value === undefined) {
    return (
      <Field orientation="horizontal" className="justify-between gap-3 rounded-lg border border-dashed p-3">
  ...
  const heading = (
    <div className="flex items-center justify-between gap-2">
      <FieldLabel htmlFor={controlId}>{name}{required && <span className="ml-1 text-destructive">*</span>}</FieldLabel>
      {removable && (
  ...
  if (schema.$ref) {
    return <Field>{heading}<FieldDescription className="text-xs">ArtifactRef（仅填写平台返回的引用，不粘贴二进制正文）</FieldDescription><JsonField artifact value={value} disabled={disabled} onChange={onChange} /><FieldIssues issues={issues} path={path} /></Field>;
  }
  if ('const' in schema) {
    return <Field>{heading}<pre className="overflow-auto rounded-md border bg-muted/50 p-2 text-xs">{JSON.stringify(schema.const, null, 2)}</pre><FieldIssues issues={issues} path={path} /></Field>;
  }
  ...
  if (type === 'array') {
    return <Field>{heading}<JsonField value={value} disabled={disabled} onChange={onChange} /><FieldIssues issues={issues} path={path} /></Field>;
  }
  if (type === 'boolean') {
    return (
      <Field>
        <Field orientation="horizontal">
          <Checkbox id={controlId} checked={Boolean(value)} disabled={disabled} onCheckedChange={(checked) => onChange(Boolean(checked))} />
          <FieldLabel htmlFor={controlId}>{name}{required && <span className="ml-1 text-destructive">*</span>}</FieldLabel>
        </Field>
        <FieldIssues issues={issues} path={path} />
      </Field>
