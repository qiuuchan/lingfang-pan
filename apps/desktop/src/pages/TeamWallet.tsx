...
export function TeamWallet() {
  ...
  useEffect(() => {
    void (async () => {
      const [b, c] = await Promise.all([
        api<{ balanceCents: number }>('/api/teams/current/balance').catch((e) => {
          toast.error(`团队余额加载失败：${(e as ApiError).message}`);
          return null;
        }),
        api<CreditData>('/api/teams/current/credits').catch((e) => {
          toast.error(`团队灵石加载失败：${(e as ApiError).message}`);
          return null;
        }),
      ]);
  ...
  const openMarketplace = async (view: 'buyer' | 'seller') => {
    ...
    try {
      if (view === 'buyer') {
        const result = await api<MarketplaceOrderPage>(
          '/api/teams/current/plugin-purchases?pageSize=50'
        );
        setBuyerOrders(result.items);
      } else {
        const result = await api<MarketplaceStatementPage>(
          '/api/teams/current/marketplace-statement?pageSize=50&timezone=Asia%2FShanghai'
        );
        setSellerOrders(result.items);
        setSellerPendingCents(
          (result.summary.by_status?.PENDING_SETTLEMENT?.seller_cents ?? 0) +
            (result.summary.by_status?.REFUND_REQUESTED?.seller_cents ?? 0)
        );
      }
...
function AccountCard({
  ...
  return (
    <section className="relative overflow-hidden rounded-xl border bg-card p-5 shadow-sm">
      ...
      <p className="relative mt-2 min-h-9 text-xs leading-relaxed text-muted-foreground">
        {description}
      </p>
      <Button
        variant="outline"
...
function FinanceAction({
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className="group flex items-center gap-3 bg-card p-4 text-left transition-colors hover:bg-muted/35"
    >
      <span className="flex size-9 shrink-0 items-center justify-center rounded-full bg-muted text-muted-foreground transition-colors group-hover:bg-primary/10 group-hover:text-primary [&_svg]:size-4">
        {icon}
      </span>
...
function MarketplaceOrdersDialog({
  view,
  onOpenChange,
  loaded,
  rows,
        ...
        <div className="border-b bg-muted/25 px-5 py-4">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              ...
              {seller ? '卖家市场对账' : '插件购买订单'}
            </DialogTitle>
            <DialogDescription>
              {seller
                ? `待结算卖家金额 ${centsToYuan(sellerPendingCents)}，以订单状态实时聚合。`
                : '许可费用使用团队人民币余额；AI 调用产生的灵石费用不随许可退款返还。'}
            </DialogDescription>
          </DialogHeader>
        </div>
          ...
          {!loaded ? (
            <div className="flex flex-col gap-2">
              {Array.from({ length: 4 }).map((_, index) => (
                <Shimmer className="h-16 w-full rounded-lg" key={index} />
              ))}
            </div>
          ) : rows.length === 0 ? (
            <EmptyFinanceState
              icon={seller ? <StoreIcon /> : <ShoppingBagIcon />}
              text="暂无订单记录"
            />
          ) : (
            <div className="divide-y overflow-hidden rounded-xl border">
              {rows.map((row) => (
                <div className="flex items-start justify-between gap-4 bg-card p-3.5" key={row.id}>
                  <div>
                    <div className="text-sm font-medium">{row.package_name}</div>
                    <div className="text-xs text-muted-foreground">
                      {fmtTime(row.created_at)}
                      {row.refundable_until && !seller
                        ? ` · 退款申请截止 ${fmtTime(row.refundable_until)}`
                        : ''}
                    </div>
                  </div>
                  <div className="text-right">
                    <div className="text-sm font-semibold tabular-nums">
                      {centsToYuan(seller ? row.seller_cents : row.price_cents)}
                    </div>
                    <Badge variant="outline">{marketplaceStatus(row.status)}</Badge>
                  </div>
                </div>
              ))}
            </div>
