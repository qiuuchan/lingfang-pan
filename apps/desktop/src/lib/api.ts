...
// 磁盘上会残留旧令牌（重启后恢复回 A）。用「请求序号 + 串联执行」保证磁盘永远收敛到最后一次设置：
// - 每次写都排在上一写的 Promise 之后执行，天然按请求顺序落盘；
// - 排到自己时若已被更新的请求取代（seq < persistSeq）则跳过本次写——最新请求自己会写最新值，
//   不会出现旧值后落地，也避免了递归补写造成的永动 Promise 链。
let persistSeq = 0;
