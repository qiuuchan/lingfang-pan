...
describe('workflow definition boundary', () => {
  it('通过共享 engine 解码并产生稳定 DAG 阶段', () => {
    ...
    const result = decodeWorkflowDefinition(
  ...
  it('缺失、非法 JSON 与循环都返回可定位诊断', () => {
    expect(decodeWorkflowDefinition([], 'workflow.json')).toMatchObject({
      ok: false,
      diagnostics: [{ path: 'workflow.json' }],
    });
    expect(
      decodeWorkflowDefinition([{ path: 'workflow.json', content: '{' }], 'workflow.json')
    ).toMatchObject({ ok: false, diagnostics: [{ code: 'workflow_invalid' }] });
    const cyclic = {
      definition_version: '1',
      input_schema: objectSchema,
      output_schema: objectSchema,
    ...
    expect(
      decodeWorkflowDefinition(
        [{ path: 'workflow.json', content: JSON.stringify(cyclic) }],
        'workflow.json'
      )
    ).toMatchObject({ ok: false, diagnostics: [{ code: 'workflow_cycle_detected' }] });
  });
  ...
  it('采纳建议只替换精确 target，保留 declared range，并提升草稿版本', () => {
    ...
    const suggestedTarget = {
      ...currentNode.target,
      release_id: 'release.image.v2',
      sha256: 'b'.repeat(64),
      action_surface_sha256: 'c'.repeat(64),
    };
    const adopted = applyWorkflowUpgradeSuggestions(files, 'workflow.json', [
      {
        node_id: 'image',
        declared_version_range: '^1.0.0',
        current_version: '1.0.0',
        current_target: currentNode.target,
        suggested_version: '1.4.0',
        suggested_target: suggestedTarget,
        reason: 'compatible',
      },
    ]);
    const nextDefinition = JSON.parse(
      adopted.files.find((file) => file.path === 'workflow.json')!.content
    );
    const nextManifest = JSON.parse(
      adopted.files.find((file) => file.path === 'manifest.json')!.content
    );
  ...
  it('拒绝把过期 suggestion 应用到已经变化的精确 target', () => {
    ...
    expect(() =>
      applyWorkflowUpgradeSuggestions(files, 'workflow.json', [
        {
          node_id: 'image',
          declared_version_range: '^1.0.0',
          current_version: '0.9.0',
          current_target: { ...currentNode.target, release_id: 'stale' },
          suggested_version: '1.1.0',
          suggested_target: currentNode.target,
          reason: 'stale',
        },
      ])
    ).toThrow('已过期');
...
describe('workflow input schema', () => {
  ...
  it('只为必填字段建立初值，并能不可变更新/删除可选字段', () => {
    ...
    expect(setWorkflowInputValue(withEnabled, ['enabled'], undefined)).toEqual({
      prompt: '',
      count: 1,
    });
  });
  ...
  it('校验 required、边界、数组和未声明字段', () => {
    ...
    expect(issues.map((issue) => issue.path)).toEqual(
      expect.arrayContaining(['/prompt', '/count', '/tags', '/extra'])
    );
...
describe('workflow execution support', () => {
  it('精确安装项在桌面 Action adapter 可用时允许本地运行', () => {
    ...
    expect(support.cloud).toEqual({
      available: true,
      reason: '可通过 Cloud 执行；选择后将检查全部节点部署、配额与团队策略',
    });
    expect(
      assessWorkflowExecutionSupport(installed, {
        desktopShell: true,
        desktopActionAdapterAvailable: false,
      }).desktop
    ).toMatchObject({ available: false });
    expect(
      assessWorkflowExecutionSupport(installed, {
        desktopShell: true,
        desktopActionAdapterAvailable: true,
      }).desktop
    ).toEqual({ available: true, reason: '本地手动运行可用' });
  });

  it('浏览器和非精确安装项即使未来启用 adapter 也不能启动', () => {
    expect(
      assessWorkflowExecutionSupport(plugin(), {
        desktopShell: false,
        desktopActionAdapterAvailable: true,
      }).desktop.reason
    ).toContain('桌面应用');
  ...
  it('待激活工作流同时禁止本地和 Cloud 精确执行', () => {
    const pending = plugin({
      ...
      releaseId: 'r1',
      releaseSha256: digest,
      pendingActivation: { releaseId: 'r2' },
    });
