 ...
 */
export function decodeWorkflowDefinition(files: DraftFile[] | undefined, entry: string): WorkflowDefinitionResult {
  const file = definitionFile(files, entry);
  if (!file) {
    return { ok: false, diagnostics: [{ code: 'workflow_invalid', path: entry, message: '安装包缺少工作流入口文件' }] };
  }
  let input: unknown;
  try {
    ...
    return { ok: false, diagnostics: [{ code: 'workflow_invalid', path: entry, message: `工作流 JSON 无法解析：${detail}` }] };
  }
...
function sameActionTarget(left: WorkflowUpgradeSuggestion['current_target'], right: WorkflowUpgradeSuggestion['current_target']): boolean {
...
export function applyWorkflowUpgradeSuggestions(
  ...
  const suggestionByNode = new Map(suggestions.map((suggestion) => [suggestion.node_id, suggestion]));
  ...
  const definition = {
    ...
    nodes: decoded.definition.nodes.map((node) => {
      ...
      if (node.declared_version_range !== suggestion.declared_version_range || !sameActionTarget(node.target, suggestion.current_target)) throw new Error(`节点 ${node.node_id} 的升级建议已过期，请刷新后重试`);
      appliedNodeIds.push(node.node_id);
      return { ...node, target: suggestion.suggested_target };
    }),
  };
  if (appliedNodeIds.length !== suggestions.length) throw new Error('升级建议包含当前工作流中不存在的节点');
  ...
  catch { throw new Error('工作流 manifest.json 无法解析'); }
  if (manifest.runtime_type !== 'workflow') throw new Error('只有 workflow 插件可以采纳工作流升级建议');
  const version = nextWorkflowDraftVersion(String(manifest.version || ''));
  const nextFiles = files.map((file) => {
    if (file.path === entry) return { ...file, binary: false, content: `${JSON.stringify(validated, null, 2)}\n` };
    if (file.path === 'manifest.json') return { ...file, binary: false, content: `${JSON.stringify({ ...manifest, version }, null, 2)}\n` };
    return file;
  });
...
export function initialWorkflowInput(schema: PortableJsonSchemaNode): unknown {
  ...
  switch (primaryType(schema)) {
    ...
    case 'number':
    case 'integer': return schema.minimum ?? schema.exclusiveMinimum ?? 0;
    case 'string': return '';
...
function validateNode(
  ...
  if ('const' in schema && !sameJsonValue(value, schema.const)) addIssue(issues, path, '值必须与工作流常量一致');
  if (schema.enum && !schema.enum.some((item) => sameJsonValue(item, value))) addIssue(issues, path, '请选择允许的枚举值');
  if (value === null) {
  ...
  if (type === 'object') {
    if (!isRecord(value)) { addIssue(issues, path, '必须是对象'); return; }
  ...
  if (type === 'array') {
    if (!Array.isArray(value)) { addIssue(issues, path, '必须是数组'); return; }
    if (typeof schema.minItems === 'number' && value.length < schema.minItems) addIssue(issues, path, `至少需要 ${schema.minItems} 项`);
    if (typeof schema.maxItems === 'number' && value.length > schema.maxItems) addIssue(issues, path, `最多允许 ${schema.maxItems} 项`);
    value.forEach((item, index) => validateNode(schema.items ?? {}, item, `${path}/${index}`, issues));
    return;
  }
  if (type === 'string') {
    if (typeof value !== 'string') { addIssue(issues, path, '必须是文本'); return; }
    if (typeof schema.minLength === 'number' && value.length < schema.minLength) addIssue(issues, path, `至少需要 ${schema.minLength} 个字符`);
    if (typeof schema.maxLength === 'number' && value.length > schema.maxLength) addIssue(issues, path, `最多允许 ${schema.maxLength} 个字符`);
    if (schema.format === 'date-time' && Number.isNaN(Date.parse(value))) addIssue(issues, path, '必须是有效的日期时间');
    if (schema.format === 'uuid' && !/^[0-9a-f]{8}-[0-9a-f]{4}-[1-8][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(value)) addIssue(issues, path, '必须是有效的 UUID');
    return;
  }
  if (type === 'boolean') {
  ...
  if (type === 'number' || type === 'integer') {
    ...
    if (typeof schema.minimum === 'number' && value < schema.minimum) addIssue(issues, path, `不能小于 ${schema.minimum}`);
    if (typeof schema.maximum === 'number' && value > schema.maximum) addIssue(issues, path, `不能大于 ${schema.maximum}`);
    if (typeof schema.exclusiveMinimum === 'number' && value <= schema.exclusiveMinimum) addIssue(issues, path, `必须大于 ${schema.exclusiveMinimum}`);
    if (typeof schema.exclusiveMaximum === 'number' && value >= schema.exclusiveMaximum) addIssue(issues, path, `必须小于 ${schema.exclusiveMaximum}`);
    if (typeof schema.multipleOf === 'number' && Math.abs(value / schema.multipleOf - Math.round(value / schema.multipleOf)) > 1e-9) addIssue(issues, path, `必须是 ${schema.multipleOf} 的倍数`);
    return;
  }
  if (type === 'null' && value !== null) addIssue(issues, path, '值必须为空');
}

export function validateWorkflowInput(schema: PortableJsonSchemaNode, value: unknown): WorkflowInputIssue[] {
...
export function assessWorkflowExecutionSupport(
  plugin: LoadedPlugin,
  options: { desktopShell: boolean; desktopActionAdapterAvailable?: boolean } = { desktopShell: false },
): WorkflowExecutionSupport {
  const adapterAvailable = options.desktopActionAdapterAvailable ?? DESKTOP_WORKFLOW_ACTION_ADAPTER_AVAILABLE;
  let desktopReason = '本地手动运行可用';
  if (!options.desktopShell) desktopReason = '需要在灵坊桌面应用中运行';
  else if (!plugin.installationId || !plugin.packageId || !plugin.releaseId || !plugin.releaseSha256) desktopReason = '仅已安装的精确发行版可以创建桌面运行';
  ...
  if (!plugin.packageId || !plugin.releaseId || !plugin.releaseSha256) cloudReason = '只有精确已发布版本可以创建 Cloud 运行';
