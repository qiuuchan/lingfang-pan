      ...
      plan: {
        ...
        desktop_executor:
          executionTarget === 'DESKTOP'
            ? { session_id: 'session-1', inventory_sha256: 'e'.repeat(64) }
            : null,
      },
      attempts: [],
    },
...
describe('workflow start transport', () => {
  ...
  it('creates CLOUD runs through the shared workflow HTTP ledger', async () => {
    ...
    await expect(startWorkflowRun(request('CLOUD'))).resolves.toMatchObject({
      id: 'run-cloud',
      execution_target: 'CLOUD',
    });
    expect(apiMock).toHaveBeenCalledWith('/api/workflows/runs', {
      method: 'POST',
      body: expect.objectContaining({ execution_target: 'CLOUD' }),
    });
  ...
  it('keeps DESKTOP run creation inside the native session bridge', async () => {
    ...
    await expect(startWorkflowRun(request('DESKTOP'))).resolves.toMatchObject({
      id: 'run-desktop',
      execution_target: 'DESKTOP',
    });
    expect(tauriInvokeMock).toHaveBeenCalledWith('workflow_executor_start_run', {
      request: expect.objectContaining({ execution_target: 'DESKTOP' }),
    });
