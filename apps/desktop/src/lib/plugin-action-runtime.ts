...
type ActionListResponse = { release_id: string; package_id: string; sha256: string; actions: unknown[] };
...
export function ensureNativeActionBridgeListener(): Promise<void> {
  ...
  nativeBridgeListener = listen<NativeActionBridgePayload>('plugin-action-bridge-call', (event) => {
    ...
    void (async () => {
      try {
        if (!payload || typeof payload.request_id !== 'string' || typeof payload.parent_invocation_id !== 'string' || !payload.caller) throw actionError('action_runtime_unavailable', 'Native Action bridge payload 无效');
        const result = await invokeInstalledPluginAction(payload.caller, payload.args, {}, { parentInvocationId: payload.parent_invocation_id });
        await tauriInvoke('respond_plugin_action_bridge', { requestId: payload.request_id, result });
      } catch (caught) {
        const error = stableError(caught);
        await tauriInvoke('respond_plugin_action_bridge', { requestId: payload?.request_id ?? '', error: { name: error.name, message: error.message, code: error.code, status: error.status, requestId: error.requestId } }).catch(() => undefined);
      }
    })();
...
function stableError(error: unknown, fallback = 'action_execution_failed'): DesktopPluginActionError {
...
function manifestFromPlugin(plugin: LoadedPlugin) {
  ...
  if (!source) {
    ...
    if (manifestFile) {
      try { source = JSON.parse(manifestFile.content); } catch { /* handled by schema error below */ }
    }
  }
  const parsed = PluginManifest.safeParse(source);
  if (!parsed.success) throw actionError('action_dependency_denied', '当前插件 manifest 无法用于跨插件调用');
  return parsed.data;
}
...
function resolveInstalledDependency(
  ...
  if (!target) throw actionError('action_runtime_unavailable', `依赖插件尚未安装：${dependency.dependency_id}`);
  if (target.activeRelease.dependencyStatus !== 'ready') {
    throw actionError('action_runtime_unavailable', `依赖插件运行环境尚未就绪：${dependency.dependency_id}`);
  }
  if (!satisfiesActionVersionRange(target.activeRelease.version, dependency.release_version_range)) {
    throw actionError('action_contract_mismatch', `依赖插件版本不满足 ${dependency.release_version_range}`);
  }
  return target;
}
...
function parseActionSurface(value: unknown): ActionSurface | null {
  ...
  return surface.success && /^[a-f0-9]{64}$/.test(digest) ? { ...surface.data, action_surface_sha256: digest } : null;
}
...
async function resolveExactTarget(
  ...
  const response = await deps.api<ActionListResponse>(`/api/plugin-releases/${encodeURIComponent(installation.activeRelease.releaseId)}/actions`);
  if (response.package_id !== installation.packageId
    || response.release_id !== installation.activeRelease.releaseId
    || response.sha256 !== installation.activeRelease.sha256) {
    throw actionError('action_contract_mismatch', '依赖插件的服务端身份与本机 active release 不一致');
  }
  const action = response.actions.map(parseActionSurface).find((candidate) => candidate?.action_id === dependency.action_id);
  if (!action) throw actionError('action_not_found', `依赖未导出 Action：${dependency.action_id}`);
  if (!satisfiesActionVersionRange(action.action_contract_version, dependency.action_contract_version_range)) {
    throw actionError('action_contract_mismatch', `Action contract 不满足 ${dependency.action_contract_version_range}`);
  }
  return {
...
async function pollTerminal(id: string, deadlineAt: number, deps: PluginActionHostDependencies): Promise<ActionInvocationSummaryType> {
  while (deps.now() <= deadlineAt + 5_000) {
    const invocation = ActionInvocationSummary.parse(await deps.api(`/api/plugin-actions/invocations/${encodeURIComponent(id)}`));
...
function terminalOutput(invocation: ActionInvocationSummaryType): Record<string, unknown> {
  ...
  const fallback = invocation.status === 'CANCELED'
    ? 'action_cancelled'
    : invocation.status === 'TIMED_OUT'
      ? 'action_timeout'
      : 'action_execution_failed';
  throw actionError(ActionErrorCode.safeParse(invocation.error_code).success ? invocation.error_code : fallback, invocation.error_message || '插件 Action 调用失败');
}

async function invokeArtifactCapability(invocationId: string, kind: string, args: unknown, deps: PluginActionHostDependencies): Promise<unknown> {
  const input = args && typeof args === 'object' && !Array.isArray(args) ? args as Record<string, unknown> : {};
  const suffix = kind === 'artifacts.create' ? '' : kind === 'artifacts.materialize' ? '/materialize' : kind === 'artifacts.import' ? '/import' : null;
  if (suffix === null) throw actionError('action_dependency_denied', `Client Action 不允许调用能力：${kind}`);
  const result = await deps.api<Record<string, unknown>>(`/api/plugin-actions/invocations/${encodeURIComponent(invocationId)}/artifacts${suffix}`, { method: 'POST', body: input });
  if (kind === 'artifacts.materialize') return {
...
export async function invokeInstalledPluginAction(
  ...
  if (!dependencyId) throw actionError('action_dependency_denied', 'actions.call 缺少 dependency_id');
  if (!input || typeof input !== 'object' || Array.isArray(input)) throw actionError('action_input_invalid', 'Action input 必须是 JSON 对象');
  ...
  const dependency = manifest.action_dependencies.find((candidate) => candidate.dependency_id === dependencyId);
  if (!dependency) throw actionError('action_dependency_denied', `插件未声明 Action 依赖：${dependencyId}`);
  ...
  const invocation = ActionInvocationSummary.parse(await deps.api('/api/plugin-actions/invocations', {
    method: 'POST',
    body: {
      target: resolved.target,
      input,
      request_idempotency_key: requestIdempotencyKey,
      ...(typeof hint === 'string' ? { effect_idempotency_key: await effectKey(plugin, caller, dependencyId, resolved.target, hint, deps) } : {}),
  ...
  if (invocation.status === 'RUNNING') return terminalOutput(await pollTerminal(invocation.id, deadlineAt, deps));

  await deps.api(`/api/plugin-actions/invocations/${encodeURIComponent(invocation.id)}/claim`, { method: 'POST' });
  try {
    const output = resolved.action.execution.runtime_type === 'client'
      ? await (async () => {
          const handler = await deps.tauriInvoke<ClientActionHandlerResponse>('workflow_executor_read_client_action_handler', { target: resolved.target });
          ...
          return deps.executeClientAction({
            ...
            onCapability: (kind, nestedArgs) => kind === 'actions.call'
              ? invokeInstalledPluginAction(targetPlugin, nestedArgs as ActionCallArgs, deps, { parentInvocationId: invocation.id })
              : invokeArtifactCapability(invocation.id, kind, nestedArgs, deps),
          });
        })()
      : resolved.action.execution.runtime_type === 'nodejs' || resolved.action.execution.runtime_type === 'python'
        ? await (async () => {
