...
/// 将 epoch 秒 + 毫秒转为 RFC 3339 / ISO 8601 UTC 字符串（`YYYY-MM-DDTHH:MM:SS.mmmZ`）。
///
/// 历史 bug（Task 4a「Invalid Date」）：旧实现 `format!("{}.{:03}Z", secs, millis)` 产出
/// 形如 `1719045678.123Z` 的「epoch 秒.毫秒Z」，既非 ISO 8601 也非 RFC 2822，浏览器
/// `new Date(...)` 无法解析，前端「启动时间」与对话时间均显示 Invalid Date。
///
/// 此处用 Howard Hinnant 的 civil_from_days 算法（仅整数运算，无外部依赖）把 epoch
/// 映射为 (年月日)，再拼时分秒。ISO 8601 字符串同样满足 `String::cmp` 字典序排序
/// （与旧用法 store 内 `started_at.cmp(...)` 排序语义兼容），故不破坏会话排序。
pub fn epoch_to_iso8601(secs: u64, millis: u32) -> String {
    let secs_of_day = secs % 86400;
    let hour = secs_of_day / 3600;
    let min = (secs_of_day % 3600) / 60;
    let sec = secs_of_day % 60;

    // civil_from_days：days since 1970-01-01 → (year, month, day)。算法来源 Howard Hinnant。
    let days = (secs / 86400) as i64;
    let z = days + 719468;
    // Rust 整除向零截断；用 floor div 保证 era 对负 days（1970 前）仍正确。
...
mod tests {
    ...
    // === Task 4a：epoch_to_iso8601 必须产出浏览器 new Date() 可解析的 RFC 3339 字符串。 ===

    #[test]
    fn epoch_to_iso8601_known_instant() {
        // 1_719_045_678 = 2024-06-22T08:41:18Z（UTC，手工验算：days=19896, 余 31278s = 8h41m18s）。
        let s = epoch_to_iso8601(1_719_045_678, 123);
        assert!(s.starts_with("2024-06-22T08:41:18"), "got {s}");
        assert!(s.ends_with(".123Z"), "got {s}");
    }

    #[test]
    fn epoch_to_iso8601_is_lexicographically_sortable() {
        // 后端按 String::cmp 排序会话时间，ISO 8601 字典序 == 时间序（与旧 epoch 语义兼容）。
        let a = epoch_to_iso8601(1_719_045_678, 0);
        let b = epoch_to_iso8601(1_719_045_999, 0);
        assert!(a < b, "{a} should sort before {b}");
    }

    #[test]
    fn epoch_to_iso8601_rolls_day_boundary() {
        // 0 = 1970-01-01T00:00:00Z；86400 = 次日；验证跨日/月/年进位正确。
        assert_eq!(&epoch_to_iso8601(0, 0)[..10], "1970-01-01");
        assert_eq!(&epoch_to_iso8601(86_400, 0)[..10], "1970-01-02");
        // 2024 是闰年，2 月有 29 天；1_709_251_200 = 2024-03-01T00:00:00Z。
        assert_eq!(&epoch_to_iso8601(1_709_251_200, 0)[..10], "2024-03-01");
    }

    #[test]
    fn now_string_is_valid_iso8601() {
        // now_string 不能再产出 `epoch.毫秒Z`（会让前端 Invalid Date）。
        let s = now_string();
        assert!(
            regex_like_is_iso8601(&s),
            "now_string() should be ISO 8601, got {s}"
        );
    }

    // 轻量 ISO 8601 校验（避免引入 regex 依赖）：YYYY-MM-DDTHH:MM:SS(.fff)?Z
    fn regex_like_is_iso8601(s: &str) -> bool {
        let bytes = s.as_bytes();
        if bytes.len() < 20 || !s.ends_with('Z') {
            return false;
        }
        let is = |i: usize, c: u8| bytes.get(i).copied() == Some(c);
        let dig = |i: usize| bytes.get(i).map_or(false, u8::is_ascii_digit);
        // 4-2-2 T 2:2:2 . 3 Z
        (0..4).all(|i| dig(i))
            && is(4, b'-')
            && dig(5) && dig(6)
