...
/// P0-8 安装器协议文本（占位内容，待法务定稿）：机制真实可用，文本为占位。
/// 版本号与后端 agreementVersions 对齐（当前 v1），升版时此处同步更新。
pub const USER_AGREEMENT_TITLE: &str = "灵坊用户协议（占位 v1）";
pub const USER_AGREEMENT_TEXT: &str =
    "【本文本为占位内容，待法务定稿】\n\n\
     1. 您确认已阅读、理解并接受本协议全部条款。\n\
     2. 您须对使用本软件的行为及产生的内容负责，不得用于任何违法违规用途。\n\
     3. 平台有权依据法律法规与社区规范对违规内容进行处理。\n\
     4. 本协议的解释、变更与争议解决均适用中华人民共和国相关法律。\n\
     5. 本协议为 Beta 测试阶段占位文本，正式版本以法务定稿稿为准。";

pub const PRIVACY_POLICY_TITLE: &str = "灵坊隐私政策（占位 v1）";
pub const PRIVACY_POLICY_TEXT: &str =
    "【本文本为占位内容，待法务定稿】\n\n\
     1. 我们仅在为您提供服务所必需的范围内收集与使用信息。\n\
     2. 注册与插件上传时，我们记录您已同意的协议版本，用于合规留痕与举证。\n\
     3. 我们不会向无关第三方出售您的个人信息；数据共享须符合法律法规要求。\n\
     4. 您有权查询、更正您的个人信息，并可依流程申请删除。\n\
     5. 本政策为 Beta 测试阶段占位文本，正式版本以法务定稿稿为准。";
