...
async function setupPluginWorkbench(page: Page) {
  await page.addInitScript(
    ({ publishedPackage, publishedRelease }) => {
  ...
  await page.route('**/api/plugin-registry/manage', (route) =>
    route.fulfill({
      status: 200,
      contentType: 'application/json',
  ...
  await page.route('**/api/plugin-registry/team', (route) =>
    route.fulfill({
      status: 200,
      contentType: 'application/json',
  ...
  await page.route('**/api/plugin-registry/marketplace', (route) =>
    route.fulfill({
      status: 200,
      contentType: 'application/json',
      body: JSON.stringify({ items: [] }),
  ...
  await page.route(`**/api/plugin-packages/${PACKAGE_ID}`, (route) =>
    route.fulfill({
      status: 200,
      contentType: 'application/json',
      body: JSON.stringify({
...
test('本地制品发布保留团队版本，市场失败只重试提审', async ({ page }) => {
  ...
  await page
    .getByPlaceholder('选择 .lfplugin 文件（开发环境可输入路径）')
    .fill('/tmp/external-demo.lfplugin');
...
test('移动视口可查看来源与四轴状态且无横向溢出', async ({ page }) => {
  ...
  expect(
    await page.evaluate(() => document.querySelector('main')!.getBoundingClientRect().width >= 300)
  ).toBe(true);
...
test('插件中心顶部 Tab 保持横向等分排列', async ({ page }) => {
  ...
  expect(
    Math.max(...tabBoxes.map((box) => box!.width)) - Math.min(...tabBoxes.map((box) => box!.width))
  ).toBeLessThan(2);
});
