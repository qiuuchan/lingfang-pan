...
- **任务队列**：拖拽排序、状态筛选（全部/等待/执行中/完成/失败）、统计、批量删除
- **自动重试（可配）**：勾选后失败任务按「间隔秒」延迟重试，达「上限」次仍失败则取消（保持失败）；原任务就地重试（保留队列位置与计数），余额不足不重试；卡片可手动 ↻ 重试（重置计数）
- **自定义输出文件夹**：
...
- **0.4.0**：新增音频声音克隆工作流 + 顶部 Tab 切换；插件更名为「RBFLow 创意工坊」；新增 `audio.generate` 能力；自动重试增强（可设重试上限与间隔秒、原任务就地重试、超限取消）。
